...
MOB_INDEX_DATA *rand_figment_mob( char_data *ch )
{
  ...
  int		count = 0;
           ...
           lfig = ch->in_room->area->min_vnum;
           ufig = ch->in_room->area->max_vnum;
           ...
           while( !(figmentmob = get_mob_index( figmentnum ) ) )
 	  {
 		...
 		if (count == 20)
 		{
 			...
 			{
 			case 1:
 				figmentnum = 10398; // Death
 				break;
 			case 2:
 				figmentnum = 1100; // Beholder
 				break;
 			case 3:
 				figmentnum = 9819; // Undead Beholder
 				break;
 			case 4:
 				figmentnum = 3114; // Hassan
 				break;
 			case 5:
 				figmentnum = 12720; // Ghost of Patrick
 				break;
 			case 6:
 				figmentnum = 2485; // Red Dragon
 				break;
 			case 7:
 				figmentnum = 2486; // Blue Dragon
 				break;
 			case 8:
 				figmentnum = 1021; // Spike the Dog
 				break;
 			case 9:
 				figmentnum = 48012; // Dravar the Lich
 				break;
 			case 10:
 				figmentnum = 9899; // Dargoth the Lich
 				break;
			}
...
void look_sky ( char_data * ch )
{
   ...
   ch->println("You gaze up towards the heavens and see:");


  precip = weather_info[ch->in_room->sector_type].sky;

   if (precip > 1)                 
   {
  	ch->println("There are some clouds in the sky so you cannot see anything else.");
   	return;
   }
       ...
       else /* plot sun or stars */
       {
         ...
         else
         {
           switch (star_map[linenum][(MAP_WIDTH + i - starpos)%MAP_WIDTH])
           {
             ...
             case 'O'    : strcat(buf,"`S ");  break;
             case 'B'    : strcat(buf,"`B ");  break;
             case 'P'    : strcat(buf,"`M ");  break;
...
char *format_obj_to_char_new( OBJ_DATA *obj, char_data *ch, bool fShort )
{
    ...
    if ( IS_OBJ2_STAT(obj, OBJEXTRA2_SHADOWPLANE)   )	strcat( buf, game_settings->aura_shadowplane);

    if ( IS_AFFECTED(ch, AFF_DETECT_GOOD)
		 &&  IS_OBJ_STAT(obj,OBJEXTRA_BLESS))		strcat( buf, game_settings->aura_good);
    if ( IS_AFFECTED3(ch, AFF3_DETECT_TAINT)
         && IS_OBJ_STAT(obj, OBJEXTRA_MAGIC))		strcat( buf, "`#`m(`rT`ma`gi`mn`rt`me`gd`m)`^");
...
//		&& IS_AFFECTED2( ch, AFF2_DET_TRAPS ))	strcat( buf, "{#{r(Trapped){& "		);
	...
	{
		if (IS_SET (obj->corpse_flags, DAM_BASH))
			strcat(buf, "`#`w(`bBattered`w)`^ ");
		if (IS_SET (obj->corpse_flags, CORPSE_DAM_PIERCE))
			strcat(buf, "`#`w(`rGouged`w)`^ ");
		if (IS_SET (obj->corpse_flags, CORPSE_DAM_SLASH))
			strcat(buf, "`#`w(`rSlashed`w)`^ ");
		if (IS_SET (obj->corpse_flags, CORPSE_DAM_FIRE))
			strcat(buf, "`#`w(`RB`ru`RR`rn`RE`rd`w)`^ ");
		if (IS_SET (obj->corpse_flags, CORPSE_DAM_COLD))
			strcat(buf, "`#`w(`CF`cr`CO`cz`CE`cn`w)`^ ");
		if (IS_SET (obj->corpse_flags, CORPSE_DAM_LIGHTNING))
			strcat(buf, "`#`w(`SB`wl`SA`wc`SK`we`SN`we`SD`w)`^ ");
		if (IS_SET (obj->corpse_flags, CORPSE_DAM_ACID))
			strcat(buf, "`#`w(`YL`yi`Yq`yu`Yi`yf`Yi`ye`Yd`w)`^ ");	
		if (IS_SET (obj->corpse_flags, CORPSE_DAM_NEGATIVE))
			strcat(buf, "`#`w(`ST`ww`SI`ws`ST`we`SD`w)`^ ");	
		if (IS_SET (obj->corpse_flags, CORPSE_DAM_POISON))
			strcat(buf, "`#`w(`gSickly`w)`^ ");	
		if (IS_SET (obj->corpse_flags, CORPSE_DAM_HOLY))
			strcat(buf, "`#`w(`rIncinerated`w)`^ ");	
		if (IS_SET (obj->corpse_flags, CORPSE_DAM_ENERGY))
			strcat(buf, "`#`w(`SC`wh`SA`wr`SR`we`SD`w)`^ ");	
		if (IS_SET (obj->corpse_flags, CORPSE_DAM_DISEASE))
			strcat(buf, "`#`w(`SDiseased`w)`^ ");	
		if (IS_SET (obj->corpse_flags, CORPSE_DAM_DROWNING))
			strcat(buf, "`#`w(`BBloated`w)`^ ");	
		if (IS_SET (obj->corpse_flags, CORPSE_DAM_LIGHT))
			strcat(buf, "`#`w(`SC`wh`SA`wr`SR`we`SD`w)`^ ");	
		if (IS_SET (obj->corpse_flags, CORPSE_DAM_HARM))
			strcat(buf, "`#`w(`cRigid`w)`^ ");	
		if (IS_SET (obj->corpse_flags, CORPSE_DAM_SOUND))
			strcat(buf, "`#`w(`bBattered`w)`^ ");	
	}
...
void show_char_to_char_0( char_data *victim, char_data *ch )
{
    ...
    if ( IS_SET(victim->config2, CONFIG2_BEHOLDER_EYE) ) strcat( buf, "`#`R(Glowing Eye)`& " );

    if ( (IS_AFFECTED2(ch, AFF2_SHADOWSIGHT) || IS_AFFECTED2(ch, AFF2_SHADOWPLANE) || HAS_HOLYLIGHT(ch)) 
    &&  IS_AFFECTED2(victim, AFF2_SHADOWPLANE)) strcat( buf, "`#`S(Shadow)`& " );

    if (HAS_HOLYLIGHT(ch) 
    && IS_AFFECTED2(victim, AFF2_POLYMORPH))   strcat( buf, "`#`G(Polymorph)`& " );

    if ( (IS_AFFECTED2(ch, AFF2_HIGHLANDER) || HAS_HOLYLIGHT(ch)) 
    &&  IS_AFFECTED2(victim, AFF2_HIGHLANDER)) strcat( buf, "`#`Y(Highlander)`& " );
    
    if ((HAS_HOLYLIGHT(ch) || IS_VAMPIRE(ch))
    && IS_VAMPIRE(victim))   strcat( buf, "`#`R(Vampire)`& " );

    if ((HAS_HOLYLIGHT(ch) || IS_WEREBEAST(ch))
    && IS_WEREWOLF(victim))   strcat( buf, "`#`R(WereWolf)`& " );

    if ((HAS_HOLYLIGHT(ch) || IS_WEREBEAST(ch))
    && IS_WERERAT(victim))   strcat( buf, "`#`R(WereRat)`& " );

    if ((HAS_HOLYLIGHT(ch) || IS_WEREBEAST(ch))
    && IS_WEREBOAR(victim))   strcat( buf, "`#`R(WereBoar)`& " );

    if ((HAS_HOLYLIGHT(ch) || IS_WEREBEAST(ch))
    && IS_WEREBEAR(victim))   strcat( buf, "`#`R(WereBear)`& " );

    if ((HAS_HOLYLIGHT(ch) || IS_WEREBEAST(ch))
    && IS_WERETIGER(victim))   strcat( buf, "`#`R(WereTiger)`& " );

    if ((HAS_HOLYLIGHT(ch) || IS_WEREBEAST(ch))
    && IS_WERESPIDER(victim))   strcat( buf, "`#`R(WereSpider)`& " );

    if ((HAS_HOLYLIGHT(ch) || IS_WEREBEAST(ch))
    && IS_WERECOBRA(victim))   strcat( buf, "`#`R(WereCobra)`& " );

    if ((HAS_HOLYLIGHT(ch) || IS_WEREBEAST(ch))
    && IS_WEREBAT(victim))   strcat( buf, "`#`R(WereBat)`& " );

    if ((HAS_HOLYLIGHT(ch) || IS_WEREBEAST(ch))
    && IS_WERECROCODILE(victim))   strcat( buf, "`#`R(WereCroc)`& " );

    if ((HAS_HOLYLIGHT(ch) || IS_AFFECTED2(ch, AFF2_ETHEREAL)) && 
    IS_AFFECTED2(victim, AFF2_ETHEREAL)) strcat( buf, "`#`W(Mist)`& " );
...
/////////////////////////////////////////////////////////////////////
	// subdued players
	if (IS_SUBDUED(victim) && IS_VAMPIRE(victim))
	{
		ch->printlnf( "%s%s is lying here mortally wounded.",
	...
	if (IS_RIDING(victim))
	{
			...
			((IS_RIDING(victim) != ch)?PERS( IS_RIDING(victim), ch ):"you"));
		return;
	}
...
//////////////////////////////////////////////////////////////
	...
	case POS_STUNNED:
		...
		break;
	case POS_SLEEPING:
		if(!IS_NPC(victim) && victim->race == race_lookup("beholder"))
		{
			strcat( buf, " is floating here asleep." );
			break;
		} else
	...
	case POS_STANDING:
		...
		{
			strcat( buf, " is standing here immobilized." );
			break;
		} else
		if(!IS_NPC(victim) && victim->race == race_lookup("beholder"))
		{
		...
		if(!IS_NPC(victim) && 
		   IS_SET(victim->config2, CONFIG2_POSING) && 
		   victim->posing != NULL)
		{
			strcat( buf, victim->posing);
		}
		else
		strcat( buf, char_position_text(victim));
		break;
...
void show_char_to_char_1( char_data *victim, char_data *ch )
{
	...
	if ( victim->description[0] != '\0' )
	{
    		...
    		{
        		ch->print(victim->poly_description );
		} else {
	...
	ch->printlnf( "%s eyes are glowing bright red.", PERS(victim,ch) );
    }
    if (IS_VAMPAFF(victim, VAM_FANGS)) {
	 ch->printlnf( "%s has a pair of long, pointed fangs.", PERS(victim,ch) );
    }
    if (IS_VAMPAFF(victim, VAM_CLAWS)) {
	ch->printlnf( "%s has sharp claws protruding from under their finger nails.", PERS(victim,ch) );
    }
    
    if(IS_AFFECTED2(victim, AFF2_POLYMORPH) && !HAS_HOLYLIGHT(ch) && ch!=victim) {
	return;
    }

    if(IS_SET(victim->config2, CONFIG2_BEHOLDER_EYE))
    {
	ch->printlnf( "%s central eye is `#`Rglowing red`^.", PERS(victim,ch) ); 
    }
    ...
    {
	ch->printlnf( "The subtle, alluring aroma of `#`Mjasmine`^ lingers about %s`x.", PERS(victim,ch) ); 
    } else
  ...
  if(IS_AFFECTED2(victim, AFF2_PREGNANT))
  {
    ...
    if( victim->pcdata->pregnant >= current_time + (7 * ICTIME_IRLSECS_PER_MONTH)) {
	ch->printlnf( "%s has a small round belly and is with child.", PERS(victim,ch) ); }

    	else {

    if( victim->pcdata->pregnant >= current_time + (4 * ICTIME_IRLSECS_PER_MONTH)) {
	ch->printlnf( "%s has a round belly and is with child.", PERS(victim,ch) ); }

    	else {

	ch->printlnf( "%s has a large round belly and is with child.", PERS(victim,ch) ); }
	}
  } // End Pregnant Look
...
void show_char_to_char( char_data *list, char_data *ch )
{
	...
	for ( rch = list; rch; rch = rch->next_in_room )
	{
		...
		if ( can_see( ch, rch ) )
		{
         		if ( IS_AFFECTED2( ch, AFF2_HALLUCINATE)  && number_percent() > 55 )
         		{
           			rch2 = rch;
           			rch = rand_figment( ch );
		...
		else if ( room_is_dark( ch->in_room ) && !IS_AFFECTED( ch, AFF_DARK_VISION)
	...
	if ( IS_AFFECTED2( ch, AFF2_HALLUCINATE))
	{
   		while( figchance != 5 )
   		{
				...
				if(!IS_SET(figment->act, ACT_IS_UNSEEN)) 
				{
 	    				ch->println( figment->long_descr );
				}
   	 		}
    			figchance += 1;
   		}
...
void do_consider( char_data *ch, char *argument )
{
    ...
    if(IS_SET(victim->dyn2, DYN2_BLUFF_STRONG))
		diffhp += abs(diffhp) * (get_skill(victim, gsn_bluff) / 300);
	if(IS_SET(victim->dyn2, DYN2_BLUFF_WEAK))
		diffhp -= abs(diffhp) * (get_skill(victim, gsn_bluff) / 300);
    ...
    if(IS_SET(victim->dyn2, DYN2_BLUFF_STRONG))
		difflevel += difflevel + (get_skill(victim, gsn_bluff) / 30);
	if(IS_SET(victim->dyn2, DYN2_BLUFF_WEAK))
		difflevel -= difflevel + (get_skill(victim, gsn_bluff) / 30);
...
void do_autolist(char_data *ch, char *)
{
	...
	ch->printlnf( "autosaycolorc %s", HAS_CONFIG2(ch,CONFIG2_AUTOSAYCOLOURCODES)?alon:aloff);	
	...
	if(!IS_NPC(ch) && IS_SET(ch->form, FORM_MOUNTABLE))
		ch->printlnf( "mountable      %s", IS_SET(ch->pcdata->pconfig, PCONFIG_MOUNTABLE)?alon:aloff);	
	if(IS_IMMORTAL(ch)){
	...
	if (IS_UNSWITCHED_MOB(ch) && ch->desc){
		if(ch->desc->colour_mode==CT_NOCOLOUR){
			...
			ch->println( "You are playing in `?color`x");	
		}
	}
...
void do_autosaymote(char_data *ch, char *)
{
	...
	ch->wraplnf("note: You can also use `=Cautosaycolorcodes`x to make your "
		"says automatically insert %s in front of color codes (so you don't "
		"talk with color).", colprefix);
}
/**************************************************************************/
void do_autosaycolourcodes(char_data *ch, char *)
{
    ...
    if (HAS_CONFIG2(ch,CONFIG2_AUTOSAYCOLOURCODES)){
		ch->println( "Say's will now allow colors in them.");
		REMOVE_CONFIG2(ch,CONFIG2_AUTOSAYCOLOURCODES);
	}else{
		ch->wraplnf( "Say's will automatically convert the color code character (%s) into %s%s "
			"so you will be able to talk color codes easily.",
...
void do_prompt(char_data *ch, char *argument)
{
	...
	if ( IS_NULLSTR(argument))
	{
		...
		{
			if (ch->desc->editor
			&&  ch->desc->editor != ED_CRAFT) // working in olc
			{
    ...
    if( !strcmp( argument, "basic" ) )
		...
		if (ch->desc->editor)
		{
			if(ch->desc->editor != ED_CRAFT)  // in olc editor
				strcpy( buf, "[`#`m%e`^ in `R%o`mv`R%O`g%Z`^ - %T`^ - %t%d>");
			else
				strcpy( buf, "`#`C[`cCrafting`C -`c %t%d`C]`^");
		}else{ // not in olc or crafting
...
void do_look( char_data *ch, char *argument )
{
	...
	if ( IS_NULLSTR(arg1) || !str_cmp( arg1, "auto" ) || !str_cmp( arg1, "brief" ))
	{
		...
		if(lockers->room_has_lockers(ch->in_room)){		
			ch->printlnf("     `=\x8d%s", 
				mxp_create_send(ch,"look treasury",
					FORMATF("`#`gA large treasury is here.`^")));
		}
	...
	if ( !str_cmp( arg1, "sky" ) )
	{
  		if ( !IS_OUTSIDE(ch) )
  		{
    			ch->println( "You can't see the sky indoors.");
    			return;
  		}
  		else
  		{
	...
	if (ch->in_room->sector_type == SECT_VEHICLE && !str_cmp(arg1, "out"))
	{
		...
		for ( building = ch->in_room->contents; building != NULL; building = building_next )
			...
			if ( building->item_type==ITEM_VEHICLE_EXIT || building->item_type==ITEM_VEHICLE_CONTROLS)
    			{
		...
		ch->println("You cannot see anything from here.");
		return;
	}
				...
				// description		
				{
					...
					if(str_len(firstname)>2){
						...
						{ // unique objects extended descriptions 
							...
							{
								if ( is_name( (char *)firstname, ed->keyword ) )
								break;
							}
							pdesc = get_extra_descr( firstname, obj->extra_descr );
							if( pdesc 
							&& (IS_IMMORTAL(ch) || !is_name("_", ed->keyword))){
								ch->sendpage(pdesc);
								return;
							}				
						}
...
void do_examine( char_data *ch, char *argument )
{
	...
	if ( ( obj = get_obj_here( ch, arg ) ) != NULL )
	{
		...
		case ITEM_FIREARM:
			ch->printlnf( "There are %d/%d rounds in the weapon.",
				obj->value[1],obj->value[0]);
			break;
...
void do_equipment( char_data *ch, char * )
{
	...
	for ( iWear = 0; iWear < MAX_WEAR; iWear++ )
	{
		...
		&&	 get_eq_char( ch, iWear ) == NULL )
 		...
 		if ( iWear == WEAR_2SECONDARY
 		&& (ch->race != race_lookup("thri-kreen")) 
 		&&	 get_eq_char( ch, iWear ) == NULL )
 			continue;

 		if ( iWear == WEAR_FINGER_L
 		&& (!IS_SET(ch->parts, PART_FINGERS)) 
 		&&	 get_eq_char( ch, iWear ) == NULL )
 			continue;

 		if ( iWear == WEAR_FINGER_R
 		&& (!IS_SET(ch->parts, PART_FINGERS)) 
 		&&	 get_eq_char( ch, iWear ) == NULL )
 			continue;

 		if ( iWear == WEAR_NECK_1
 		&& (!IS_SET(ch->parts, PART_HEAD)) 
 		&&	 get_eq_char( ch, iWear ) == NULL )
 			continue;

 		if ( iWear == WEAR_NECK_2
 		&& (!IS_SET(ch->parts, PART_HEAD)) 
 		&&	 get_eq_char( ch, iWear ) == NULL )
 			continue;
		...
		if ( iWear == WEAR_WRIST_L
 		&& (!IS_SET(ch->parts, PART_HANDS)) 
 		&&	 get_eq_char( ch, iWear ) == NULL )
 			continue;

		if ( iWear == WEAR_WRIST_R
 		&& (!IS_SET(ch->parts, PART_HANDS)) 
 		&&	 get_eq_char( ch, iWear ) == NULL )
 			continue;
		...
		if ( ( obj = get_eq_char( ch, iWear ) ) == NULL ){
			...
			continue;
 			...
 			//Damage Displays
 			char objdam[MSL] = "";
 			if (obj->condition < 100 && obj->condition >= 80)
 			{
 				sprintf(objdam," `#`r[Scratched]`^");
 			}
 			else
 			if (obj->condition < 80 && obj->condition >= 60)
 			{
 				sprintf(objdam," `#`r[Corroded]`^");
 			}
 			else
 			if (obj->condition < 60 && obj->condition >= 40)
 			{
				sprintf(objdam," `#`r[Ruined]`^");
			}
 			else
 			if (obj->condition < 40 && obj->condition >= 20)
 			{
 				sprintf(objdam," `#`r[Trashed]`^");
 			}
 			else
 			if (obj->condition < 20)
 			{
 				sprintf(objdam," `#`r[Destroyed]`^");
			}


		if ( can_see_obj( ch, obj ) )
		{
			ch->printlnf( "%s %s`=\xab", format_obj_to_char( obj, ch, true ), objdam);
...
void do_description( char_data *ch, char * )
{
	if(IS_NPC(ch))
    	...
    	if (ch->race == race_lookup("changeling") && IS_AFFECTED2(ch, AFF2_SHAPESHIFT))
    	{
		ch->println("Not while shapechanged.");
		return;
    	}
