...
void do_hurl( char_data *ch, char *argument )
{
	...
	if ( !dagger )
	{
		if(!(dagger = get_eq_char(ch, WEAR_CONCEALED)))
		{
		...
		else
		{
			if(number_percent( ) >= get_skill(ch,gsn_conceal)){
				act("You fumble as you pull $p out.",ch, dagger, NULL, TO_CHAR);
				act("$p falls next to $n.", ch, dagger, NULL, TO_ROOM);
				...
				act("You draw $p from it's hiding place!",ch, dagger, NULL, TO_CHAR);
				act("$p appears in $n's hands from nowhere!", ch, dagger, NULL, TO_ROOM);
				equip_char(ch, dagger, WEAR_WIELD);
...
void do_dervish( char_data *ch, char * )
{
	...
	if ( !polearm )
	{
		ch->println("You must be wielding a polearm to perform this dance.");
		return;
	}
	...
	act( "$n holds $p firmly and begins to whirl about like a dervish...", ch, polearm, NULL, TO_ROOM );
	act( "You hold $p firmly and begin to whirl like a maddened dervish...",  ch, polearm, NULL, TO_CHAR );
	...
	if ( !found )
	{
		...
		{
			act( "$n drops $p.", ch, polearm, NULL, TO_ROOM );
			act( "You drop $p.", ch, polearm, NULL, TO_CHAR );
			obj_from_char( polearm );
			obj_to_room( polearm, ch->in_room );
...
void do_conceal( char_data *ch, char * )
{
	...
	SET_CONFIG2( ch, CONFIG2_CONCEALED );
	act( "When no one is looking, you secretly conceal $p.", ch, weapon, NULL, TO_CHAR );

	return;
}

void do_spinblock( char_data *ch, char * )
{
	OBJ_DATA	*weapon = get_eq_char( ch, WEAR_WIELD );

	if ( !IS_IMMORTAL( ch ))
	{
		if (get_skill(ch,gsn_spinblock) == 0)
		{
			do_huh(ch,"");
			return;
		}
	...
	if ( !weapon )
	{
		ch->println("You must be wielding your staff in order to spin it.");
		return;
	}
	...
	{
		ch->println("This weapon would not provide an effective defense.");
		return;
	}
	else{
		if(IS_SET(ch->dyn2, DYN2_DOING_SPINBLOCK))
		{
			REMOVE_BIT( ch->dyn2, DYN2_DOING_SPINBLOCK );
			act( "You release your grip to spin $p.", ch, weapon, NULL, TO_CHAR );
		}
		else
		{
			SET_BIT( ch->dyn2, DYN2_DOING_SPINBLOCK );
			act( "You prepare to spin $p to defend yourself.", ch, weapon, NULL, TO_CHAR );
		}
	}
	return;
}
...
void do_chicanery( char_data *ch, char * )
{
	if(get_skill(ch, gsn_chicanery) > 0)
	{
		if(IS_SET(ch->dyn2, DYN2_CHICANERY))
		{
			ch->println("You stop doing slight of hand.");
			REMOVE_BIT(ch->dyn2, DYN2_CHICANERY);
		}
		else
		{
			ch->println("You start doing slight of hand.");
			SET_BIT(ch->dyn2, DYN2_CHICANERY);
		}
	}
	else
	{
...
void do_evade( char_data *ch, char * )
{
	if(get_skill(ch, gsn_evasion) > 0)
	{
		if(IS_SET(ch->dyn2, DYN2_EVADING))
		{
			ch->println("You stop looking for victims.");
			REMOVE_BIT(ch->dyn2, DYN2_EVADING);
		}
		else
		{
			ch->println("You begin studying the living targets around you.");
			SET_BIT(ch->dyn2, DYN2_EVADING);
		}
	}
	else
	{
...
void do_charge( char_data *ch, char *argument )
{
	...
	else if (IS_SET(ch->off_flags,OFF_FAST) || IS_AFFECTED(ch,AFF_HASTE))
			chance += 10;
	...
	if(mount && mount->move < mount->max_move/2 ) chance /= 2;
