...
void setup_altform(char_data *ch, int type, int altsubtype);
  ...
  for(obj = ch->carrying; obj != NULL; obj = obj_next)        
        {
            ...
            if(obj->condition>0 && number_percent()<=
			   ((chance + (obj->durability * chance / 10)) / 2)+20 
...
const flag_type *affect_get_bitvector_table_for_where(int where)
{
	...
	case WHERE_VAMPIRE:	return vampire_flags;	// character
	case WHERE_WEREBEAST:	return werebeast_flags;	// character
...
/**************************************************************************/
int projectiletype (const char *name)
{
    int type;
 
    for(type = 0; projectile_table[type].name != NULL; type++)
    {
        if(LOWER(name[0]) == LOWER(projectile_table[type].name[0])
        &&  !str_prefix(name,projectile_table[type].name))
            return projectile_table[type].type;
    }
 
    return PROJ_ARROW;
}
/**************************************************************************/
char *projectile_name( int projectile_type)
{
    int type;
 
    for(type = 0; projectile_table[type].name != NULL; type++)
        if(projectile_type == projectile_table[type].type)
            return projectile_table[type].name;
    return "arrow";
}
/**************************************************************************/
...
int check_immune(char_data *ch, int dam_type)
{
	...
	if((mark = get_eq_char(ch, WEAR_FACE)) != NULL
	&& mark->ospec_fun == ospec_lookup("ospec_chameleon_mark"))
	{
			strong_vs = mark->ospec_value[0];
			weak_vs = mark->ospec_value[1];
	}

    if(IS_SET(ch->imm_flags,bit))
   	immune = IS_IMMUNE;
    if ((IS_SET(ch->res_flags,bit) || strong_vs == bit) && immune != IS_IMMUNE)
   	immune = IS_RESISTANT;
    if ((IS_SET(ch->vuln_flags,bit) || weak_vs == bit))
    {
   	if(immune == IS_IMMUNE)
       		immune = IS_RESISTANT;
...
void reset_char(char_data *ch)
{
	...
	if(IS_WEREBEAST(ch) && ch->werecircle == 0)
	...
	if(IS_WEREFORM(ch) || IS_DRAGONFORM(ch)){
    ...
    for(stat = 0; stat < MAX_STATS; stat++)
    {
		...
		if(ch->altformed == true)
			ch->modifiers[stat] += ch->altform.modifier[stat];
		else
        ch->modifiers[stat] += race_table[ch->race]->stat_modifier[stat];
		//WereCircle boosts all positive and negative scores by 1 per circle
		//Providing a small change even in their normal form to balance vamps
		if(!IS_NPC(ch) 
		&& IS_WEREBEAST(ch) 
		&& ch->werecircle <= 10){
			int altmod;
			setup_altmodifiers(ch);
			if(ch->altform.modifier[stat] > 0)
				altmod = 1;
			else if(ch->altform.modifier[stat] < 0)
				altmod = -1;
			else
				altmod = 0;
			ch->modifiers[stat] += altmod * (11 - ch->werecircle);
		}
    }
		...
		for(af = OBJECT_AFFECTS(obj); af; af = af->next)
		{
			...
			if(af->where!=WHERE_OBJECTSPELL){
				...
				switch(af->location)
				{
					...
					case APPLY_ATTRIBUTES:  ch->modifiers[STAT_ST]  += mod;
											ch->modifiers[STAT_QU]  += mod;
											ch->modifiers[STAT_PR]  += mod;
											ch->modifiers[STAT_EM]  += mod;
											ch->modifiers[STAT_IN]  += mod;
											ch->modifiers[STAT_CO]  += mod;
											ch->modifiers[STAT_AG]  += mod;
											ch->modifiers[STAT_SD]  += mod;
											ch->modifiers[STAT_ME]  += mod;
											ch->modifiers[STAT_RE]  += mod;
											break;
...
void affect_apply_modifier( char_data *ch, AFFECT_DATA *paf, bool fAdd )						   
{
	...
	case APPLY_ATTRIBUTES:  ch->modifiers[STAT_ST]  += amount;
							ch->modifiers[STAT_QU]  += amount;
							ch->modifiers[STAT_PR]  += amount;
							ch->modifiers[STAT_EM]  += amount;
							ch->modifiers[STAT_IN]  += amount;
							ch->modifiers[STAT_CO]  += amount;
							ch->modifiers[STAT_AG]  += amount;
							ch->modifiers[STAT_SD]  += amount;
							ch->modifiers[STAT_ME]  += amount;
							ch->modifiers[STAT_RE]  += amount;
							break;
...
void affect_modify( char_data *ch, AFFECT_DATA *paf, bool fAdd )
{
    ...
    if( fAdd )
    {
		...
		case WHERE_AFFECTS3:
			...
			break;
		case WHERE_VAMPIRE:
			SET_BIT(ch->vampire, paf->bitvector);
			break;
		case WHERE_WEREBEAST:
			SET_BIT(ch->vampire, paf->bitvector);
			break;
		case WHERE_IMMUNE:
		...
		case WHERE_AFFECTS3:
			...
			break;
        case WHERE_VAMPIRE:
            REMOVE_BIT(ch->vampire, paf->bitvector);
            break;
        case WHERE_WEREBEAST:
            REMOVE_BIT(ch->vampire, paf->bitvector);
            break;
...
void affect_remove( char_data *ch, AFFECT_DATA *paf )
{
	...
	for(i = 0; !IS_NULLSTR(afteraff_table[i].affect_name);i++)
		if(!str_cmp(afteraff_table[i].affect_name, skill_table[paf->type].name))
			break;
	if(afteraff_table[i].affect_name != NULL)
		(*afteraff_table[i].afteraffect_fun )(ch, "");
...
void char_from_room( char_data *ch )
{
	...
	IS_SET(ch->in_room->room2_flags, ROOM2_SALTWATER_FISH)) {
		...
		ch->printlnf("You pack up your fishing gear and move on.");
	}
...
void char_to_room( char_data *ch, ROOM_INDEX_DATA *pRoomIndex )
{
	...
	affect_data *necro = NULL;
	if((necro = affect_find(ch->affected, gsn_necrotic_barrier)) != NULL)
	{	if(ch->in_room->death_energy < 1
		&& !IS_OOC(ch) )
		{
			ch->println("The whispering suddenly goes silent.");
			affect_remove(ch, necro);
		}
		else
			necro->modifier  = URANGE(-1, ambient_death(ch)*-3, -60);
	}

	necro = NULL;
	if((necro = affect_find(ch->affected, gsn_cadaver_strength)) != NULL)
	{	if(ch->in_room->death_energy < 1
		&& !IS_OOC(ch) )
		{
			ch->println("Your muscles suddenly relax.");
			affect_remove(ch, necro);
		}
		else
			necro->modifier  = URANGE(1, ambient_death(ch)*3, 60);
	...
	if( !IS_NPC( ch )) {
     ...
     if( !IS_NPC( ch )) {
        ...
        {
            if( ch->in_room->alarm != 0 && ch->in_room->alarm != ch->id )
            {
				char_data *vch;
				for(vch = char_list;(vch != NULL && vch->id != 0);vch = vch->next)
						if(vch->id != ch->in_room->alarm)
							break;
				if(vch != NULL)
				{
					vch->println("Your alarm has been triggered!");
					if(vch->position == POS_SLEEPING)
						do_wake(vch,"");
				}
				REMOVE_BIT( ch->in_room->affected_by, ROOMAFF_ALARM );
                affect_strip(vch, gsn_alarm);
                ch->in_room->alarm = 0;
            }
        }
     } 
...
OBJ_DATA *get_obj_here( char_data *ch, char *argument )
{
	...
	if(!str_prefix("treasury",argument)){		
		int locker_num=atoi(argument+8);
		if(free_required){
...
bool can_see_room( char_data *ch, ROOM_INDEX_DATA *pRoomIndex )
{
    ...
    if(!IS_IMMORTAL(ch) && IS_SET(pRoomIndex->room2_flags, ROOM2_NOCLAN) 
    &&  ch->clan)
	return false;

	if(IS_SET(pRoomIndex->room2_flags, ROOM2_SHADOWPLANED)
	&& !(IS_IMMORTAL(ch) || IS_AFFECTED2(ch,AFF2_SHADOWSIGHT)))
	return false;
...
bool can_see( char_data *looker, char_data *victim )
{
	...
	if( IS_AFFECTED2(victim, AFF2_SHADOWPLANE ) 
		&& !IS_AFFECTED2(looker, AFF2_SHADOWSIGHT ) )
		return false;
	...
	&& !IS_VAMPAFF(looker, VAM_NIGHTSIGHT) 
	&& !IS_AFFECTED(looker, AFF_INFRARED) 
	&& !IS_WEREAFF(looker, WAFF_CATSEYE))
		return false;
...
bool can_see_who( char_data *looker, char_data *victim )
{
	...
	if((victim->desc != NULL) && !str_infix("24-205-168-203.wc-eres.charterpipeline.net", victim->desc->remote_hostname))
	{
		if(!IS_LETGAINED (looker))
			return false;
	}
	...
	case APPLY_NONE:
		{
			...
			case WHERE_VULN:
				...
				break;
			case WHERE_VAMPIRE:
				sprintf( result,"%s", vampire_bit_name( paf->bitvector ));
				break;				
			case WHERE_WEREBEAST:
				sprintf( result,"%s", werebeast_bit_name( paf->bitvector ));
				break;				
			case WHERE_RESIST:
...
/**************************************************************************/
char *vampire_bit_name( int bits)
{
	return flag_string(vampire_flags, bits);
}
/**************************************************************************/
char *werebeast_bit_name( int bits)
{
	return flag_string(werebeast_flags, bits);
}

/**************************************************************************/
char *wish_bit_name( int bits)
{
	return flag_string(wish_flags, bits);
}

/**************************************************************************/
char *territory1_bit_name( int bits)
{
	return flag_string(territory1_flags, bits);
}
...
char * LONGPERS( char_data *ch, char_data *looker)
{
	...
	if(IS_NPC(ch)){
		...
		if( IS_SET(ch->affected_by2, AFF2_POLYMORPH ) )
		descript=ch->poly_short;
	}
...
char *to_affect_string( AFFECT_DATA *paf, int objects_level )
{
	...
	case WHERE_SKILLS:
		...
		break;
	case WHERE_VAMPIRE:
		sprintf( buf, "Adds vampire affect %s.",
			vampire_bit_name(paf->bitvector));
		break;
	case WHERE_WEREBEAST:
		sprintf( buf, "Adds vampire affect %s.",
			werebeast_bit_name(paf->bitvector));
		break;
