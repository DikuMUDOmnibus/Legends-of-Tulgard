...
SPRESULT spell_vicegrip(int sn, int level, char_data *ch, void *vo, int target)
{
	...
	if (IS_SET(ch->affected_by2, AFF2_VICEGRIP))
	{		
		if (victim == ch)
		{
...
SPRESULT spell_sb_strength_of_the_land( int sn, int level, char_data *ch, void *, int )
{
   ...
   switch ( ch->in_room->sector_type )
   {
      ...
      case SECT_LAVA:
         ch->printf( "Nature won't bless those who call in places not meant for them.\r\n" );
         return HALF_MANA;
         break;
      case SECT_AIR:
      ...
      case SECT_WATER_NOSWIM:
         if ( !is_affected(ch,sn) )
         {
            ...
            af.type         = sn;
            af.level      = level;
            af.duration      = level / 4 - duration_modifier;
            af.location      = APPLY_DAMROLL;
            af.modifier      = level / 5 + 2;
            af.bitvector   = 0;
            affect_to_char( ch, &af );

            ch->printf( "The force of waves gathers within you!\r\n" );
            act( "$n glows an intense blue-green for a moment.\r\n", ch, NULL, NULL, TO_ROOM );
         }
         else
            success = false;
         break;
      case SECT_UNDERWATER:
         if ( !is_affected(ch,sn) )
         {
            ...
            ch->printf( "You feel imbued with the stillness of the deep water around you!\r\n" );
            act( "$n glows deep blue for a moment.\r\n", ch, NULL, NULL, TO_ROOM );
         }
         else
         {
      ...
      case SECT_SWAMP:
         if ( !is_affected(ch,sn) )
         {
            ...
            ch->printf( "The bog whispers it's mysteries of decay and death to you!\r\n" );
            act( "$n glows smoky grey for a moment.\r\n", ch, NULL, NULL, TO_ROOM );
            if (check_dispel(200,ch,gsn_plague))
            {
               ch->println( "Your sores vanish." );
      ...
      case SECT_DESERT:
         ...
         af.where      = WHERE_AFFECTS;
            ...
            if (check_dispel(200,ch,gsn_blindness))
            {
               ch->println( "Your vision returns in a blaze of light!" );
               act("$n's eyes cast out a bit of light as $s vision returns.",ch,NULL,NULL,TO_ROOM);
            }
            ch->printf( "The sands shift underfoot, revealing the ways of the sun!\r\n" );
            act( "$n glows sandy brown for a moment.\r\n", ch, NULL, NULL, TO_ROOM );
      ...
      case SECT_HILLS:
         if ( !is_affected(ch,sn) )
         {
            ...
            af.where        = WHERE_IMMUNE;
            af.bitvector    = IMM_SCRY;
            affect_to_char( ch, &af );

            ch->printf( "The hills and uneven ground underfoot hide you from prying eyes!\r\n" );
            act( "$n glows light green for a moment.\r\n", ch, NULL, NULL, TO_ROOM );
         }
         else
            success = false;
         break;   
      case SECT_ICE:
            if ( !is_affected(ch,sn) )
         {
            ...
            af.location      = APPLY_NONE;
            af.modifier      = 0;
            af.bitvector   = IMM_HUNGER;
            affect_to_char( ch, &af );
            
            af.bitvector   = IMM_THIRST;
            affect_to_char( ch, &af );
            ch->printf( "The artic wind cuts like knives through you, stripping away weakness!\r\n" );
            act( "$n glows crisp white for a moment.\r\n", ch, NULL, NULL, TO_ROOM );
         }
         else
            success = false;
         break;
      ...
      case SECT_CAVE:
         if ( !is_affected(ch,sn) )
         {
            ...
            ch->printf( "The rock around you lends it's memories of enduring!\r\n" );
            act( "$n glows slate grey for a moment.\r\n", ch, NULL, NULL, TO_ROOM );
         }
         else
            success = false;
         break;
      case SECT_FOREST:
         if ( !is_affected(ch,sn) )
         {
            ...
            ch->printf( "The power of the forest around you enriches you!\r\n" );
            act( "$n glows deep green for a moment.\r\n", ch, NULL, NULL, TO_ROOM );
         }
         else
            success = false;
         break;
      ...
      case SECT_FIELD:
         if ( !is_affected(ch,sn) )
         {
            ...
            ch->printf( "Old tracks and trails awaken a forgotten hunting instinct!\r\n" );
            act( "$n glows dark red for a moment.\r\n", ch, NULL, NULL, TO_ROOM );
         }
         else
            success = false;
         break;
...
SPRESULT spell_decay_magic(int sn,int level,char_data *ch,void *vo,int target)
{
   AFFECT_DATA *af, *af_next;
   char_data *victim = (char_data *) vo;

   if(ch == victim)
      ch->println("Your magic seeks out spells to decay.");   
   else if(victim->affected)
   {
      act( "Your magic warps the air as it leaps to $N!", ch, NULL, victim, TO_CHAR );
      if(IS_AFFECTED(victim, AFF_DETECT_MAGIC))
         act( "$n's warping magic flows towards you with liquid menace!", ch, NULL, victim, TO_VICT );
      else
         act( "Distortions form as $n raises $s hands!", ch, NULL, victim, TO_VICT );
   }

    for ( af = victim->affected; af; af = af_next )
    {
      af_next=af->next;
      if( !(af->type == gsn_berserk
       || af->type == gsn_vanish
      ...
      {
         if(af->duration != -1)
               af->duration =UMAX(0,af->duration - number_range(1, level / 20));
         af->level    -= number_range(1, 1 + level / 20);
         if(af->level < 1 || af->duration == 0)
            affect_remove(ch, af);
      }
    }
    return FULL_MANA;
...
SPRESULT spell_enchantment_roots(int sn, int level, char_data *ch, void *vo, int target) // BLACKOUT - EOTR
{
   char_data *victim = (char_data *) vo;
   AFFECT_DATA af;
   ...
   if (IS_AFFECTED2(victim, AFF2_NO_DISPEL)){      
      if (victim == ch){
         ch->println("You've already cast the enchantment of roots.");
      }
      return HALF_MANA;
   }
   ...
   af.location   = APPLY_NONE;
   af.modifier   = 0;
   af.bitvector= AFF2_NO_DISPEL;
   
   affect_to_char(victim, &af);
   act("$n weaves a complicated pattern in the air.",victim, NULL,NULL, TO_ROOM);
   act("You painstakingly trace the enchantment of roots about yourself!",victim, NULL,NULL, TO_CHAR);
   return FULL_MANA;
}
/***************************************************************************/
