...
/*	ch->prompt	= oc->prompt; */
 ...
 /*  if(!oc->orig_short_descr)
   {
		ch->orig_short_descr = str_dup(oc->short_descr);
		ch->orig_description = str_dup(oc->description);
   }
	else 
   {
		ch->orig_short_descr = str_dup(oc->orig_short_descr);
		ch->orig_description = str_dup(oc->orig_description);
		ch->description = str_dup(oc->description);
   } */
	...
	if(IS_SET(oc->affected_by2, AFF2_NO_VAMP))
...
void do_remort(char_data *ch, char *argument)
{
	...
	char_data*v=get_char_world(ch, name);
	...
	begin_remort(v);
    REMOVE_BIT(v->act, PLR_CAN_ADVANCE);
	ch->println(text);
}
/**************************************************************************/
void do_reremort(char_data *ch, char *argument)
{
	char name[MIL];
	...
	argument=one_argument(argument, name);
	if(IS_NULLSTR(name)){
		ch->println("syntax: reremort <playername>");
		return;
	}
	...
	if(v==NULL){
		ch->printlnf("Couldn't find player '%s' in current room to reremort.", name);
		return;
	}

	// found the player, if pass idiot checks remort them
	if(IS_NPC(v)){
		ch->println("Can only reremort players.");
		return;
	}
	...
	char subject[MIL];
	sprintf(subject,"%s beginning reremort %d", v->name, v->beginning_remort);
	char text[MIL];
	sprintf(text,"%s beginning reremort %d`1Pfile of player backed up to %s before remort begun.`1"
		"Sent on remort by '%s' %d.", 
		v->name, v->beginning_remort, 
		backname,
		ch->name, ch->pIndexData?ch->pIndexData->vnum:0);

	autonote(NOTE_INOTE, "reremort system", subject, "imm", text, true);
