...
#define CHK_ISCLOAKED (82)
#define CHK_HASMURDER (83)
#define CHK_HASTHEFT (84)
...
const	struct	fn_keyword_type		fn_keyword[] =
{
	...
	{"tired",	"if tired $n > 10"},	
	{"fealty",  	"if fealty $n bob - if in fealty to bob"},
	{"act2",	"act2 flags"},
	...
	{"isconcealed", "if isconcealed $n - has weapon concealed"}, // Ix
 	{"iscloaked", "if iscloaked $n - has cloak drawn"},
 	{"hasmurder", "if hastrains $n > 1 - used for murder in justice system"},
 	{"hastheft", "if hastrains $n > 1 - used for thefts in justice system"},
...
//				vnum, numval, numpercent,	numval>numpercent?"true":"false");	
	...
	case CHK_ISIMMORT:
 	...
 	case CHK_ISCLOAKED:
 		return( lval_char != NULL && !IS_NPC(lval_char) && HAS_CONFIG2(lval_char, CONFIG2_CLOAKED) );		
	...
	case CHK_FEALTY:
		return( lval_char != NULL && is_name( buf, lval_char->fealty ) );
	...
	const void *arg1, const void *arg2, int type )
{
    ...
    {
    	if ( prg->trig_type == type 
	&&   event_active == atoi( prg->trig_phrase ) )
        {
	    program_flow( prg, mob, ch, arg1, arg2 );
	    return ( true );
	}
