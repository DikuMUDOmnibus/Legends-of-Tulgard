...
bool aedit_show(char_data *ch, char *)
{
    ...
    ch->printlnf( " `=rColor code:  `x%s", pArea->colour);
    ...
    ch->printlnf( " `=rWorld:   [`x%s`=r]", 
                pArea->world?pArea->world->name:"none" );
...
bool aedit_colour(char_data *ch, char *argument)
{
    ...
    if ( IS_NULLSTR(argument) || str_len(argument)>1)
    {
		...
		{
            ch->println("The color code should be only 1 character!\r\n");
		}
        ch->println("Syntax:   color <code>");
        ch->println("This is the color of the area on the area list.  The color code should");
        ch->println("be only the single code character (it doesn't include the color prefix.");
		return false;
    }
    ...
    ch->printlnf( "Color code set to %s.", pArea->colour);
    return true;
}
...
/**************************************************************************/
bool aedit_world( char_data *ch, char *argument )
{
        AREA_DATA *pArea;
        world_type *world;
        ...
        if ( !HAS_SECURITY( ch, 7 ))
        {
                ch->println( "You must have a security of 7 to set the world name." );
                return false;
        }
    ...
    EDIT_AREA(ch, pArea);
        ...
        if( IS_NULLSTR( argument )){
                ch->println( "Syntax: world none");
                ch->println( "Syntax: world <name>");           
                world_show(ch);
                return false;
        }

        if( !str_cmp(argument, "none") ){
                ch->printlnf( "World set to none (was in '%s').",                       
                        pArea->world?pArea->world->name:"none");
                pArea->world = NULL;
                return true;
        }
        ...
        // look up world
        world = world_lookup( argument );
        if(!world ){            
                ch->printlnf( "Invalid World '%s'.", argument );
                world_show( ch );
                return false;
        }

        ch->printlnf( "World now resides in %s (was in '%s').", 
                world->name,
                pArea->world?pArea->world->name:"any");
        pArea->world = world;
        return true;
