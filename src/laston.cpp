...
laston_data    *laston_crime_list; // last on Most Wanted list system
laston_data    *laston_arena_list;
...
void resort_top_crime();
void resort_top_arena();
...
void laston_update_node(laston_data *node, char_data *ch)
{
    ...
    node->crime = (ch->pcdata->theft + ch->pcdata->murder) * 50;
    node->remort = ch->remort;
    node->arenakills = ch->pcdata->akills;
    node->arenadeaths = ch->pcdata->adeaths;
    if(node->arenadeaths>=node->arenakills)
	node->arenascore=0;
    else node->arenascore = node->arenakills - node->arenadeaths;
    node->xp = ch->exp;
...
/**************************************************************************/
void add_top_crime(laston_data *pnode)
{
	laston_data *node, *prev;

	if (!laston_crime_list)
	{
		laston_crime_list=pnode;
		return;		
	}

	// go down the list till we find the place they are less than
	prev=NULL;
    for (node= laston_crime_list; node; node=node->next_crime)
    {
		if (node->crime < pnode->crime){
			if (!prev)
			{
				pnode->next_crime=laston_crime_list;
				laston_crime_list=pnode;
				return;
			}

			pnode->next_crime = node;
			prev->next_crime  =pnode;
			return;
		}
		prev=node;
	}
...
void resort_top_crime()
{
	...
	log_string("resorting most wanted list...");
	// first wipe the old settings
	laston_crime_list=NULL;
    for (node= laston_list; node; node=node->next)
    {
		node->next_crime=NULL;
	}

	// now readd to the list if their rps is over 10000
    for (node= laston_list; node; node=node->next)
    {
		if (node->crime>=250  // nonimms on in the last 8 weeks
			&& (node->level[node->index]<LEVEL_IMMORTAL)
			&& node->off[node->index]> current_time-(60*60*24*7*8))
			add_top_crime(node);	
	}
...
/**************************************************************************/
void do_topcrime(char_data *ch, char *)
{
	laston_data *node;
    	BUFFER *output;
    	char buf[MSL];
	...
	sprintf( buf,"`?%s`x\r\n", makef_titlebar("MOST WANTED CRIMINALS - Bounties Listed In Gold"));
    add_buf(output,buf);
    ...
    for (node= laston_crime_list; node; node=node->next_crime)
    {
   	if(node->crime != 0) 
	{        
	 	sprintf(buf, "`W%3d `w%-13s%6d    ", count, node->name, node->crime);
		add_buf(output,buf);
...
/**************************************************************************/
void add_top_arena(laston_data *pnode)
{
	laston_data *node, *prev;

	if (!laston_arena_list)
	{
		laston_arena_list=pnode;
		return;		
	}

	// go down the list till we find the place they are less than
	prev=NULL;
    for (node= laston_arena_list; node; node=node->next_arena)
    {
		if (node->arenascore < pnode->arenascore){
			if (!prev)
			{
				pnode->next_arena=laston_arena_list;
				laston_arena_list=pnode;
				return;
			}

			pnode->next_arena = node;
			prev->next_arena  =pnode;
			return;
		}
		prev=node;
	}
...
void resort_top_arena()
{
	...
	log_string("resorting arena list...");
	// first wipe the old settings
	laston_arena_list=NULL;
    for (node= laston_list; node; node=node->next)
    {
		node->next_arena=NULL;
	}

    for (node= laston_list; node; node=node->next)
    {
		if (node->arenascore>=0  // nonimms on in the last 8 weeks
			&& (node->level[node->index]<LEVEL_IMMORTAL)
			&& node->off[node->index]> current_time-(60*60*24*7*8))
			add_top_arena(node);	
	}
...
/**************************************************************************/
void do_toparena(char_data *ch, char *)
{
	laston_data *node;
    	BUFFER *output;
    	char buf[MSL];
	...
	sprintf( buf,"`?%s`x\r\n", makef_titlebar("Top Arena Fighters - Wins/Defeats"));
    add_buf(output,buf);

    for (node= laston_arena_list; node; node=node->next_arena)
    {
   	if(node->arenascore != 0) 
	{        
	 	sprintf(buf, "`W%3d `w%-13s%6d/%d    ", count, node->name, node->arenakills, node->arenadeaths);
		add_buf(output,buf);
...
/**************************************************************************/
void do_pwipe(char_data *ch, char *)
{
    laston_data *node;
    laston_data *node_next;
    char buf[MSL];
	...
	if(node->remort>0)
        ...
        if (node->level[ni]== 1 && ((node->off[ni]+(60*60* 24 *2)) < current_time ))
        {
            count++;
            ch->printlnf("%3d) LASTON - Deleting old player file %s ", 
				count, node->name);
			...
			if(pt==PFILE_NONE){
				ch->printlnf("ERROR: a pfile for %s wasnt found!", node->name);
				logf("do_pwipe(): ERROR - a pfile for %s wasnt found!", node->name);
				laston_id_delete(node->id);
				continue;
			}
			...
			if(pt==PFILE_MULTIPLE){
				ch->printlnf("ERROR: Multiple pfiles for %s were found!", node->name);
				logf("do_pwipe(): ERROR - Multiple pfiles for %s were found!", node->name);
				continue;
			}
			sprintf( strsave, pfilename(node->name, pt));

			logf("do_pwipe():[%3d] removing level 1 pfile %s from %s", 
				count, node->name, strsave);
        ...
        if (node->level[ni]<=4 && ((node->off[ni]+(60*60* 24 *7)) < current_time ))
        {
            count++;
            ch->printlnf("%3d) LASTON - Deleting old player file %s ", 
				count, node->name);
			...
			logf("do_pwipe():[%3d] removing level 2-4 pfile %s from %s", 
				count, node->name, strsave);
       ...
       if (node->level[ni]<=15 && ((node->off[ni]+(60*60* 24 * 30)) < current_time ))
        {
            count++;
            ch->printlnf("%3d) LASTON - Deleting old player file %s ", 
				count, node->name);
			...
			logf("do_pwipe():[%3d] removing level 5-15 pfile %s from %s", 
				count, node->name, strsave);
       ...
       if (node->level[ni]<=34 && ((node->off[ni]+(60*60* 24 * 60)) < current_time ))
        {
            count++;
            ch->printlnf("%3d) LASTON - Deleting old player file %s ", 
				count, node->name);
			...
			logf("do_pwipe():[%3d] removing level 16-34 pfile %s from %s", 
				count, node->name, strsave);
       ...
       if (node->level[ni]<=64 && ((node->off[ni]+(60*60* 24 * 90)) < current_time ))
        {
            count++;
            ch->printlnf("%3d) LASTON - Deleting old player file %s ", 
				count, node->name);
			...
			logf("do_pwipe():[%3d] removing level 35-64 pfile %s from %s", 
				count, node->name, strsave);
       ...
       if (node->level[ni]<=91 && ((node->off[ni]+(60*60* 24 * 120)) < current_time ))
        {
            count++;
            ch->printlnf("%3d) LASTON - Deleting old player file %s ", 
				count, node->name);
			...
			logf("do_pwipe():[%3d] removing level 65-91 pfile %s from %s", 
				count, node->name, strsave);
...
bool laston_find(char *argument )
{
    ...
    for (node= laston_list; node; node=node->next)
    {
        if (!str_prefix(argument, node->name))
        {
                found = true;
        }
