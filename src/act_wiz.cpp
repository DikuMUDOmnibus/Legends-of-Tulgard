...
void do_short( char_data *ch, char *argument)
{
    ...
    else
    {
        ...
        if (str_len(strip_colour(arg))>55){
...
void wiznet_index( int index, char_data *ch, char *argument )
{
    ...
    if ( IS_NULLSTR(argument) ) // display status 
    {
		...
		if(IS_NULLSTR(TRUE_CH(ch)->wiznet_colour[index])){
			sprintf(buf,       "        %s color prefix is unset.\r\n", wiznet_name);
		}else{
			sprintf(buf,       "        %s color prefix is set to '%s'\r\n", wiznet_name, 
...
void wiznet_colourindex( int index, char_data *ch, char *argument )
{
	...
	char buf[MSL];
	sprintf(buf,"%s color prefix set to '%s'\r\n", wiznet_name, argument);
	ch->printbw(buf);
	ch->println("Note, you dont have to put only color codes in the prefix.");	
}
/**************************************************************************/
...
void do_wiznetdefault( char_data *ch, char *argument )
{
	...
	ch->println("(this includes wiznet colors prefixes).");
}
...
void do_rstat( char_data *ch, char *argument )
{
	...
	ch->printlnf("Death energy: %d", location->death_energy);
...
void do_charinfo( char_data *ch, char *argument )
{
	...
	if(victim->colour_prefix!=COLOURCODE){
		ch->printlnfbw(" Color code prefix: %c", victim->colour_prefix);
	}
	...
	if(!GAMESETTING2(GAMESET2_NO_DUEL_REQUIRED)){
 	...
 	// JUSTICE SYSTEM
 	ch->titlebar("JUSTICE SYSTEM");
 	ch->printlnf(" Murder: %d           Theft: %d \r\n"
 		     " Jail Release Date: %s (%.24s)\r\n"
 		     " Criminal Score: %d",
 	victim->pcdata->murder,	victim->pcdata->theft,  
 		get_shorticdate_from_time(victim->pcdata->jail, "", 0),
 		ctime(&victim->pcdata->jail),
 		victim->pcdata->crime);
 	// Ignore List
 	ch->titlebar("IGNORE LIST");
	int pos=0;
	for (pos = 0; pos < MAX_IGNORE; pos++)
	{
		if (!IS_NULLSTR(victim->pcdata->ignore[pos]))
		{
      			ch->printlnf("%d. %s",pos+1, victim->pcdata->ignore[pos]);
		}
    	}
...
void do_mstat( char_data *ch, char *argument )
{
	...
	ch->printlnf("`#| Last Physical Damage: %s%-18s `^Last Magical Damage: %s%-14s`^|",
		(victim->last_damage[0] >= 1000) ? "`R" : "", (victim->last_damage[0] >= 1000) ? damtype_types[victim->last_damage[0]-1000].name : damtype_types[victim->last_damage[0]].name,
		(victim->last_damage[1] >= 1000) ? "`R" : "", (victim->last_damage[1] >= 1000) ? damtype_types[victim->last_damage[1]-1000].name : damtype_types[victim->last_damage[1]].name);
    ...
    if (!IS_NPC(victim) && victim->vampire && (IS_VAMPIRE(ch) || IS_VAMPAFF(ch, VAM_HUMAN)))
		ch->printlnf("| Vampire: %-66s |", vampire_bit_name(victim->vampire));
    if (!IS_NPC(victim) && victim->vampire && (IS_WEREBEAST(ch)))
		ch->printlnf("| Werebeast: %-64s |", werebeast_bit_name(victim->vampire));
    if (!IS_NPC(victim) && victim->wish)
		ch->printlnf("| Wish: %-69s |", wish_bit_name(victim->wish));
    ...
    for ( paf = victim->affected; paf != NULL; paf = paf->next )
    {
        ...
        case WHERE_AFFECTS2:
            sprintf( buf,
                "`WSp:`x '%s' mods %s by %d for %d tick%s (aff2bits %s), lvl%d.",
                skill_table[(int) paf->type].name,
                affect_loc_name( paf->location ),
                paf->modifier,
                paf->duration,
        ...
        case WHERE_AFFECTS3:
            sprintf( buf,
                "`WSp:`x '%s' mods %s by %d for %d tick%s (aff3bits %s), lvl%d.",
                skill_table[(int) paf->type].name,
                affect_loc_name( paf->location ),
                paf->modifier,
                paf->duration,
...
/**************************************************************************/
void do_oswhere(char_data *ch, char *argument )
{
    char buf[MIL];
    BUFFER *buffer;
    OBJ_DATA *obj;
    ...
    for ( obj = object_list; obj; obj = obj->next )
    {
		...
		if(ospec_lookup( argument ) == NULL
		|| ospec_lookup( argument ) != obj->ospec_fun)
			continue;
...
void restore_victim( char_data *ch, char_data *victim)
{
	...
	affect_parentspellfunc_strip( victim, gsn_despair	);
	affect_parentspellfunc_strip( victim, gsn_hallucinate	);
...
void do_log( char_data *ch, char *argument )
{
    ...
    {
		ch->println( "Immortal logs cannot be removed." );
		SET_BIT(ch->act, PLR_LOG);
    		save_char_obj( victim );
		sprintf(buf,"%s attempted to remove the Immortal log of %s.", ch->name, victim->name);
		autonote(NOTE_INOTE, "Immortal Monitor","Log Removal Alert", "Ixliam", buf, true);
		return;
    }
    ...
    if (ch==victim)
    {
		...
		SET_BIT(ch->act, PLR_LOG);
    		save_char_obj( ch );
		sprintf(buf,"%s attempted to remove their Immortal log.", ch->name);
		autonote(NOTE_INOTE, "Immortal Monitor","Log Removal Alert", "Ixliam", buf, true);
		return;
    }    
...
/**************************************************************************/
void do_impslay( char_data *ch, char *argument )
{
    char_data *victim;
    char arg[MIL];
    ...
    act("`#$n points a finger at $N and utters the words, `R'Soul Drain'`^.",ch,NULL,victim,TO_NOTVICT);
    act("`#$n points a finger at you and utters the words, `R'Soul Drain'`^.",ch,NULL,victim,TO_VICT);
    ch->printlnf("You point a finger at %s and utter the words, `R'Soul Drain'`^.", victim->name);
    ...
    act("`#`YYour wrath vaporises $N's heart, killing $M instantly!`^",ch,NULL,victim,TO_CHAR);
...
void do_allowimmtalk( char_data *ch, char *argument )
{
    ...
    if(!IS_NULLSTR(victim->pcdata->immtalk_name))
    {
		...
		if(count_char(immname, COLOURCODE)){
			ch->println("You can't have color code characters in immtalk names.");
			return;
		}
...
/**************************************************************************/
void superboon_victim( char_data *ch, char_data *victim)
{
	if(IS_NPC(victim)){ // dont boon mobs at this stage :)
		return;
	}
	...
	affect_parentspellfunc_strip( victim, gsn_second_wind);	
	spell_bless(gsn_bless,	600, ch, victim, TARGET_CHAR);
	spell_armor(gsn_armor,	600, ch, victim, TARGET_CHAR);
	spell_giant_strength(gsn_giant_strength, 600, ch, victim, TARGET_CHAR);	
	spell_illusions_grandeur(gsn_illusions_grandeur, 600, ch, victim, TARGET_CHAR);
	spell_shield(gsn_shield, 600, ch, victim, TARGET_CHAR);
	spell_sanctuary(gsn_sanctuary,	600, ch, victim, TARGET_CHAR);
	spell_haste(gsn_haste, 600, ch, victim, TARGET_CHAR);
	spell_second_wind(gsn_second_wind, 600, ch, victim, TARGET_CHAR);
	...
	if (victim->in_room != NULL)
	{
		if (!IS_SILENT(ch) || IS_IMMORTAL(victim))
		{
			if (IS_NPC(ch))
			{
				act( "$n has given you a `#`YS`yU`YP`yE`YR`yB`YO`yO`YN`^ from the gods!", ch, NULL, victim, TO_VICT);
			}else{
				victim->printlnf("%s has given you a `#`YS`yU`YP`yE`YR`yB`YO`yO`YN`^ from the gods!", TRUE_CH(ch)->name);
				}
				msp_to_room(MSPT_ACTION, MSP_SOUND_RESTORE, 0, victim, true, false );
		}
	}	
...
/**************************************************************************/
// Kal - Jan 99
void do_superboon( char_data *ch, char *argument )
{
    char arg[MIL], buf[MSL];
	char_data *victim;
	char_data *vch;
	...
	if (IS_NULLSTR(arg) || !str_cmp(arg,"room"))
	{
		...
		for (vch = ch->in_room->people; vch != NULL; vch = vch->next_in_room)
		{
			superboon_victim( ch, vch);
        }
        sprintf(buf,"$N `#`YS`yU`YP`yE`YR`yB`YO`yO`YN`y'D`^ room %d.",ch->in_room->vnum);
        wiznet(buf,ch,NULL,WIZ_QUESTING,WIZ_SECURE,get_trust(ch));      
		ch->println( "Room `YS`yU`YP`yE`YR`yB`YO`yO`YN'`yD`x" );
        return;
    }
	...
	if ( get_trust(ch) >=  MAX_LEVEL - 3 && !str_cmp(arg,"all"))
	{
		...
		for(victim=player_list; victim; victim=victim->next_player)
		{
			...
			superboon_victim( ch, victim);
		}
		ch->println( "All active players `YS`yU`YP`yE`YR`yB`YO`yO`YN'`yD`x." );
		return;
	}
