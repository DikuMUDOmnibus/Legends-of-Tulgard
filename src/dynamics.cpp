    ...
    {	// couldn't find the file to read in
		...
		if(file_exists(CLASSES_LIST )){
			...
			if(runlevel==RUNLEVEL_BOOTING){
				...
				"`1"
				"If this is the first time the mud has been run, you can tell the mud "
				"to create a clean class file with a single class called 'classless' in it, then "
				"either manually add classes to this file or add classes within the game "
				"using the class olc interface. (to start up in this mode type '%s 5')`1"
				"`1"
				"If this isn't the first the mud has been run, the information of the "
				"previous class table might be contained in " CLASSES_LIST ".bak or "
				...
				CLASSES_LIST" then restart the mud.", EXE_FILE);
				write_shutdown_file(NULL);
