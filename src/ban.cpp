...
#ifdef unix
	char unlockname[MIL];
	sprintf(unlockname, "/home/wotl/emailunlockfile_%d_%d.tmp", getpid(), (int)current_time);
	char buf[MSL];
	unlink(unlockname);
	sprintf(buf,
"Hello %s,\n"
...
"\n", CH(d)->pcdata->email, MUD_NAME,
	...
	sprintf(buf,"mail %s < %s ; sleep 10; rm %s &", 
		CH(d)->pcdata->email, unlockname, unlockname);
	log_string(buf);
	system(buf);
#else
