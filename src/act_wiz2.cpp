...
DECLARE_DO_FUN(do_lastonremove  ); // Ixliam
...
char *const eq_table[]=
{ 
"light", "finger_l", "finger_r","neck_1","neck_2","torso","head","legs","feet","hands", 
"arms","shield","about","waist","wrist_l","wrist_r","wield","hold","float","second",
"lodged_arm","lodged_leg","lodged_rib","sheathed","concealed","eyes","ear_l","ear_r",
"face","ankle_l","ankle_r","back",NULL
};
...
void do_mset( char_data *ch, char *argument )
{
	...
	if ( arg1[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0' )
	{
		...
		ch->println( "    autoafkafter murder theft alttimer");
    ...
    if ( !str_prefix( arg2, "theft" ) )
     {
         if ( value < 0 )
         {
             ch->println( "*bonk self* - They can't have negative theft!" );
             return;
         }
         if ( value > 1000 )
         {
 		...
 		ch->printf("Theft change from %d to %d on %s(%s)\r\n",
 			victim->pcdata->theft, value, victim->short_descr, victim->name);
         victim->pcdata->theft = value;
         return;
     }
     ...
     if ( !str_prefix( arg2, "murder" ) )
     {
         if ( value < 0 )
         {
             ch->println( "*bonk self* - They can't have negative murder!" );
             return;
         }
         if ( value > 1000 )
         {
 		...
 		ch->printf("Murder change from %d to %d on %s(%s)\r\n",
 			victim->pcdata->murder, value, victim->short_descr, victim->name);
	...
	if ( !str_prefix( arg2, "alttimer" ) )
    {
        ...
        if ( value > 50 )
        {
            ch->println( "That is getting a bit high dont you think? (try 5)" );
            return;
        }
		ch->printlnf("Alttimer changed from %d to %d on %s(%s)",
			victim->altform.timer, value, victim->short_descr, victim->name);
        victim->altform.timer = value;
        return;
    }
...
void do_force( char_data *ch, char *argument )
{
    ...
    if ( !str_cmp( arg, "all" ) )
	{
		...
		for ( vch = player_list; vch; vch = vch_next_player )
		{
			...
			if (vch->desc != NULL && !IS_NPC(vch) && get_trust( vch ) < get_trust( ch ) )
			{
    ...
    else
    {
		...
		if (ch->desc != NULL && get_trust( victim ) >= get_trust( ch ) )
		{
...
/**************************************************************************/
void do_holycast( char_data *ch, char *)
{
    if ( IS_UNSWITCHED_MOB(ch) )
	return;

    if ( IS_SET(TRUE_CH(ch)->config2, CONFIG2_HOLYCAST) )
    {
        REMOVE_BIT(TRUE_CH(ch)->config2, CONFIG2_HOLYCAST);
        ch->println( "Holycasting mode off." );
    }
    else
    {
        SET_BIT(TRUE_CH(ch)->config2, CONFIG2_HOLYCAST);
        ch->println( "Holycasting mode on." );
    }
    return;
}
...
void do_pinfo( char_data *ch, char *argument )
{
    ...
    for ( wch = player_list; wch; wch = wch->next_player )
    {
		...
		// get generic short description color
		{
			char tempbuf[MIL];
...
void do_holylist(char_data *ch, char *)
{
	...
	ch->printlnf( "holycast       %s",
		IS_SET( TRUE_CH(ch)->config2, CONFIG2_HOLYCAST) ? "ON" : "OFF" );
...
void do_rawcolour( char_data *ch, char *argument )
{
    if ( IS_NULLSTR( argument ))
    {
        ch->println( "Rawcolor what?" );
        return;
    }			
	
	if(!ch->desc){
        ch->println( "You must have a valid connection to use raw color." );
        return;
	}

    ch->println("RAWCOLOUR BEGIN.");
	// condensing system has to be flushed for raw color to work correctly
...
{  
	if ( str_cmp("confirm", argument)) {
		ch->printf("Turns on raw color permantantly (till use the raw command to turn it off):\r\n");
...
/**************************************************************************/
void do_wpeace(char_data *ch, char * )
{
     char_data *rch;
	...
	if ( rch->fighting )
	  {
	   rch->printlnf( "`#`Y%s has declared World Peace.`^", ch->name );
	   stop_fighting( rch, true );
          }
      }

    ch->printlnf( "`#`YYou have declared World Peace.`^");
    return;
}
/**************************************************************************/
...
{	
	...
	if( IS_NULLSTR(argument)){
        ch->println("What would you like to think about?");
        return;
	}
	
	ch->printlnf( "You think to yourself: `s%s`x", argument );

	for ( rch = ch->in_room->people; rch; rch = rch->next_in_room )
	{
		if(rch != ch && IS_SET(race_table[rch->race]->flags, RACEFLAG_TELEPATHY))
			rch->printlnf( "You sense %s thinking: `s%s`x",PERS(ch, rch), argument );

	}

	if (!IS_SET(ch->comm, COMM_NOPRAY) && !IS_SET(ch->config2, CONFIG2_SHUTUP))
	{
        sprintf(buf,"`w%s thinks `W'`s%s`w'`x\n", 
			ch->name, argument);
		wiznet(buf,NULL,NULL,WIZ_PRAYERS_DREAMS,0,AVATAR); 
	}
	//Thought sharer ospec
	obj_data *obj;
	obj_data *obj_next;
	char_data *vch;
	char_data *vch_next;
	obj_data *pObj;
	obj_data *pObj_next;
	bool sharer_found = false;
	for ( obj = ch->carrying ; obj != NULL; obj = obj_next)
	{
		obj_next = obj->next_content;
		if( obj->wear_loc != WEAR_NONE
		&&	obj->ospec_fun == ospec_lookup( "ospec_thought_sharer" )
		&&	obj->ospec_value[0] != 0)
			for ( vch = char_list; (vch != NULL && sharer_found==false); vch = vch_next )
			{	
				vch_next = vch->next;
				if(vch!= NULL
				&& vch->id == obj->ospec_value[0])
				{
					for ( pObj = vch->carrying ; pObj != NULL; pObj = pObj_next)
					{ 

						pObj_next = pObj->next_content;
						if( pObj != NULL
						&&	pObj->wear_loc != WEAR_NONE
						&&	pObj->ospec_fun == ospec_lookup( "ospec_thought_sharer" )
						&&  pObj->ospec_value[0] == ch->id
						&& ch != vch)
						{
							sprintf(buf,"`w%s's thoughts were shared with %s`x\n\r",ch->name, vch->name);
							wiznet(buf,NULL,NULL,WIZ_PRAYERS_DREAMS,0,AVATAR);
							vch->printlnf("Thoughts enter your mind: `s%s`x", argument);
							break;
						}
					}
				}
 ...
 // wiznet RPMONITOR
    ...
    if (!IS_NPC(ch))
	{
		if (!IS_OOC(ch)) 
		{
			...
			{
				// Wiznet spitout here
				sprintf (buf2, "Wiznet rpmonitor: %s thinks '%s`x'", ch->name, argument);
...
void do_wizzap( char_data *ch, char *argument )
{
   ...
   if (argument[0] == '\0')
   {
      ch->printlnf( "Syntax: wizzap <char>.");
      return;
   }
   ...
   if (victim == ch)
   {
      ch->printlnf( "If you are trying to revive brain cells, this is not the way.");
      return;
   }
   if (victim->fighting)
   {
      ch->printlnf( "Might as well slay them or wait till they are done fighting.");
      return;
   }
   if ( get_trust( victim ) >= get_trust( ch ) )
   {
   ...
   victim->mana = 1;
   victim->move = 1;
   act("$n has zapped you with a big bolt of lightning!!!", ch, NULL, victim, TO_VICT);
   act("You zap $N with a big bolt of lightning!!!", ch, NULL, victim, TO_CHAR);
   act("$n zaps $N with a huge bolt of lightning, be glad it was not you.", ch, NULL, victim, TO_ROOM);
   return;
}

/**************************************************************************/
//Causes the victim to fail all skill rolls except languages 
//and be unable to raise any skill.
void do_dunce( char_data *ch, char *argument )
{
   char_data *victim;
   
   if (argument[0] == '\0')
   {
      ch->printlnf( "Syntax: dunce <char>.");
      return;
   }
   if ( ( victim = get_char_world( ch, argument ) ) == NULL )
   {
   ...
   if (victim == ch)
   {
		...
		ch->println("`W		   /     \\");
		ch->println("`W	          /   D   \\");
		ch->println("		 /_________\\");
		ch->println("`y	       //''\\\\'''//''\\\\");
		ch->println("`y	       :/`B  o     o`y  \\;");
		ch->println("`Y	      ((      /      ))");
		ch->println("`Y	       : `r*`Y   (__   `r*`Y ;");
		ch->println("`Y		\\   >---<   /");
		ch->println("`Y		 '-._____.-'");
		ch->println("`c		    /'""""""'\\  \\");
		ch->println("`c		   /    \\  \\");
		ch->println("`c		  /|     \\ _\\"); 
		ch->println("`c		 (_|`RLOSER`c \\__)");
		ch->println("`c		   |_______|");
		ch->println("		  `G__`b)_ |_ (`G__");
		ch->println("`G		 (_____|_____)`x");
      ch->printlnf( "Masochistic much?");
      return;
   }
   if (victim->fighting)
   {
      ch->printlnf( "You're one dirty dirty imm.");
      return;
   }
   if ( get_trust( victim ) >= get_trust( ch ) )
   {
      ch->printlnf( "We call that 'suicide'.");
      return;
   }
   if(IS_SET(victim->config2, CONFIG2_DUNCE))
   {
	   	REMOVE_BIT(victim->config2, CONFIG2_DUNCE);
		act("$n has un-dunced you. Be thankful.", ch, NULL, victim, TO_VICT);
		act("You removed $N's dunce hat.", ch, NULL, victim, TO_CHAR);
		act("$N is no longer a dunce. Blame $n.", ch, NULL, victim, TO_ROOM);
		return;
   }
	if (victim->position == POS_SLEEPING || victim->position == POS_RESTING)
      victim->position = POS_STANDING;
		...
		victim->println("`W		   /     \\");
		victim->println("`W	          /   D   \\");
		victim->println("		 /_________\\");
		victim->println("`y	       //''\\\\'''//''\\\\");
		victim->println("`y	       :/`B  o     o`y  \\;");
		victim->println("`Y	      ((      /      ))");
		victim->println("`Y	       : `r*`Y   (__   `r*`Y ;");
		victim->println("`Y		\\   >---<   /");
		victim->println("`Y		 '-._____.-'");
		victim->println("`c		    /'""""""'\\  \\");
		victim->println("`c		   /    \\  \\");
		victim->println("`c		  /|     \\ _\\"); 
		victim->println("`c		 (_|`RLOSER`c \\__)");
		victim->println("`c		   |_______|");
		victim->println("		  `G__`b)_ |_ (`G__");
		victim->println("`G		 (_____|_____)`x");
		SET_BIT(victim->config2, CONFIG2_DUNCE);
		act("$n has officially named you a dunce!", ch, NULL, victim, TO_VICT);
		act("You named $N for the dunce they really are.", ch, NULL, victim, TO_CHAR);
		act("$N has been named a dunce by $n. Begin teasing... now.", ch, NULL, victim, TO_NOTVICT);
   return;
}
/**************************************************************************/
...
void do_smite( char_data *ch, char *argument )
{
    ...
    {
	ch->printlnf( "Syntax:");
        ch->printlnf( "Smite <victim> <hp> <mana> <move> <position> <equipment_location>");
	return;
    }
    ...
    {
	ch->printlnf( "Syntax:");
        ch->printlnf( "Smite <victim> <hp> <mana> <move> <position> <equipment>");
	return;
    }
    ...
    if ( hp_percent > 95 || hp_percent < 0 )
    {
        ch->printlnf("Hp percent must be between 0 and 95." );
        return;
    }
    ...
    if ( mana_percent > 95 || mana_percent < 0 )
    {
        ch->printlnf("Mana percent must be between 0 and 95." );
        return;
    }
    ...
    if ( move_percent > 95 || move_percent < 0 )
    {
        ch->printlnf("Move percent must be between 0 and 95." );
        return;
    }
    ...
    if (ch->alliance > -2 && ch->alliance < 2)
    {
    ...
    if ( hp_percent > 75 && victim->hit > victim->max_hit / 4 )
        victim->printlnf( "`#`RThat really did HURT!`^");
   ...
   /* Let's see if equipment needs to be 'blown away' */
    ...
    ch->printlnf("Your will is done, your power felt.");
    return;
}
...
void do_pretitle(char_data *ch,char *argument)
{
    ...
    if(IS_NULLSTR(arg1)){
		ch->println("Syntax:");
		ch->println("Pretitle <char> <title>");
		ch->println("Gives a player a particular title before their name.");
		return; 
	}
	...
	if ( (value = flag_value(title_flags, arg2) ) == NO_FLAG )
	{
		ch->printlnf( "Invalid title '%s'", argument );
		return;
	}

    TOGGLE_BIT( victim->title, value );
	 ch->println( "Title Flag toggled." );
    return;
}
...
void do_challenge(char_data *ch, char *argument)
{
    ...
    if (argument[0] == '\0')
    {
        ch->println("You must specify who you want to challenge.");
        return;
    }

    if(HAS_CONFIG2(ch,CONFIG2_NOPKILL))
    {
	ch->println("You are banned from killing other players.");
	return;
    }

    if ( IS_AFFECTED2( ch, AFF2_HALLUCINATE ) )
    {
         ch->println("You are too stoned to fight.");
         return;
    }

    if(IS_SET(ch->in_room->room_flags, ROOM_NO_RECALL))
    {
         ch->println("You cannot even recall out of here, much less fight.");
         return;
    }

	if (is_challenge)
	{
		ch->println("Someone has been challenged wait a few moments to try again.");
		return;
    }

	if (arena_is_busy)
    {
		ch->println("The arena is being used at the moment..Please wait a few minutes.");
		return;
    }

    if ( ( victim = get_char_world( ch, argument ) ) == NULL )
    {
	ch->println( "They are saved only by their absence.");
	return;
    }

    if (!can_see( victim, ch ))
    {
	ch->printlnf( "Sorry but %s cannot see you at all.", victim->name );
        return;
    }

    if(HAS_CONFIG2(victim,CONFIG2_NOPKILL))
    {
	ch->println("They are banned from killing other players, and cannot fight you.");
	return;
    }

    if(IS_SET(ch->act2,ACT2_PLR_CHALLENGED))
    {
		ch->println("You have been challenged already.  Accept or decline.");
		return;
    }

    if(IS_SET(victim->act2,ACT2_PLR_CHALLENGER))
    {
		ch->printlnf("%s has already challenged someone else.", victim->name);
		return;
    }

	if (IS_SET(victim->comm,COMM_AFK))
	{
		ch->printlnf("Sorry but %s is AFK at the moment.", victim->name);
		return; 
    }

	if (IS_NPC(victim) || victim == ch) 
    {
        ch->println("You can only challenge players or yourself.");
        return; 
    }

    if ( IS_AFFECTED2( victim, AFF2_HALLUCINATE ) )
    {
         ch->println("They are too stoned to fight.");
         return;
    }
    ...
    {
        ch->printlnf("Sorry but %s is in combat right now.", victim->name);
        return;
    }
	...
	SET_BIT(ch->act2,ACT2_PLR_CHALLENGER);
	ch->println("Your challenge has been sent if they done accept in 3 ticks it will decline");
	ch->printlnf("If they do not accept it will be automatically withdrawn from %s.",victim->name);
	victim->printlnf("`#`RYou have been challenged by `W%s `Rto fight in the Arena.`^", ch->name);
	victim->printlnf("`#`wType `Yarenayes %s `wto accept or `Yarenano %s `w`to decline.`^",ch->name, ch->name);
	challenge_tme = 3;
	is_challenge = true;
	return;
}
...
void do_accept(char_data *ch, char *argument)
{
    ...
    if (IS_NPC(ch))
    {
         ch->println("Hrmm....You must be a new kinda mobile to accept challenges eh ?");
         return;
    }
    ...
    if (argument[0] == '\0')
    {
        ch->println("You must be specify who challenged you. I am only a MUD not a mindreader!");
        ch->println("`#Syntax: `Yarenayes `c<`gname`c>`^");
        return;
    }
    ...
    if (IS_NPC(victim))
    {
        ch->println("Hrmm, I dont think I can let you die like that.");
        return;
    }

    if (victim == ch)
    {
        ch->println("I bet you think your funny eh ?");
        return;
    }

    if (!IS_SET(victim->act2, ACT2_PLR_CHALLENGER))
    {
        ch->println("Now have you drank that much ale that you can't remember who challenged you ?");
        return;
    }

    if ( IS_AFFECTED2( victim, AFF2_HALLUCINATE ) )
    {
         ch->println("They are too stoned right now.");
         return;
    }
    
    /* these will need to get changed for your mud :p */
    room1 = get_room_index(number_range(3200,3219));
    room2 = get_room_index(number_range(3200,3219));
    info_broadcast(victim, "%s has accepted the challenge of %s to duel in the Arena.", ch->name, victim->name);
    victim->printlnf("%s has accepted your challenge!", ch->name);
    victim->println("You suddenly find yourself in the Arena.");
    char_from_room(victim);
    char_to_room( victim, room1 );
    do_look(victim,"auto");
    ch->println("You suddenly find yourself in the Arena.");
    char_from_room(ch);
    char_to_room( ch, room2 );
    do_look(ch, "auto");
    SET_BIT(ch->act2,ACT2_PLR_CHALLENGED);
    arena_is_busy = true;
    is_challenge = false;
    challenge_tme = 0;
    return;
...
void do_decline(char_data *ch, char *argument)
{
    ...
    if (argument[0] == '\0')
    {
        ch->println("You must specify who you want to decline.");
        ch->println("`#Syntax: `Yarenano `c<`gname`c>`^");
        return;
    }
    ...
    if (arena_is_busy)
    {
	ch->println("Alright the arena is in use so no one yet again has challenged you.");
	return;
    }
    ...
    if(!IS_SET(victim->act2,ACT2_PLR_CHALLENGER))
    {
	ch->printlnf("%s hasn't challenged you.", victim->name);
	return;
    }
    ...
    if (IS_NPC(victim) || victim == ch)
    {
        ch->println("How did that happen?");
        return; 
    }
    ...
    victim->printlnf("`#`R%s declined your invitation to duel!", ch->name);
    ch->printlnf("`#`RYou have choosen not to duel %s!", victim->name);
    return;
}
...
void do_withdraw(char_data *ch, char *argument)
{
    ...
    {
        ch->println("You haven't challenged anyone!");
        return;
    }
    ...
    if (arena_is_busy)
    {
        ch->println("The arena is busy at the moment.");
        return;
    }

    if (IS_NPC(victim))
    {
        ch->println("I think you are brain dead, have you been hitting the dwarven ale again ?");
        return;
    }

    is_challenge = false;
    REMOVE_BIT(ch->act2, ACT2_PLR_CHALLENGER);
    victim->printlnf("`#`R%s has withdrawn their challenge!", ch->name);
    ch->printlnf("`#`RYou withdraw your challenge to %s!", victim->name);
    return;
} 

void do_resetarena(char_data *ch, char * )
{
is_challenge = false;
arena_is_busy = false;
challenge_tme = 0;
ch->println("`#`YArena reset.`^");
}
/************************** Battlefield Code ****************************************/
bool has_wartoken( char_data *ch, char *argument)
{
	char arg[MIL];
	OBJ_DATA *obj;
	int number;
...
void do_war_blue(char_data *ch, char *)
{
    ROOM_INDEX_DATA *room1;
    
    if (IS_NPC(ch))
    {
         ch->println("Hrmm....You must be a new kinda mobile to think you ca go there ?");
         return;
    }
    ...
    if (has_wartoken(ch, "50097") )
    {
         ch->println("It has not been long enough since you left the battlefield.");
         return;
    }
  
    room1 = get_room_index(50021);
    info_broadcast(ch, "`W%s is fighting for the `BBLUE TEAM`W in the battlefield.`^", ch->name);
    ch->println("You suddenly find yourself in in the Battlefield.");
    char_from_room(ch);
    char_to_room( ch, room1 );
    do_look(ch, "auto");
    SET_BIT(ch->act2,ACT2_WAR_BLUE);
    return;
}

void do_war_red(char_data *ch, char *)
{
    ROOM_INDEX_DATA *room1;
    ...
    room1 = get_room_index(50022);
    info_broadcast(ch, "`W%s is fighting for the `RRED TEAM`W in the battlefield.`^", ch->name);
    ch->println("You suddenly find yourself in in the Battlefield.");
    char_from_room(ch);
    char_to_room( ch, room1 );
    do_look(ch, "auto");
    SET_BIT(ch->act2,ACT2_WAR_RED);
    return;
}

void do_war_leave(char_data *ch, char *)
{
    ROOM_INDEX_DATA *room1;
    OBJ_DATA *token;
    ...
    if (!IS_SET(ch->in_room->room2_flags,ROOM2_WAR))
    {
         ch->println("You are not even in the battlefield!");
         return;
    }

    room1 = get_room_index(50020);
    info_broadcast(ch, "`W%s has left the battlefield.`^", ch->name);
    ch->println("You suddenly find yourself out of the Battlefield.");
    char_from_room(ch);
    char_to_room( ch, room1 );

    if (IS_SET(ch->act2,ACT2_WAR_RED))
      REMOVE_BIT(ch->act2,ACT2_WAR_RED);
    if (IS_SET(ch->act2,ACT2_WAR_BLUE))
      REMOVE_BIT(ch->act2,ACT2_WAR_BLUE);
	
    token = create_object(get_obj_index(50097)); // Timed token
    token->timer = 10; // Number of ticks they cannot battle
    obj_to_char( token, ch ); // Send token to loser

    if (( token = get_obj_token( ch, "50098")) != NULL)
	{ 
	   obj_from_char (token);
	   extract_obj (token);
	}
	
    if (( token = get_obj_token( ch, "50099")) != NULL)
	{ 
	   obj_from_char (token);
	   extract_obj (token);
	}
...
/**************************************************************************/
void do_deckban( char_data *ch, char *argument )
{
	char arg[MIL],buf[MSL];
	char_data *victim;
	...
	if ( victim==ch){
		ch->println( "You really are hooked on that thing aren't you ?" );
		return;
	}
    ...
    if( IS_AFFECTED2(victim, AFF2_BAN_DECK))
    {
		REMOVE_BIT(victim->affected_by2, AFF2_BAN_DECK);
		victim->println( "You can play again." );
		ch->println( "BAN DECK removed." );
		sprintf(buf,"$N allows %s to use the Deck of Fate.",victim->name);
		wiznet(buf,ch,NULL,WIZ_PENALTIES,WIZ_SECURE,0);
    }
    else
    {
		SET_BIT(victim->affected_by2, AFF2_BAN_DECK);
		victim->println( "You have unable to use the Deck of Fate anymore!" );
		ch->println( "BAN DECK set." );
		sprintf(buf,"$N bans %s from the Deck of Fate.",victim->name);
		wiznet(buf,ch,NULL,WIZ_PENALTIES,WIZ_SECURE,0);
    }
...
void do_mphunt( char_data *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' )
    {
	ch->println( "Mphunt <victim> <mob vnum>" );
	return;
    }

    if ( ( victim = get_char_world ( ch, arg1 ) ) == NULL )
    {
	ch->println( "No such person");
	return;
    }
	...
	( master = victim->master ) )
	{
	    ...
	    {
		ch->println( "NULL NPC Master" );
		return;
	    }
	}
	else
	{
	    ch->println( "Mphunt - NPC victim" );
	    return;
	}
    }
    ...
    if ( arg2[0] == '\0' )
    {
	ch->println( "Mphunt - bad syntax, no aggressor");
	return;
    }
    else 
    {
	if ( is_number( arg2 ) )
	{
	    ...
	    if ( vnum == 0 )
	    {
		ch->println("Mphunt -- aggressor vnum out of range" );
		return;
	    }
	}
	else
	{
	    ch->println("Mphunt -- aggressor no vnum" );
	    return;
	}
    }
	...
	{
	    mob->mobmemory = victim;
	    hunt_victim( mob);
	}
    }
}

void vampgen( char_data *ch, char *argument )
{
    char 	arg1[MIL];
    char 	arg2[MIL];
    char_data   *victim;
    ...
    if ( arg1[0] == '\0' )
    {
	ch->println( "`BSyntax: `xvampgen <victim> <generation>" );
	return;
    }
   ...
   if (IS_NPC(victim))
   {
	ch->println("Not on an NPC.");
	return;
   }

   if (!IS_VAMPIRE(victim))
   {
	ch->printlnf("%s is not a vampire.", victim->name);
	return;
   }
	...
	if ( vnum == 1 )
	    {
		ch->printlnf("%s is now a 1st Generation Vampire.", victim->name );
		victim->println("You are now a 1st Generation Vampire.");
		victim->vampgen = 1;
		victim->bloodpool = 17;
		return;
	    }
	...
	if ( vnum == 2 )
	    {
		ch->printlnf("%s is now a 2nd Generation Vampire.", victim->name );
		victim->println("You are now a 2nd Generation Vampire.");
		victim->vampgen = 2;
		victim->bloodpool = 16;
		return;
	    }
	if ( vnum == 3 )
	    {
		ch->printlnf("%s is now a 3rd Generation Vampire.", victim->name );
		victim->println("You are now a 3rd Generation Vampire.");
		victim->vampgen = 3;
		victim->bloodpool = 15;
		return;
	    }
	if ( vnum == 4 )
	    {
		ch->printlnf("%s is now a 4th Generation Vampire.", victim->name );
		victim->println("You are now a 4th Generation Vampire.");
		victim->vampgen = 4;
		victim->bloodpool = 14;
		return;
	    }
	if ( vnum == 5 )
	    {
		ch->printlnf("%s is now a 5th Generation Vampire.", victim->name );
		victim->println("You are now a 5th Generation Vampire.");
		victim->vampgen = 5;
		victim->bloodpool = 13;
		return;
	    }
	if ( vnum == 6 )
	    {
		ch->printlnf("%s is now a 6th Generation Vampire.", victim->name );
		victim->println("You are now a 6th Generation Vampire.");
		victim->vampgen = 6;
		victim->bloodpool = 12;
		return;
	    }
	if ( vnum == 7 )
	    {
		ch->printlnf("%s is now a 7th Generation Vampire.", victim->name );
		victim->println("You are now a 7th Generation Vampire.");
		victim->vampgen = 7;
		victim->bloodpool = 11;
		return;
	    }
	if ( vnum == 8 )
	    {
		ch->printlnf("%s is now a 8th Generation Vampire.", victim->name );
		victim->println("You are now a 8th Generation Vampire.");
		victim->vampgen = 8;
		victim->bloodpool = 10;
		return;
	    }
	if ( vnum == 9 )
	    {
		ch->printlnf("%s is now a 9th Generation Vampire.", victim->name );
		victim->println("You are now a 9th Generation Vampire.");
		victim->vampgen = 9;
		victim->bloodpool = 9;
		return;
	    }
	if ( vnum == 10 )
	    {
		ch->printlnf("%s is now a 10th Generation Vampire.", victim->name );
		victim->println("You are now a 10th Generation Vampire.");
		victim->vampgen = 10;
		victim->bloodpool = 8;
		return;
	    }
	if ( vnum == 11 )
	    {
		ch->printlnf("%s is now a 11th Generation Vampire.", victim->name );
		victim->println("You are now a 11th Generation Vampire.");
		victim->vampgen = 11;
		victim->bloodpool = 7;
		return;
	    }
	if ( vnum == 12 )
	    {
		ch->printlnf("%s is now a 12th Generation Vampire.", victim->name );
		victim->println("You are now a 12th Generation Vampire.");
		victim->vampgen = 12;
		victim->bloodpool = 6;
		return;
	    }
	if ( vnum == 13 )
	    {
		ch->printlnf("%s is now a 13th Generation Vampire.", victim->name );
		victim->println("You are now a 13th Generation Vampire.");
		victim->vampgen = 13;
		victim->bloodpool = 5;
		return;
	    }

	ch->println("Pick a generation between 1 & 13" );
	ch->println("The earlier the generation the more powerfull the vampire.");
	return;
    }
    return;
}
...
/**************************************************************************/
void do_removeactive( char_data *ch, char *argument )
{
	char arg[MIL];
	char_data *victim;
	...
	if ( arg[0] == '\0' )
	{
		ch->println( "Remove active status on whom?" );
		return;
    }
    ...
    REMOVE_CONFIG(victim, CONFIG_ACTIVE);
    save_char_obj(victim);
    ch->printlnf("%s is no longer active.", victim->name);
    victim->println("You are no longer active.");
    return;
}
...
void do_omni( char_data *ch, char *argument )
{
	...
	for( wlevel=101; wlevel>91; wlevel-- )
	{
		for ( d = connection_list; d != NULL; d = d->next )
		{

	        if ( d->connected_state != CON_PLAYING || !can_see( ch, d->character ) )
		        continue;
			...
			if (!can_see(ch,wch)  || wch->level < 92)
				continue;

	        immmatch++;
	        nMatch++;

			sprintf( buf, "`G%-14s `R%d    `M%-3d    `C%-3d    `G[`W%5d`G]`x\n\r",
				wch->name,
				wch->level,
				wch->invis_level,
				wch->incog_level,
	...
	for( wlevel=91; wlevel>0; wlevel-- )
	{
	    for ( d = connection_list; d != NULL; d = d->next )
	    {
			...
			sprintf( buf, "`G%-8s `R%11s`G/`c%3s    `C%-15s `B%-3d  `M%3d%%  `G[`W%5d`G] %3d`x\n\r",
				wch->name,
				race_table[wch->race]->name,
				class_table[wch->clss].short_name,
	 ...
	 */
	sprintf( buf2, "\n\r`S IMMs found: `W%d`x\n\r", immmatch );
    add_buf(output,buf2);
    sprintf( buf2, "`DMorts found: `W%d`x\n\r", mortmatch );
    add_buf(output,buf2);
    ch->println( buf_string(output));
    free_buf(output);
    return;
}
...
void do_restring( char_data *ch, char *argument )
{
	...
	if (IS_HERO(ch))
	{
		ch->println( "Use the STRING command for free restrings." );
		return;
	}
    ...
    {
	ch->println( "Syntax:" );
	ch->println( "  RESTRING <name_of_object> <field> <string>" );
	ch->println( "  (hold the object you wish to string)" );
	ch->println( "    fields: name short long" );
	ch->println( "Each part costs 1/3 of the cost. There are no refunds if you goof up." );
	return;
    }
	...
	if (cost/3 > (ch->gold * 100 + ch->silver))
	{
		ch->wraplnf("It costs `#`C%d silver`^ to restring %s. `SYou do not have enough.`^", cost, obj->short_descr );
		return;
	}
...
void do_award(char_data *ch, char *argument)
{
     ...
     {
          ch->println( "Syntax: award <char> <award type> <value>.");
          ch->println( "Award types are: gold, exp, prac, train ");
          ch->println( "For more information, type 'HELP AWARD'.");
         return;
     }
     if (( victim = get_char_world ( ch, arg1 ) ) == NULL )
 	{
     ...
     if ( ch == victim )
 	{
 	      ch->println( "You may not award yourself!" );
 	     return;
 	}
 
 	if (IS_IMMORTAL(victim) || victim->level >= LEVEL_IMMORTAL)
 	{
 	      ch->println( "You cannot award an immortal!\n\r" );
 	     return;
     }
 
     if (is_name(arg2, "gold"))
     {
 		value = atoi( arg3 );
 
 	if (value < -10000 || value > 10000 )
 	{
 		 ch->println( "Value must be between -10000 and 10000." );
 	    return;
 	}
 
 	if (value == 0 )
 	{
 		 ch->println( "The value must not be equal to 0." );
 		return;
 	}
 
 	victim->gold += value;
 	 ch->wraplnf("You have awarded %s %d gold pieces", victim->name, value );
 
 	if ( value > 0 )
 	{
 	    ch->wraplnf("`GYou have been awarded %d gold pieces!!`^", value );
 	}
 	else
 	{
 	     ch->wraplnf("`RYou have been penalized %d gold pieces!!`^", value );
           return;
 	  }
   }
 	...
 	ch->wraplnf("You have awarded %s %d experience points.", victim->name, value);
     		
     	if ( value > 0 )
 		    {
 		      victim->wraplnf("`GYou have been awarded %d experience points.`^", value );
 		     }
 		    else
 		    {
 		      victim->wraplnf("`RYou have been penalized %d experience points.`^", value );
 		return;
 		  }
 	  }
 		  else if (is_name(arg2, "prac practice" ) )
 		  {
 		  	 ...
 		  	 {
 		  	 	 ch->println( "Value must be between -10 and 10." );
 		  	 	return;
 		  	 }
 			 ...
 			 victim->practice += value;
 			 ch->wraplnf("You have awarded %s %d prac(s).", victim->name, value);
 
 			 if ( value > 0 )
 			 {
 			   victim->wraplnf("`GYou have been awarded %d prac(s).`^", value );
 			 }
 			 else
 			 {
 	    	             victim->wraplnf("`RYou have been penalized %d prac(s).`^", value );
 				  return;
 			  }
 		  }
 		  else if (is_name(arg2, "tra train" ) )
 		  {
 			  value = atoi( arg3 );
 
 			  if (value < -5 || value > 5 )
 			  {
 				   ch->println( "Value must be between -5 and 5.");
 				  return;
 			  }
 			  if (value == 0 )
 			  {
 			  ...
 			  victim->train += value;
 			  ch->wraplnf("You have awarded %s %d train(s).", victim->name, value);
 
 			  if ( value > 0 )
 			  {
 			        victim->wraplnf("`GYou have been awarded %d train(s).`^", value );
 			  }
 			  else
 			  {
 			       victim->wraplnf("`RYou have been penalized %d train(s).`^", value );
 			       return;
 			  }
 		  }
 		  return;
...
void do_nuke( char_data *ch, char *argument)
{
    ...
    one_argument( argument, arg );
     if ( arg[0] == '\0' )
     {
         ch->println( "Syntax:nuke  <character name>" );
 	return;
     }
 
     if ( ( victim = get_char_world( ch, arg ) ) == NULL )
     {
 	ch->println( "They are not logged on\n\r");
 	return;
     }
 
    if (IS_NPC(victim))
     {
      ch->println( "This command only works on players");
       return;
     }
   SET_BIT(victim->act, PLR_DENY);
    stop_fighting(victim, true);
    save_char_obj(victim);
    info_broadcast( ch,"%s has been nuked by %s",victim->name,ch->name);
    do_quit(victim,"");
    // find the pfile and remove it
    PFILE_TYPE pt=find_pfiletype(victim->name);
    sprintf( buf, pfilename(victim->name, pt));
    unlink(buf);
    do_lastonremove(ch, victim->name); // remove them from the laston list 
    return;
}
/**************************************************************************/
void do_quazer( char_data *ch, char *argument )
{
    char 	arg1[MIL];
    char 	arg2[MIL];
    char_data   *victim;
    ...
    if ( arg1[0] == '\0' )
    {
	ch->println( "`BSyntax: `xquazer <victim> <points>" );
	return;
    }
	...
	if(!victim->quazer)
	    victim->quazer = 0;
	victim->quazer += points;
	ch->wraplnf("%s has been awarded `#`R%d quazer points`^.",victim->name, points);
	victim->wraplnf("You have been awarded `#`R%d quazer points`^.", points);
	return;
    }
...
/**************************************************************************/
void do_wishgrant(char_data *ch, char *argument)
{
     char_data *victim;
     char arg1 [MIL];
     char arg2 [MIL];
     ...
     if ( arg1[0] == '\0' || arg2[0] == '\0'  )
     {
          ch->println( "Syntax: wish <char> <wish type>.");
	  ch->println("`=rWishes: [`x darkvision scryproof hearing no_food flying `=r]");
	  ch->println("`=rWishes: [`x otterlungs regen sanctuary cleanspeech      `=r]");
	  ch->println("`=rWishes: [`x eatall vicegrip attack1 attack2 attack3    `=r ]");
          return;
     }
     if (( victim = get_char_world ( ch, arg1 ) ) == NULL )
 	{
     ...
     if (is_name(arg2, "darkvision"))
     {
	if(IS_SET(victim->wish, WISH_DARKVISION))
	{
		REMOVE_BIT(victim->wish, WISH_DARKVISION);
		ch->println("Darkvision removed.");
		return;
	}
	SET_BIT(victim->wish, WISH_DARKVISION);
	ch->println("Darkvision Granted.");
	return;
    }
    if (is_name(arg2, "scryproof"))
    {
	if(IS_SET(victim->wish, WISH_SCRYPROOF))
	{
		REMOVE_BIT(victim->wish, WISH_SCRYPROOF);
		ch->println("Scryproof removed.");
		return;
	}
	SET_BIT(victim->wish, WISH_SCRYPROOF);
	ch->println("Scryproof Granted.");
	return;
    }
    if (is_name(arg2, "hearing"))
    {
	if(IS_SET(victim->wish, WISH_HEARING))
	{
		REMOVE_BIT(victim->wish, WISH_HEARING);
		ch->println("Hearing removed.");
		return;
	}
	SET_BIT(victim->wish, WISH_HEARING);
	ch->println("Hearing Granted.");
	return;
    }
    if (is_name(arg2, "no_food"))
    {
	if(IS_SET(victim->wish, WISH_IMMFOOD))
	{
		REMOVE_BIT(victim->wish, WISH_IMMFOOD);
		ch->println("Food Immunity removed.");
		return;
	}
	SET_BIT(victim->wish, WISH_IMMFOOD);
	ch->println("Food Immunity Granted.");
	return;
    }
    if (is_name(arg2, "flying"))
    {
	if(IS_SET(victim->wish, WISH_FLYING))
	{
		REMOVE_BIT(victim->wish, WISH_FLYING);
		ch->println("Flying removed.");
		return;
	}
	SET_BIT(victim->wish, WISH_FLYING);
	ch->println("Flying Granted.");
	return;
    }
    if (is_name(arg2, "eatall"))
    {
	if(IS_SET(victim->wish, WISH_EATALL))
	{
		REMOVE_BIT(victim->wish, WISH_EATALL);
		ch->println("Eatall removed.");
		return;
	}
	SET_BIT(victim->wish, WISH_EATALL);
	ch->println("Eatall Granted.");
	return;
    }
    if (is_name(arg2, "otterlungs"))
    {
	if(IS_SET(victim->wish, WISH_OTTERLUNGS))
	{
		REMOVE_BIT(victim->wish, WISH_OTTERLUNGS);
		ch->println("Otterlungs removed.");
		return;
	}
	SET_BIT(victim->wish, WISH_OTTERLUNGS);
	ch->println("Otterlungs Granted.");
	return;
    }
    if (is_name(arg2, "regen"))
    {
	if(IS_SET(victim->wish, WISH_REGEN))
	{
		REMOVE_BIT(victim->wish, WISH_REGEN);
		ch->println("Regen removed.");
		return;
	}
	SET_BIT(victim->wish, WISH_REGEN);
	ch->println("Regen Granted.");
	return;
    }
    if (is_name(arg2, "sanctuary"))
    {
	if(IS_SET(victim->wish, WISH_SANCTUARY))
	{
		REMOVE_BIT(victim->wish, WISH_SANCTUARY);
		ch->println("Sanctuary removed.");
		return;
	}
	SET_BIT(victim->wish, WISH_SANCTUARY);
	ch->println("Sanctuary Granted.");
	return;
    }
    if (is_name(arg2, "cleanspeech"))
    {
	if(IS_SET(victim->wish, WISH_CLEANSPEECH))
	{
		REMOVE_BIT(victim->wish, WISH_CLEANSPEECH);
		ch->println("Cleanspeech removed.");
		return;
	}
	SET_BIT(victim->wish, WISH_CLEANSPEECH);
	ch->println("Cleanspeech Granted.");
	return;
    }
    if (is_name(arg2, "vicegrip"))
    {
	if(IS_SET(victim->wish, WISH_VICEGRIP))
	{
		REMOVE_BIT(victim->wish, WISH_VICEGRIP);
		ch->println("Vicegrip removed.");
		return;
	}
	SET_BIT(victim->wish, WISH_VICEGRIP);
	ch->println("Vicegrip Granted.");
	return;
    }
    if (is_name(arg2, "attack1"))
    {
	if(IS_SET(victim->wish, WISH_ATTACK_1))
	{
		REMOVE_BIT(victim->wish, WISH_ATTACK_1);
		ch->println("Attack1 removed.");
		return;
	}
	SET_BIT(victim->wish, WISH_ATTACK_1);
	ch->println("Attack1 Granted.");
	return;
    }
    if (is_name(arg2, "attack2"))
    {
	if(IS_SET(victim->wish, WISH_ATTACK_2))
	{
		REMOVE_BIT(victim->wish, WISH_ATTACK_2);
		ch->println("Attack2 removed.");
		return;
	}
	SET_BIT(victim->wish, WISH_ATTACK_2);
	ch->println("Attack2 Granted.");
	return;
    }
    if (is_name(arg2, "attack3"))
    {
	if(IS_SET(victim->wish, WISH_ATTACK_3))
	{
		REMOVE_BIT(victim->wish, WISH_ATTACK_3);
		ch->println("Attack3 removed.");
		return;
	}
	SET_BIT(victim->wish, WISH_ATTACK_3);
	ch->println("Attack3 Granted.");
	return;
    }
          ch->println( "Syntax: wish <char> <wish type>.");
	  ch->println("`=rWishes: [`x darkvision scryproof hearing no_food flying `=r]");
...
/**************************************************************************/
void do_purgearea( char_data *ch, char *argument )
{
   char arg[MIL];
   ...
   OBJ_DATA *obj, *obj_next;
	...
	int high_vnum, low_vnum;
	bool complete_purge;
	bool purged_mob, purged_obj;
   one_argument( argument, arg );
	...
	if( one_argument( argument, arg ) && !str_cmp("complete", arg ) )
		complete_purge = true;
	else 
		complete_purge = false;
	
	for(int i = low_vnum; i < high_vnum; i++)
	{
		// check if this room exists and skip vnum if it doesn't
		if ( !( room = get_room_index( i ) ) )
			continue;

		// Strip this whole room bare
		for ( victim = room->people ; victim != NULL; victim = vnext )
		{
			...
			if (IS_NPC(victim)  && IS_SET(victim->act,ACT_NOPURGE) )
			{
				...
				if(!purged_mob)
					ch->println( "Nopurge mobs were purged.");
		...
		for ( obj = room->contents; obj != NULL; obj = obj_next )
		{
			...
			if (IS_OBJ_STAT(obj,OBJEXTRA_NOPURGE))
			{
				...
				if(!purged_obj)
					ch->println( "Nopurge objects were purged.");
		...
		room->last_mined_in_room=0; // clear the last mined stats
   }
	...
	for (d = connection_list; d; d = d->next)
   {
		...
		&&  d->character->in_room->area == ch->in_room->area)
		{
			...
			d->character->printlnf( "A black wind blows through scouring everything away!");
		}
   }	
	ch->printlnf( "Ok, area purged." );
   return;
}
/**************************************************************************/
void do_pcletgainer( char_data *ch, char *argument )
{
	char arg[MIL];
	char_data *victim;
	...
	if ( arg[0] == '\0' )
	{
		ch->println( "Toggle letgainer status on whom ?" );
		return;
    }
    ...
    if ( HAS_CONFIG2(victim, CONFIG2_LETGAIN))
    {
		ch->println( "Letgaining Power Removed." );
		REMOVE_CONFIG2(victim, CONFIG2_LETGAIN);
    		save_char_obj(victim);
		return;
    }
    ...
    SET_CONFIG2(victim, CONFIG2_LETGAIN);
    save_char_obj(victim);
    ch->printlnf("%s can now letgain players.", victim->name);
    victim->println("You can now letgain players.");
    return;
}
/**************************************************************************/
void do_scatter( char_data *ch, char *argument )
{
  ...
  one_argument(argument, arg);
  	...
  	if (arg[0] == '\0') 
	{
		ch->println("You must specify object, mobile, or players.");
		return;
    	}
	if (!str_prefix(arg, "object"))
	{
  		...
  		for ( obj = ch->in_room->contents; obj != NULL; obj = obj_next ) 
		{
			   ...
			   if (	!IS_SET(pRoomIndex->room_flags,		ROOM_NO_RECALL	)
				&&  !IS_SET(pRoomIndex->room_flags,	ROOM_NO_SUMMON	)
				...
				&&  !IS_SET(pRoomIndex->room_flags,	ROOM_ANTIMAGIC	)
				&&  !IS_SET(pRoomIndex->room_flags,	ROOM_OOC	)
				&&  !IS_SET(pRoomIndex->room_flags,	ROOM_INN	)
				&&  !IS_SET(pRoomIndex->room_flags,	ROOM_PRIVATE	)
				&&  !IS_SET(pRoomIndex->room_flags,	ROOM_SOLITARY	)
				&&  !is_name( pRoomIndex->area->file_name, "houses.are" ) )
					break;
    			}
	...
	if (!str_prefix(arg, "players"))
	{
		for (victim = ch->in_room->people; victim != NULL; victim = vnext) 
		{
			...
			if (!IS_NPC(victim) && !IS_IMMORTAL(victim))
			{
				char_from_room(victim);
    				char_to_room(victim, pRoomIndex);
				victim->println("`#`RYou have been scattered!`^");
				do_look(victim, "auto");
			}
  		}
		ch->println("Players scattered.");
		return;
	}
}
/**************************************************************************/
