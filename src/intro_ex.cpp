	...
	if(!str_cmp(argument, "all")){
		for(victim=ch->in_room->people; victim; victim=victim->next_in_room){
			...
			if(ch->title)
			{
				...
				char buf[MSL];
				sprintf(buf,"$N introduces $Mself to you as %s $t", capitalize( flag_string(title_flags, ch->title)));
				act(buf,victim, ch->name, ch, TO_CHAR);
			}
			else
			{
				ch->printlnf("You introduce yourself to %s", PERS(victim, ch));
...
char * PERS( char_data *ch, char_data *looker)
{
	...
	strcpy(short_descr, mxp_create_tag(looker, mxp_tag_for_mob(looker, ch), ch->poly_short));
				   ...
				   && str_cmp(ch->short_descr, ch->name) )
				{
					if(looker && HAS_CONFIG2(looker,CONFIG2_NAME_ONLY_FOR_KNOWN)){
						...
						{						
							if(ch->title)
							{
								sprintf( result, "(%s %s) %s", capitalize( flag_string(title_flags, ch->title)), capitalize(ch->name), short_descr);
							}
							else{
							...
							if(ch->title)
							{
								sprintf( result, "%s (%s %s)", short_descr, capitalize( flag_string(title_flags, ch->title)), capitalize(ch->name));
							}
							else{
					...
					if (IS_OOC(ch) && IS_OOC(looker)){
						...
						if (!IS_VAMPAFF(looker,VAM_AUSPEX) && HAS_CONFIG2( ch, CONFIG2_CLOAKED))
						{
							strcpy( result, short_descr);
						}
    						if ( !IS_VAMPAFF(looker, VAM_AUSPEX) && IS_VAMPAFF(ch, VAM_DISGUISED) )
						{
							strcpy( result, short_descr);
						}
					}
