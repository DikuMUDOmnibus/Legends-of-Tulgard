	...
	for ( ; ; )
	{
		...
		for ( ; ; )
		{
			...
			case 'S':
				...
				{
					pRoomIndex->sector_type = fread_wordflag( sector_types, fp );
					pRoomIndex->death_energy = sect_table[pRoomIndex->sector_type].death_energy;
					fMatch = true;
					break;
				}
			}
...
void save_object_NAFF( FILE *fp, OBJ_INDEX_DATA *pObjIndex )
{
    ...
    fprintf( fp, "Cost %d\n", pObjIndex->cost );
	if(pObjIndex->condition!=5){
		fprintf( fp, "Durability %d\n", URANGE(1,pObjIndex->durability,10));
	}
    ...
    if (pObjIndex->clan != NULL)
	 {	  

	 	fprintf( fp, "Clan %s~\n",	pack_word(pObjIndex->clan->savename()));
	 }
...
void load_objects_NAFF( FILE *fp, int version)
{
	...
	for ( ; ; )
	{
		...
		for ( ; ; )
		{
			...
			case 'C':
				KEY( "Clan",			pObjIndex->clan,	clan_slookup(fread_word( fp )));
			...
			case 'D':
				...
				KEY( "Durability",		pObjIndex->durability,		fread_number( fp ));
				break;
...
void save_mobile_NAFF( FILE *fp, MOB_INDEX_DATA *pMobIndex )
{
	...
	fwrite_wordflag( terr_restrict_flags,		pMobIndex->terr_restrict,		"Ter    ", fp);
...
void fread_mob_NAFF( MOB_INDEX_DATA *pMob, FILE *fp )
{
	...
	for ( ; ; )
	{
		...
		case 'T':
			KEY( "Ter",	pMob->terr_restrict,		fread_wordflag( terr_restrict_flags, fp ));
			break;
