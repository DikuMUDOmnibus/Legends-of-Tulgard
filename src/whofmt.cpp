...
char *whoformat_dawn ( char_data *ch, char_data *wch, bool two_column)
{
 ...
 // Format it up //%[%*%]#  %new%i%w%b%a%o%O%P%i%h%s%Q%C%l%o%q
	sprintf( buf, "%s%s%s%s%s`^%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s",
		left_bracket,
		clss,
		right_bracket,
		...
		HAS_CONFIG2(wch, CONFIG2_DUNCE)? " `#`Y[DUNCE]`^" : "",		 
...
char *whoformat_endless( char_data *ch, char_data *wch, bool two_column)
{
	...
	char name[MIL];
	{
		...
		if(wch->title)
		{
			strcpy(name, capitalize(FORMATF("`#`R%s`^", flag_string(title_flags, wch->title))));
			strcat(name, FORMATF(" %s", wch->name));
		}
		else strcpy(name, wch->name);
		// generate the name displayed
		// put the afk message or title depending on if they are afk
		if(IS_SET(wch->comm, COMM_AFK) && wch->pcdata 
			&& !IS_NULLSTR(wch->pcdata->afk_message))
	...
	if( wch->level<LEVEL_IMMORTAL)
	{
		if (IS_VAMPIRE(wch) && IS_IMMORTAL(ch) && HAS_CONFIG2(wch, CONFIG2_LETGAIN))
			clss="`#`YVam`^";
		else
		if (IS_WEREBEAST(wch) && IS_IMMORTAL(ch) && HAS_CONFIG2(wch, CONFIG2_LETGAIN))
			clss="`#`YLyc`^";
		else
		if (IS_VAMPIRE(wch) && (IS_IMMORTAL(ch) || wch == ch))
			clss="`#`RVam`^";
		else
		if (IS_WEREBEAST(wch) && (IS_IMMORTAL(ch) || wch == ch))
			clss="`#`CLyc`^";
		else
		if (HAS_CONFIG2(wch, CONFIG2_LETGAIN) && IS_IMMORTAL(ch) && IS_NEWBIE_SUPPORT(wch))
		{
			clss = class_table[wch->clss].short_name;
			sprintf(whoclass,"`#`Y%s`^", clss);
			clss=whoclass;
		}
		else
		...
		if (HAS_CONFIG2(wch, CONFIG2_LETGAIN) && IS_IMMORTAL(ch))
		{
			clss = class_table[wch->clss].short_name;
			sprintf(whoclass,"`#`Y%s`^", clss);
			clss=whoclass;
 ...
 // Format it up //idl%[%*%]# %new%i%w%b%a%o%O%i%h%s%Q%C%B%l%o%q%newbie
    ...
    if (wch->clan>0)
   {
		if ((wch->clan->hidden() == false) || (wch->clan->hidden() == true && IS_IMMORTAL(ch)))
		{
			if (c_str_len(buf)>40)
			{
				sprintf( buf2, "%s %s", buf, 
		...
		if((wch->clan->hidden() == true) && is_same_clan(wch, ch))  
		{
			if (c_str_len(buf)>40)
			{
				sprintf( buf2, "%s %s", buf, 
