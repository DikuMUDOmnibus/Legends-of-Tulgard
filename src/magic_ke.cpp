...
SPRESULT spell_treeform( int sn, int level, char_data *ch, void *, int )
{
	...
	{
		ch->println( "You may only cast this places that might be forested." );
		return NO_MANA;
	}
...
/*******************************************************************************/
/*******************************************************************************/
SPRESULT spell_blight( int sn, int level, char_data *ch, void *, int )
{
	AFFECT_DATA	af;
	char_data *rch;
	int dam;
	...
	if (  ch->in_room->sector_type == SECT_INSIDE
		...
		|| IS_SET( ch->in_room->room_flags, ROOM_INDOORS ))
	{
		ch->println( "You can only blight out of doors." );
		return NO_MANA;
	}

	
	ch->println( "The ground blackens." );
	act( "The ground blackens around $n.", ch, NULL, NULL, TO_ROOM );
	...
	af.location		= APPLY_NONE;
	af.modifier		= 0;
	af.bitvector	= ROOMAFF_BLIGHT;
						
	affect_to_room( ch->in_room, &af );

	ch->println( "The room grows cold." );
	act( "The room grows cold.", ch, NULL, NULL, TO_ROOM );
	return FULL_MANA;
}
/*******************************************************************************/
