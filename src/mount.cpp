...
void do_ride( char_data *ch, char *argument )  
{
    ...
    if (!IS_NPC(victim) && !IS_SET(victim->pcdata->pconfig, PCONFIG_MOUNTABLE))
    {
        ch->println( "You cannot mount other players without consent!" );
        return;
    }
...
void do_tether( char_data *ch, char *argument)
{
	...
	{
		ch->println("I don't think they'd appreciate that.");
		return;
	}
...
void do_dismount( char_data *ch, char *)
{
    ...
    act("$n dismounts $N.",ch, NULL, ch->mounted_on, TO_NOTVICT);
    act("You dismount $N.",ch, NULL, ch->mounted_on, TO_CHAR);
    act("$n dismounts you.",ch, NULL, ch->mounted_on, TO_VICT);
...
void do_buck( char_data *ch, char *argument)
{
	...
	if(!IS_SET(ch->form, FORM_MOUNTABLE))
		do_huh(ch, "");
    if ( !rider){
			ch->println( "You don't have anyone to buck off." );
			return;
	}
	
	if(number_percent() + get_skill(rider, gsn_riding) <= ch->level * 2)
	{
		act("$n is thrown from $N!",rider, NULL, ch, TO_NOTVICT);
		act("You have been thrown off of $N!",rider, NULL, ch, TO_CHAR);
		act("$n has been thrown from your back.",rider, NULL, ch, TO_VICT);
		dismount(rider);		
	}
	else{
