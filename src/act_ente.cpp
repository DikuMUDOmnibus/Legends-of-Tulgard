...
ROOM_INDEX_DATA  *get_random_room(char_data *ch)
{
    ...
    for ( count=0; count<1000; count++)
    {
		...
		if ( !can_see_room(ch,room)
			...
			|| (IS_SET(room->room2_flags, ROOM2_SHADOWPLANED)
			&& !IS_AFFECTED2(ch, AFF2_SHADOWPLANE))
			)
			continue;

        if ( room->area->world == NULL)
                   continue;  
        ...
        if ( ch->in_room->area->world == NULL)
	{
        ...
        if ( ch->in_room->area->world->name != room->area->world->name )
                   continue;  
...
{    
    ...
    if (!IS_NULLSTR(argument))
    {
		...
		if (ch->ridden_by!=NULL && IS_NPC(ch))
		{
		...
		{
			ch->println("Your rider couldn't fit inside.");
			return;
		}
