...
void	load_territory_db	args( ( void ) );
void	load_herb_db		args( ( void ) );
void	load_offmoot_db args( ( void ) );
void	load_pigeon_db args( ( void ) );
void	load_vehicle_db args( ( void ) );
...
void boot_db()
{
	...
	load_worlds();				update_currenttime();
	...
	*/
	{
		...
		resort_top_crime();					update_currenttime();
		resort_top_arena();					update_currenttime();
	}
	...
	// loadup territory database
	load_territory_db();				update_currenttime();
...
void load_arearom( FILE *fp )
{
	...
	pArea->world		= NULL;		// default them as none
	...
	for ( ; ; )
	{
		...
		case 'W':
			...
			if ( !str_cmp( word, "World" ) ){
				char *t=fread_string( fp );
				pArea->world = world_lookup( t );

				if ( !pArea->world){										
					logf("Automatically added world '%s' while reading in area.", t),
					autonote(NOTE_SNOTE, "load_area()",
						FORMATF("Automatically added world '%s'", t),
						"admin", 
						FORMATF("Automatically added world '%s' while reading in area '%s' (%s).", 
							t, pArea->name, pArea->file_name ),
						true);
					
					pArea->world=new world_type;

					// insert the new world at the front of the list
					pArea->world->name=str_dup(ltrim_string(rtrim_string(t)));
					pArea->world->next=world_list;
					world_list=pArea->world;					
					save_worlds();
				}
				fMatch = true;
			}
...
/* 	mob->gamble_fun		= pMobIndex->gamble_fun; */
	...
	mob->terr_restrict              = pMobIndex->terr_restrict;
...
void clone_mobile(char_data *parent, char_data *clone)
{
    ...
    clone->terr_restrict        = parent->terr_restrict;
    ...
    for ( i = 0; str[i] != '\0'; i++ ){
		if(str[i]=='`'){ // we have a colour code
			...
			if(first_char && is_alpha(str[i])){
				...
				strcap[index][i] = str[i]; // not first, non colour code character - lowercase
			}
		}
	}
    strcap[index][i] = '\0';
