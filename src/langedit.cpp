...
bool langedit_show(char_data *ch, char *)
{
	...
	ch->printlnf("`=rDifficulty:   `x%d    `x",lang->difficulty);	
...
void language_recreatebase(language_data *lang)
{
	...
	for(i=0; i<25; i++){
		...
		char l=upshuffle[i];
		upshuffle[i]=upshuffle[pos];
		upshuffle[pos]=l;
	}

	// load the shuffled array as new words
	char from[2], to[2], upfrom[2], upto[2];
	from[1]='\0';
	to[1]='\0';
	upfrom[1]='\0';
	upto[1]='\0';
	for(i=0; i<26; i++){
		...
		upfrom[0]=i+'A';
		upto[0]=upshuffle[i];
		lang->add_wordmap_to_tree(upfrom, upto);
	}
}
/**************************************************************************/
...
/**************************************************************************/
bool langedit_difficulty( char_data *ch, char *argument )
{
	language_data *lang;
	int value;
    ...
    {
		EDIT_LANGUAGE(ch,lang);
		ch->printlnf( "Difficulty changed from %d to %d.", 
			lang->difficulty, value);
		lang->difficulty = value;
		return true;
    }

    ch->println("Syntax:  difficulty <number>");
	ch->println("Where number can range from 1 (childish) to 10 (ungodly).");
    return false;
}
/**************************************************************************/
