...
char *olc_ed_vnum( char_data *ch )
{
	...
	case ED_DEITY:
		...
		break;
	case ED_TERRITORY:
		pTerritory= (TERRITORY_DATA *)ch->desc->pEdit;
		sprintf( buf, "%s", pTerritory ? pTerritory->name : "???" );
		break;
	case ED_QUEST:
...
void do_resets( char_data *ch, char *argument )
{
	...
	*/
	...
	if(IS_SET(ch->in_room->room2_flags, ROOM2_STORAGE))
	{
		ch->println("No resets allowed in storage room. Load the object and/or drop one to save.");
		show_resets_help(ch);
		return;
	}
...
void do_vlist( char_data *ch, char *argument )
{
    ...
    sprintf( result, "[%3s] [%-27s] (%-5s-%5s) [%-14s] %3s [%-10s] [%-12s] [%-12s]\r\n",
       "Num", "Area Name", "lvnum", "uvnum", "Filename", "Sec", "Builders", "World", "Continent" );
    ...
    for ( pArea = area_vnumsort_first; pArea; pArea = pArea->vnumsort_next )
    {
		...
		if (!is_name( argument, pArea->name ) // filter in only required areas
			&& !is_name( argument, pArea->file_name) 
			&& !is_name( argument, pArea->builders)
            && !is_name( argument, pArea->world?pArea->world->name:"none")
			&& !is_name( argument, pArea->continent?pArea->continent->name:"any"))
			continue;

		sprintf(aname,"%s", str_width(pArea->name,29));
        sprintf( buf, "[%3d] %s (`#%s%-5d`^%s-%5d`^) %-16.16s [%d] [%-10.10s] [%-16.16s] [%-16.16s]`x\r\n",
			pArea->vnum,
			aname,
			prenum,
			pArea->min_vnum,
...
void tedit( char_data *ch, char *argument )
{
    ...
    for ( cmd = 0; tedit_table[cmd].name != NULL; cmd++ )
    {
        if ( !str_prefix( command, tedit_table[cmd].name ))
        {
            ...
            {
				SET_BIT(TERRITORY_FLAGS,DEDIT_CHANGED);
				return;
            }
            else
				return;
