...
const struct flag_type game_settings_flags4[] =
{
	...
	{ "random_objects_enabled",		GAMESET4_RANDOM_OBJECTS_ENABLED, true},
	{ "no_pk_outside_10_levels",		GAMESET4_NO_PK_OUTSIDE_10_LEVELS, true},
	{ NULL,0,0}
};
...
const struct flag_type game_settings_flags5[] =
{
	...
	{ "ramdom_events_enabled",	GAMESET5_RANDOM_EVENTS_ENABLED, true},
	{ NULL,0,0}
};
...
void do_load_gamesettings(char_data *, char *)
{
	...
	if(IS_NULLSTR(game_settings->aura_shadowplane)){
		game_settings->aura_shadowplane=str_dup("`#`S(Shadowed)`& ");
	}
...
/**************************************************************************/
GIO_START(world_type)
GIO_STR(name)
GIO_FINISH_STRDUP_EMPTY
/**************************************************************************/
void save_worlds()
{
	log_string("save_worlds(): saving worlds list.");
	GIOSAVE_LIST(world_list, world_type, WORLDS_FILE, true);
}

/**************************************************************************/
void create_worlds()
{
	const char *world_create_table[]={
		"none", "Arthos", "Olympus", "StupidWorld", "Abaddon",
		NULL
	};
	...
	int i;
	for(i=0; !IS_NULLSTR(world_create_table[i]); i++){
		// loop thru till one past the final entry
	}
	
	world_list=NULL;
	// now loop backwards to add worlds in reverse order
	for(i--;i>0; i--){
		c=new world_type; 
		c->name=str_dup(world_create_table[i]);
		c->next=world_list;
		world_list=c;
	}
}
...
void load_worlds()
{
	log_string("load_worlds(): Reading in worlds list from '"WORLDS_FILE"'.");
	if(file_exists(CONTINENTS_FILE)){
		GIOLOAD_LIST(world_list, world_type, WORLDS_FILE);
		if(!world_list){
			log_string("No worlds found in file, creating new set.");
			create_worlds();
		}			
		return;		
	}
	log_string("Worlds file not found, creating default set of worlds.");
	create_worlds();
	save_worlds();
}
/**************************************************************************/
