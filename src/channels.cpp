...
void channel_generic_handler( int channel_index, char_data *ch, char *message)
{
	...
	for ( c = player_list; c; c=c->next_player )
	{
		...
		if (IS_SET(ch->config2, CONFIG2_SHUTUP) && c != ch)
			continue;

		if(is_ignoring(c, ch) == true)
			continue;
...
{
   if (IS_SET(ch->comm,COMM_NOOOC) || (!IS_SET(ch->act,PLR_CAN_ADVANCE)))
 		ch->println( "You cannot use OOC." );
   else 
   {
...
{
   if (IS_SET(ch->comm,COMM_NOOOC) || (!IS_SET(ch->act,PLR_CAN_ADVANCE)))
 		ch->println( "You cannot use this channel at this time." );
   else 
   {
...
{
   if (IS_SET(ch->comm,COMM_NOOOC) || (!IS_SET(ch->act,PLR_CAN_ADVANCE)))
 		ch->println( "ou cannot use this channel at this time." );
   else 
   {
...
void judge_broadcast(char_data *ch, char * fmt, ...)
{
 	...
 	sprintf(buf2, "`=\x8a[CRIME: `=\x8b%s`=\x8a]`x", buf);
 	...
 	for( wch = player_list; wch; wch = wch->next_player )
     {
 		if(IS_IMMORTAL(ch) && !IS_IMMORTAL(wch)){
 		 ...
 		 if( (IS_SET(wch->territory1, TERRITORY_GUARD) || IS_IMMORTAL(wch))
...
void highlander_broadcast(char_data *ch, char * fmt, ...)
{
	...
	sprintf(buf2, "`=\x8a[HIGHLANDER: `=\x8b%s`=\x8a]`x", buf);
