...
void do_teach(char_data *ch, char * argument)
{
	char arg1[MIL];
	char arg2[MIL];
	bool teach_language, teach_spell;
	int sn;
	char_data *victim;
	
	argument = one_argument( argument, arg1 );
	argument = one_argument( argument, arg2 );
	teach_language = false;
	teach_spell = false;

	if(arg1[0]=='\0' || arg2[0]=='\0')
	{
		ch->println( "Syntax: teach <character> <spell>" );
		ch->println( "    Or: teach <character> <language>" );
		return; 
	}
	...
	else
	{
		...
		if(teach_language == false)
		{
			ch->printlnf( "'%s' is not teachable.", arg2 );
			return;
		}
	}
		
	if(IS_NPC(victim)){
		ch->println( "You can't teach NPCs." );
		return;
	}
	...
	if(!IS_IMMORTAL(ch)){
		...
		{
			ch->printlnf( "You do not know '%s'.", skill_table[sn].name);
			return;
		}
	}
	...
	{
		ch->printlnf( "'%s' is not teachable.", skill_table[sn].name);
		return;
	}
	...
	if(IS_SET(skill_table[sn].flags, SKFLAGS_NO_INTERCLASS_TEACH) 
		&& ch->clss!=victim->clss && teach_spell)
	{
	...
	// can't learn  the level the same as the low_percent_level
	if (skill_table[sn].skill_level[ch->clss]
			== skill_table[sn].low_percent_level[ch->clss])
	{
	...
	if((teach_spell && victim->pcdata->learned[sn]>0) 
	|| (teach_language && victim->pcdata->learned[sn]==100))
	{
	...
	else if(teach_language)
	{
			ch->printlnf("You try to teach %s '%s'.", PERS(victim, ch), skill_table[sn].name);
			victim->printlnf("%s tries to teach you '%s'.", PERS(ch, victim), skill_table[sn].name);
		
		int chance;
		//Teach_language is being unofficially REdefined as "Did you manage to teach them?"

		//Language Barrier
		chance = ( get_skill(ch, victim->language->gsn) + get_skill(victim, ch->language->gsn) )/ 2 + 10;
		
		if(teach_language && number_range(1, 10) >= chance )  //Always a chance to fail
		{
			ch->printlnf("But your student did not understand your words.");
			victim->println("But you couldn't understand a word of it.");
			teach_language = false;
		}
		
		//This was the teacher's fault
		chance= ch->modifiers[STAT_PR] + 2*ch->modifiers[STAT_RE];
		
		if(teach_language
			&& number_range(1,200 * (ch->language->difficulty)) > UMIN(chance * 5, 50) )
		{
			ch->println("But you couldn't get your lesson across.");
			victim->println("But you couldn't understand a thing.");
			teach_language = false;
		}	

		//This was the student's fault
		chance = victim->modifiers[STAT_ME] + 2*victim->modifiers[STAT_IN];

		if(teach_language 
			&& number_range(1,200 * (ch->language->difficulty)) > UMIN(chance * 5, 50)  )
		{
			ch->println("But your student just stares at you dumbly.");
			victim->println("But everything just runs together in a mess.");
			teach_language = false;
		}	
		...
		if(teach_language)
		{
			victim->pcdata->learned[sn]= 
				UMIN(100, victim->pcdata->learned[sn] + number_range(1,5) );
			if(victim->pcdata->learned[sn] < 20)
			{
				ch->printlnf("You've helped %s with the basics.", PERS(victim, ch));
				victim->printlnf("%s has helped you with the basics.", PERS(ch, victim));
			}
			else if(victim->pcdata->learned[sn] < 40)
			{
				ch->printlnf("You've improved %s's grammar.", PERS(victim, ch));
				victim->printlnf("%s has improved grammar.", PERS(ch, victim));
			}
			else if(victim->pcdata->learned[sn] < 60)
			{
				ch->printlnf("You've widened %s's vocabulary.", PERS(victim, ch));
				victim->printlnf("%s has added to your vocabulary.", PERS(ch, victim));
			}
			else if(victim->pcdata->learned[sn] < 80)
			{
				ch->printlnf("You've outlined sentence structure to %s.", PERS(victim, ch));
				victim->printlnf("%s has outlined sentence structure to you.", PERS(ch, victim));

			}
			else if(victim->pcdata->learned[sn] <= 100)
			{
				ch->printlnf("You've honed %s's accent.", PERS(victim, ch));
				victim->printlnf("%s has honed your accent.", PERS(ch, victim));
			}
			else
			{
				ch->printlnf("You've perfected %s's linguistics!", PERS(victim, ch));
				victim->printlnf("%s has helped you perfect your linguistics!", PERS(ch, victim));
...
/****************************************************************************/
void do_oldteach(char_data *ch, char * argument)
{
	char arg1[MIL];
	char arg2[MIL];
	int sn;
...
//      check_realm_improve(ch,sn,false); // taken out due to cheaters
    ...
    // can't learn a spell with the level the same as the low_percent_level
    if (skill_table[sn].skill_level[ch->clss] == skill_table[sn].low_percent_level[ch->clss])
	{
		ch->wrapln( "After you started the lesson, you realized your student is unable to grasp the "
			"concepts to learn this spell, and it is very apparent they never will." );
