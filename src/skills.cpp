...
void do_vault( char_data *ch, char *argument )
{
		...
		&&       ch->level < skill_table[gsn_vault].skill_level[ch->clss]))
	{
		if (!IS_CONTROLLED(ch))
		{
			ch->println("Vaulting? What's that?");
			return;
		}
	}
	...
	{
			ch->println("You cannot vault without a staff!");
			return;
	}
    ...
    {	
		...
		if((get_skill(ch, gsn_vault)/10) < number_range(1,8))
		{
		...
		victim->position = POS_RESTING;
		damage(ch,victim,number_range(2,2 + ((get_skill(ch, gsn_vault)+25)/50) * ch->size + chance/10),
