...
void do_hallucinate(char_data *); // Ixliam
...
void affects_update( void )
{
	...
	for ( ch = char_list; ch; ch = ch->next )
	{
		...
		{ 
 		...
 		if (IS_AFFECTED2(ch, AFF2_HALLUCINATE) && (number_range(1,5)==1))
 		{ 
 			do_hallucinate(ch);
 		} 
		...
		if (ch->ridden_by)
		{
			if (ch->ridden_by->in_room!=ch->in_room)
			{
				ch->println("Your rider has inexplicably disappeared.");
				dismount(ch);
				if(IS_NPC(ch->ridden_by))
					bug("BUG affects_update(): Mount in different room than rider.");
			}
		}
...
/***************************************************************************/
/**************************************************************************/
void do_hallucinate(char_data *ch)
{
	if IS_OOC(ch)
		return;
	...
	while (IS_SET(figmentmob->act, ACT_IS_UNSEEN)) 
	{	
		figmentmob = rand_figment_mob( ch );
	}
         ...
         if ( !fig )
         {
          switch( dice( 1, 6 ) )
          {
           case 1:
            ...
            {
             case 1:
		act("$N says, 'Hey, what are you doing here?'",ch, NULL, figment, TO_CHAR );
           	break;
             case 2: 
		act("$N says, 'You stole my pink dragon!'",ch, NULL, figment, TO_CHAR );           
		break;
             case 3: 
		act("$N says, 'We could group and go kill that dragon!'",ch, NULL, figment, TO_CHAR );           
		break;
             case 4: 
		act("$N says, 'Feudalism was just a red herring.'",ch, NULL, figment, TO_CHAR );            
		break;
             case 5: 
		act("$N says, 'I'm late, I'm late, for a very important date!'",ch, NULL, figment, TO_CHAR );            
		break;
             case 6: 
		act("$N says, 'How about them paladins ?'", ch, NULL, figment, TO_CHAR );            
		break;
            }   
           break;
           case 2:
		...
		break;
           ...
           case 5:
            switch( dice( 1, 3 ) )
            {
             case 1: sprintf( figbuf, "%s `#`Gmisses`^ you! (0)",
	      figment->short_descr );
		break;
             case 2: sprintf( figbuf, "%s's smash `#`Rscratches`^ you. (2)",
	      figment->short_descr );
		break;
             case 3: sprintf( figbuf, "%s's backstab does `#`RUNSPEAKABLE`^ things to you. (367891)",
	      figment->short_descr );
		break;
           ...
           case 6:
            if ( number_percent() > 75 )
             ch->print(  "From a great distance, ");
            switch( dice( 1, 4 ) )
            {
		...
		break;
           ...
           case 5:
            switch( dice( 1, 2 ) )
            {
             case 1: sprintf( figbuf, "%s `#`Gmisses `^you!",
	      figmentmob->short_descr );
		break;
             case 2: sprintf( figbuf, "%s's smash `#`RMASSACRES `^you.",
	      figmentmob->short_descr );
		break;
            }
            ch->println( figbuf); break;
