...
bool hotreboot_GEL_character(char *argument)
{
	...
	if(!load_char_obj( c, charname )){
		c->write("`#`YFor some reason your pfile has disappeared in the hotreboot... sorry, try logging on again.\r\n",0);
		connection_close(c);
	};
...
void hotreboot_finalise()
{
	...
	for ( ch = player_list; ch; ch = ch->next_player )
	{
	    ...
	    {
			ch->println("`R==================================`x");
			ch->println("`R=======`Y HOTREBOOT COMPLETE `R=======`x");
			ch->println("`R==================================`x");
			ch->println("`x");
			do_look (ch, "auto");
		}else{
...
void do_hotreboot(char_data* ch, char * argument)
{
	...
	for (connection_data *d=connection_list; d; d=d->next){
		if(d->editor){
			...
			CH(d)->wrapln("`R=====================================`x");
			CH(d)->wrapln("`R=======`Y HOTREBOOT IN PROGRESS `R=======`x");
			CH(d)->wrapln("`R=====================================`x");
		}
	}
#endif
