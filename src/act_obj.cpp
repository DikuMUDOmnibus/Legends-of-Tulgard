...
int find_door args( ( char_data *ch, char *arg, char *action) );
...
DECLARE_OSPEC_FUN(	ospec_gnomish_lockpick);
#undef OD
#undef CD
...
void get_obj( char_data *ch, OBJ_DATA *obj, OBJ_DATA *container)
{
   ...
   // Clan Gear Items - Ixliam 
   if (!IS_IMMORTAL(ch) && !IS_NPC(ch) && (obj->pIndexData->clan != NULL))
   {
		if (obj->pIndexData->clan != ch->clan)
		{
			ch->println("That belongs to another clan! Item removed.");
			extract_obj(obj);
			return;
		}
   }

    if (IS_SET(obj->extra2_flags, OBJEXTRA2_RUNIC) && obj->level > ch->level)
    {
	act( "$n is struck with lightning as they try to get $p.", ch, obj, NULL, TO_ROOM );
	ch->printf( "You are struck by lightning when you touch %s.\r\n", obj->short_descr );
	ch->printf( "%s is to powerful for you to handle.\r\n", obj->short_descr );
	return;
    }

	//ospec_blessed_chalice
    if (obj->ospec_fun == ospec_lookup("ospec_blessed_chalice") 
    ...
    if ( container != NULL ){
		if(container->pIndexData->vnum == OBJ_VNUM_PIT
			...
			obj->timer = 0;
	if(IS_SET(ch->dyn2, DYN2_CHICANERY) && get_skill(ch, gsn_chicanery) > number_percent())
	{
        act( "You secretly get $p from $P.", ch, obj, container, TO_CHAR );
		// hide them to mortals
		int old_invis_level = ch->invis_level;
		ch->invis_level = UMAX(ch->invis_level, LEVEL_IMMORTAL);
        act( "$n gets $p from $P.[CHICANERY]", ch, obj, container, TO_ROOM );
		ch->invis_level=old_invis_level;
		check_improve(ch,gsn_chicanery,true,10);
	}
	else
	{
    ...
    }else
	{
		if(IS_SET(ch->dyn2, DYN2_CHICANERY) && get_skill(ch, gsn_chicanery) > number_percent())
		{
			act( "You secretly get $p.", ch, obj, container, TO_CHAR );
			// hide them to mortals
			int old_invis_level = ch->invis_level;
			ch->invis_level = UMAX(ch->invis_level, LEVEL_IMMORTAL);
			act( "$n gets $p.[CHICANERY]", ch, obj, container, TO_ROOM );
			ch->invis_level=old_invis_level;
			check_improve(ch,gsn_chicanery,true,10);
		}
		else
		...
		if(!IS_NPC(ch)){
				...
				&& !strcmp(obj->lastdrop_remote_ip,ch->remote_ip_copy))
			{
				multilog_alertf(ch, "`Y%s<%d>`x getting `Y%d gold `xand `S%d silver `xfrom somewhere "
					...
					ch->printlnf("#`YExchanging money between your PC's or someone from your site is illegal!!!`^");
					ch->printlnf("#`RAll your money is forfeit!!!`^");
					ch->gold = 0;
					ch->silver = 0;
					ch->bank = 0;
					extract_obj( obj );
		...
		if(!IS_NPC(ch)){
				...
				!strcmp(obj->lastdrop_remote_ip,ch->remote_ip_copy))
			{
				...
				multilog_alertf(ch, 
					...
					char mbuf [MSL];
					sprintf(mbuf, "`#`Y%s has been removed for being a multilog item. `RDO NOT MULTI ITEMS!!!`^", obj->short_descr);
					ch->printlnf(mbuf);
					extract_obj( obj );
					return;
			}
...
void donate(char_data *ch, OBJ_DATA *obj)
{
	...
	if (obj->level < 11){
		...
		if (obj->pIndexData->vnum == OBJ_VNUM_RANDOM_OBJ){
			room_vnum=3064;
		}else if (obj->item_type == ITEM_ARMOR){
...
void do_get( char_data *ch, char *argument)
{
    ...
    if( IS_NULLSTR(arg2) )
	{
		...
		else
		{
		   if(!IS_IMMORTAL(ch))
		   {
			if(ch->in_room->vnum==ROOM_VNUM_NEWBIEWEAPON_DONATE || ch->in_room->vnum==ROOM_VNUM_NEWBIEARMOR_DONATE ||
			   ch->in_room->vnum==ROOM_VNUM_NEWBIEMISC_DONATE || ch->in_room->vnum==ROOM_VNUM_WEAPON_DONATE ||
			   ch->in_room->vnum==ROOM_VNUM_ARMOR_DONATE || ch->in_room->vnum==ROOM_VNUM_MISC_DONATE)
			{
				ch->println("Don't be so greedy. You cannot use 'get all' here.");
				return;
			}
  		   }
...
void do_put( char_data *ch, char *argument )
{
	...
	case ITEM_FOUNTAIN:
			...
			if(ch->gold == 0)
			{
				ch->println("You do not have a gold coin to flip into anything.");
				return;
			}
			ch->gold -=1;
			act( "$n tosses a `#`Ygold coin`^ into $P.", ch, NULL, container, TO_ROOM );
			act( "You make a wish and toss a `#`Ygold coin`^ into $P.", ch, NULL, container, TO_CHAR );
		} else {
			ch->printlnf( "%s is not a container.", container->short_descr);
			if(IS_NEWBIE(ch))
			{
   ...
   if (!IS_IMMORTAL(ch) && !IS_NPC(ch) && (obj->pIndexData->clan != NULL))
   {
    ...
    if (IS_SET(obj->extra2_flags, OBJEXTRA2_RUNIC) && obj->level > ch->level)
    {
	act( "$n is shocked as they try to put $p away.", ch, obj, NULL, TO_ROOM );
	ch->printf( "%s is to powerful for you to handle.\r\n", obj->short_descr );
	return;
    }
		...
		if (IS_SET(container->value[1],CONT_PUT_ON))
		{
				if(IS_SET(ch->dyn2, DYN2_CHICANERY) && get_skill(ch, gsn_chicanery) > number_percent())
				{
					act( "You secretly put $p on $P.", ch, obj, container, TO_CHAR );
								// hide them to mortals
					int old_invis_level = ch->invis_level;
					ch->invis_level = UMAX(ch->invis_level, LEVEL_IMMORTAL);
					act( "$n puts $p on $P.[CHICANERY]", ch, obj, container, TO_ROOM );
					ch->invis_level=old_invis_level;
					check_improve(ch,gsn_chicanery,true,10);
				}
				else
		...
		else
		{
				if(IS_SET(ch->dyn2, DYN2_CHICANERY) && get_skill(ch, gsn_chicanery) > number_percent())
				{
					act( "You secretly put $p in $P.", ch, obj, container, TO_CHAR );
					// hide them to mortals
					int old_invis_level = ch->invis_level;
					ch->invis_level = UMAX(ch->invis_level, LEVEL_IMMORTAL);
					act( "$n puts $p on $P.[CHICANERY]", ch, obj, container, TO_ROOM );
...
void do_drop( char_data *ch, char *argument)
{
	...
	if ( is_number( arg ) )
    {
		...
		obj_to_room( obj, ch->in_room );
				if(IS_SET(ch->dyn2, DYN2_CHICANERY) && get_skill(ch, gsn_chicanery) > number_percent())
				{
					act( "You secretly drop some coins.", ch, NULL, NULL, TO_CHAR );
					// hide them to mortals
					int old_invis_level = ch->invis_level;
					ch->invis_level = UMAX(ch->invis_level, LEVEL_IMMORTAL);
					act( "$n drops some coins.[CHICANERY]", ch, NULL, NULL, TO_ROOM );
					ch->invis_level=old_invis_level;
					check_improve(ch,gsn_chicanery,true,10);
				}
				else
    ...
    if ( str_cmp( arg, "all" ) && str_prefix( "all.", arg ) )
    {
		...
		if ( IS_SET(ch->in_room->room2_flags, ROOM2_PC_SHOP_STORAGE) &&
		     !IS_SET(obj->extra2_flags, OBJEXTRA2_PC_CRAFTED))
		{
			ch->println( "You cannot place that item here to sell." );
			return;
		}
		...
		if(IS_SET(ch->dyn2, DYN2_CHICANERY) && get_skill(ch, gsn_chicanery) > number_percent())
		{
			act( "You secretly drop $p.", ch, obj, NULL, TO_CHAR );
			// hide them to mortals
			int old_invis_level = ch->invis_level;
			ch->invis_level = UMAX(ch->invis_level, LEVEL_IMMORTAL);
			act( "$n drops $p.[CHICANERY]", ch, obj, NULL, TO_ROOM );
			ch->invis_level=old_invis_level;
			check_improve(ch,gsn_chicanery,true,10);
		}
		else
	...
	if ( IS_SET(ch->in_room->room2_flags, ROOM2_PC_SHOP_STORAGE))
	{
		ch->println( "You must drop them individually." );
		return;
	}
	for ( obj = ch->carrying; obj != NULL; obj = obj_next )
	{
...
void do_give_new( char_data *ch, char *argument, int silent)
{
   ...
   if (!IS_IMMORTAL(ch) && !IS_NPC(ch) && (obj->pIndexData->clan != NULL))
   {
		...
		if (obj->pIndexData->clan != victim->clan)
		{
			ch->println("That belongs to another clan! Item removed.");
			extract_obj(obj);
			return;
    ...
    if (IS_SET(obj->extra2_flags, OBJEXTRA2_RUNIC) && obj->level > victim->level)
    {
	act( "$n is struck with lightning as they try to get $p.", victim, obj, NULL, TO_ROOM );
	victim->printf( "You are struck by lightning when you touch %s.\r\n", obj->short_descr );
	victim->printf( "%s is to powerful for you to handle.\r\n", obj->short_descr );
	obj_from_char( obj );
	obj_to_room(obj,ch->in_room);
	return;
    }
    ...
    else
    {
        ...
        MOBtrigger = true;
		if(obj->ospec_fun == ospec_lookup("ospec_thought_sharer")
		&& !IS_NPC(ch)
		&& !IS_NPC(victim))
		{
			act("$p glows as it is passed from $n to $N", ch, obj, victim, TO_NOTVICT);
			act( "As $n gives you $p it glows for a moment.",   ch, obj, victim, TO_VICT    );
			act( "As you give $p to $N it glows for a moment.", ch, obj, victim, TO_CHAR    );
			ospec_thought_sharer(obj, ch);
		}
	}
...
*/
   ...
   if (!IS_IMMORTAL(ch) && !IS_NPC(ch) && (obj->pIndexData->clan != NULL))
   {
    ...
    if (IS_SET(obj->extra2_flags, OBJEXTRA2_RUNIC) && obj->level > ch->level)
    {
	act( "$n is struck with lightning as they try to grab $p.", ch, obj, NULL, TO_ROOM );
	ch->printf( "You are struck by lightning when you touch %s.\r\n", obj->short_descr );
	ch->printf( "%s is to powerful for you to handle.\r\n", obj->short_descr );
	return;
    }
...
void do_drink( char_data *ch, char *argument )
{
	...
	case ITEM_DRINK_CON:
		...
		amount = UMIN(amount, (obj->value[1] < 0 ? amount : obj->value[1]));
		break;
	}
	
    if ( (strstr(liq_table[liquid].liq_name, "blood")) && IS_VAMPIRE(ch))
	{
	...
	{
		ch->println( "You can only drink blood." );
		return;
	}
...
void do_eat( char_data *ch, char *argument )
{
    ...
    if ( ch->level<LEVEL_IMMORTAL )
    {
        ...
        if ( (ch->race == race_lookup("thri-kreen") || 
              ch->race == race_lookup("dragon") ||
              ch->race == race_lookup("beholder") ||
	      IS_SET(ch->wish, WISH_EATALL) ||
	      ch->race == race_lookup("illithid")) && 
              obj->item_type != ITEM_FOOD && 
              obj->item_type != ITEM_CORPSE_NPC && 
              obj->item_type != ITEM_PILL )
        {
            ch->printlnf( "That's not edible." );
            return;
        } 
	...
	if ( ch->race != race_lookup("thri-kreen") && 
	     ...
	     ch->race != race_lookup("illithid") && 
	     !IS_SET(ch->wish, WISH_EATALL) &&
              obj->item_type != ITEM_FOOD && obj->item_type != ITEM_PILL )
        {
    ...
    if (ch->race == race_lookup("illithid") && obj->item_type == ITEM_CORPSE_NPC)
    {
    	act( "$n tentacles reach down and suck the brain from $p.", ch, obj, NULL, TO_ROOM );
    	act( "You suck the brain from $p.", ch, obj, NULL, TO_CHAR );
    }
    else	
    {	
    ...
    case ITEM_CORPSE_NPC:
		if ( !IS_NPC(ch) && 
                   (ch->race == race_lookup("thri-kreen") ||
      		    ch->race == race_lookup("dragon") ||
      		    ch->race == race_lookup("beholder") ||
	            ch->race == race_lookup("illithid") ))
		{
			...
			if ( condition == 0 && ch->pcdata->condition[COND_HUNGER] > 0 )
				ch->printlnf( "You are no longer hungry." );
			else if ( ch->pcdata->condition[COND_FULL] > 40 )
				ch->printlnf( "You are full." );
		}
		break;
    ...
    case ITEM_FOOD:
		if ( !IS_NPC(ch) )
		{
			...
			{
				gain_condition(ch, COND_HUNGER, (obj->value[0] / 2));
				gain_condition(ch, COND_HUNGER, (obj->value[1] / 2));
				ch->pcdata->tired=UMAX(ch->pcdata->tired - number_range(1, 3), 0);
				ch->move+=number_range(1, obj->level/2);
			}
			if ( IS_SET(obj->value[3], FOOD_JUICY))
			{
...
bool remove_obj( char_data *ch, int iWear, bool fReplace )
{
   ...
   if (!IS_IMMORTAL(ch) && !IS_NPC(ch) && (obj->pIndexData->clan != NULL))
   {
    ...
    if (IS_SET(obj->extra2_flags, OBJEXTRA2_RUNIC) && obj->level > ch->level)
    {
	ch->printf( "%s is too powerful for you to handle.\r\n", obj->short_descr );
	obj_from_char( obj );
	obj_to_char(obj, ch);
	return true;
    }
...
void wear_obj( char_data *ch, OBJ_DATA *obj, bool fReplace, bool hold )
{
   ...
   if (!IS_IMMORTAL(ch) && !IS_NPC(ch) && (obj->pIndexData->clan != NULL))
   {
	...
	{
		act("$p cannot be worn on your body.",ch,obj,NULL,TO_CHAR); 
		return;
	} 
...
void do_remove( char_data *ch, char *argument )
{
        ...
        for ( obj = ch->carrying; obj != NULL; obj = obj->next_content )
        {
				...
				&&   can_see_obj( ch, obj ) )
            {
				...
				||	 obj->wear_loc == WEAR_CONCEALED )
				{
					...
					if ( obj->wear_loc == WEAR_CONCEALED)
						REMOVE_CONFIG2(ch, CONFIG2_CONCEALED);
   ...
   if (!IS_IMMORTAL(ch) && !IS_NPC(ch) && (obj->pIndexData->clan != NULL))
   {
		...
		if ( get_eq_char(ch, WEAR_SHEATHED) != NULL && obj->wear_loc == WEAR_WIELD)
			{ SET_CONFIG2(ch, CONFIG2_SHEATHED); }
...
void do_quaff( char_data *ch, char *argument )
{
	...
	{
		act( "$n quaffs $p and tries to `#`YCHEAT`^ in the `RARENA`^.", ch, obj, NULL, TO_ROOM );
		act( "You quaff $p and try to `#`YCHEAT`^ in the `RARENA`^.", ch, obj, NULL ,TO_CHAR );
	    	info_broadcast(ch, "%s tried to use Potions in the Arena.", ch->name);
		ch->hit = 1;
		extract_obj( obj );
		return;
	}
...
void do_steal( char_data *ch, char *argument )
{
    ...
    if(IS_SET(ch->in_room->room2_flags,ROOM2_NOSTEAL))
    {
        ch->println( "You'd better not -- you might get caught." );
        return;
    }
	...
	if(!IS_NPC(ch) && !IS_NPC(victim)){
 	...
 	if(IS_SET(ch->affected_by2, AFF2_BOUNTYHUNTER)) {
 		ch->println("`#`YYour bounty hunter license is revoked for stealing!!!`^");
		...
		|| (ch->level<=LEVEL_HERO && victim->level>LEVEL_HERO))
    {
        ...
        if ( !IS_NPC(ch) )
        {
 		...
 		if (IS_SET(victim->act2, ACT2_CRIME))
 			ch->pcdata->theft += 1;
   ...
   if (!IS_IMMORTAL(ch) && !IS_NPC(ch) && (obj->pIndexData->clan != NULL))
   {
		if (obj->pIndexData->clan != ch->clan)
		{
			ch->println("That belongs to another clan!");
			return;
		} 
   }
...
void obj_to_keeper( OBJ_DATA *obj, char_data *ch )
{
    ...
    for (t_obj = ch->carrying; t_obj != NULL; t_obj = t_obj_next)
    {
		...
		{
			if(!str_cmp(obj->short_descr,t_obj->short_descr))
			{
			...
			duplicate = true;
			if(!IS_OBJ_STAT(t_obj,OBJEXTRA_INVENTORY)
			&& IS_OBJ_STAT(obj, OBJEXTRA_INVENTORY))
				inventory = false;
		}
    }
	if(!duplicate && inventory)
		REMOVE_BIT( obj->extra_flags, OBJEXTRA_INVENTORY);
...
/**************************************************************************/
void buy_in_playershop( char_data *ch, char *argument )
{
	int cost;
	char arg[MIL];
	char buf[MSL];
	...
	// player shop storage room is the room which vnum 
	// is one greater than the current room
	pRoomIndexNext = get_room_index( ch->in_room->vnum + 1 );
	
	if ( pRoomIndexNext == NULL )
	{
		bugf( "do_buy(): bad player shop at vnum %d.", ch->in_room->vnum );
		ch->println("Sorry, you can't buy that here.");
		return;
	}
	...
	money=create_object(get_obj_index(1808));
	money->value[0] = cost;
	obj_to_room(money, pRoomIndexNext);
	return;
}
/************************************************************************/
/**************************************************************************/
...
void do_buy( char_data *ch, char *argument )
{
	...
	// player shops
	if( IS_SET(ch->in_room->room2_flags, ROOM2_PLAYER_SHOP))
	{
		buy_in_playershop(ch, argument);
		return;
    	}

	// locker code
	if(!str_prefix("treasury", argument)){
		do_lockers(ch, FORMATF("%s startrent", argument+8));
		return;
	}
...
void do_list( char_data *ch, char *argument )
{
	...
	if(str_len(argument)>3 && !str_prefix(argument, "treasury")){
		do_lockers(ch, "list");
		return;
	}
	...
	ROOM_INDEX_DATA *in_room;
        ...
        if (pRoomIndexNext == NULL )
        {
            bugf( "do_list(): bad player shop at vnum %d.", ch->in_room->vnum );
            ch->println("You can't do that here.");
            return;
        }
	...
	for ( obj = ch->in_room->contents; obj; obj = obj_next ) // Look For Forge
	{
		...
		cost = obj->cost;
            	...
            	if ( obj->wear_loc == WEAR_NONE &&   can_see_obj( ch, obj )&& cost>0 )
            	{
			// do the filtering
			if (doingFiltering)
			{
...
void do_throw( char_data *ch, char *argument )
{
	...
	if( IS_SET(ch->in_room->room2_flags, ROOM2_PLAYER_SHOP))
	{
		ch->println("You can't throw objects from here.");
		return;
	}
...
void do_zap( char_data *ch, char *argument )
{
    ...
    if (( wand->value[2] > 0 ) || (wand->value[2] = -1))
    {
		...
		if(victim && IS_SET(victim->config2, CONFIG2_BEHOLDER_EYE))
		{
			act("Your anti-magic ray prevents $n's magic.", ch, NULL, victim, TO_VICT);
			act("$N's anti-magic ray prevents $n magic.", ch, NULL, victim, TO_NOTVICT);
			act("$N anti-magic ray prevents your power.", ch, NULL, victim, TO_CHAR);
			return;
		}
...
//shouldn't check improve if all they are doing is dropping it
    ...
    if (IS_SET(obj->extra2_flags, OBJEXTRA2_RUNIC) && obj->level > victim->level)
    {
	ch->printf( "%s is to powerful for them to handle.\r\n", obj->short_descr );
	return;
    }
...
{
    static char *headers[] = { "the corpse of ", "corpse of ",
    ...
    if (arg[0] == '\0')
    {
	    ch->println( "`BSyntax: `xdecapitate corpse" );
	    return;
    }
    ...
    return;
    head  = create_object( get_obj_index( OBJ_VNUM_SEVERED_HEAD ));
    ...
    for (i = 0; i < 3; i++)
    {
          ...
          sprintf( buf, "head %s", obj->short_descr+len );
          free_string( head->name );
          head->name = str_dup(buf);

          sprintf( buf, "The severed head of %s is lying here.  ",
             obj->short_descr+len );
          free_string( head->description );
          head->description = str_dup( buf );

          sprintf( buf, "the severed head of %s", obj->short_descr+len );
    ...
    act( "$n decapitates $p.", ch, obj, NULL, TO_ROOM );
    act( "You decapitate $p.", ch, obj, NULL, TO_CHAR );
...
/**************************************************************************/
// wear object as a third weapon 
void do_triwield (char_data *ch, char *argument)
{
	OBJ_DATA *obj;
	...
	if ( !IS_NPC(ch) && ch->get_skill(gsn_triwield)<1){
		ch->println("You dont have a clue how to hold a weapon in your third hand correctly.");
		return;
	}

	if (IS_NULLSTR(argument)){
		ch->printlnf( "Wear which weapon in your thrid-hand?" );
		return;
	}
	...
	if (obj == NULL)
	{
		ch->printlnf( "You have no such thing in your backpack." );
		return;
	}
	...
	if(obj->item_type != ITEM_WEAPON)
	{
		ch->printlnf( "That is not a weapon." );
		return; }
	
	if(!CAN_WEAR( obj, OBJWEAR_WIELD ))
	{
        ch->printlnf( "You can not wield that." );
        return;
	}

	if ( IS_SET( obj->extra2_flags, OBJEXTRA2_NOSECONDARY ))
	{
		act( "You cannot wield $p in your third hand.", ch, obj, NULL, TO_CHAR );
		return;
	}
	...
	if (get_eq_char (ch, WEAR_WIELD) == NULL) 
	{
		ch->printlnf( "You need to wield a primary weapon, before using a third one!" );
		return;
	}
	
	// at last - the char uses the weapon 
	if (!remove_obj(ch, WEAR_2WIELD, true)) // remove the current weapon if any 
		return;                                // remove obj tells about any no_remove 
	
	act ("$n wields $p in $s third hand.",ch,obj,NULL,TO_ROOM);
	act ("You wield $p in your third hand.",ch,obj,NULL,TO_CHAR);
	equip_char ( ch, obj, WEAR_2WIELD);
	return;
}

/**************************************************************************/
// wear object as a quad weapon 
void do_quadwield (char_data *ch, char *argument)
{
	OBJ_DATA *obj;
	...
	if ( !IS_NPC(ch) && ch->get_skill(gsn_quadwield)<1){
		ch->println("You dont have a clue how to hold a weapon in your fourth hand correctly.");
		return;
	}

	if (IS_NULLSTR(argument)){
		ch->printlnf( "Wear which weapon in your fourth hand?" );
		return;
	}
	...
	if ( IS_SET( obj->extra2_flags, OBJEXTRA2_NOSECONDARY ))
	{
		act( "You cannot wield $p in your fourth hand.", ch, obj, NULL, TO_CHAR );
		return;
	}
	...
	// check that the character is using a first weapon at all 
	if (get_eq_char (ch, WEAR_2WIELD) == NULL) 
	{
		ch->printlnf( "You need to wield a weapon in your third hand, before using a fourth one!" );
		return;
	}
	...
	if (get_eq_char (ch, WEAR_WIELD) == NULL) 
	{
		ch->printlnf( "You need to wield a primary weapon, before using a fourth one!" );
		return;
	}
	...
	if ( get_obj_weight( obj ) > ( (ch->modifiers[STAT_ST] + 15)*5) )
	{
		ch->printlnf( "This weapon is too heavy to be used as a secondary weapon by you." );
		return;
	}
	
	if (IS_WEAPON_STAT(get_eq_char(ch,WEAR_2WIELD),WEAPON_TWO_HANDS)
		&& ch->size < SIZE_LARGE)
	{
		ch->printlnf( "You cannot wear a weapon in your off hand if using a two handed weapon." );
		return;
	}
	

	if ( get_eq_char(ch,WEAR_2HOLD) != NULL)
	{		
		if ((!remove_obj(ch, WEAR_2HOLD, true))){
	// ch->printlnf( "You cannot use a fourth weapon while using a holding an item in that hand." );
			return;
		}
	}
	...
	// at last - the char uses the weapon 
	if (!remove_obj(ch, WEAR_2SECONDARY, true)) // remove the current weapon if any 
		return;                                // remove obj tells about any no_remove 
	
	if (IS_WEAPON_STAT(obj,WEAPON_TWO_HANDS))
	{
		ch->printlnf( "You can't use a two handed weapon in your fourth hand." );
		return;
	}
	
	act ("$n wields $p in $s fourth hand.",ch,obj,NULL,TO_ROOM);
	act ("You wield $p in your fourth hand.",ch,obj,NULL,TO_CHAR);
	equip_char ( ch, obj, WEAR_2SECONDARY);
	return;
}

/**************************************************************************/
/* Use command by Ixliam - Oct 2003 
// Originally from my old mud Four Kingdoms
// Pulled from my Godsword Code
*/

#define OBJ_VNUM_SERPENT_BRACELET 	5097
#define OBJ_VNUM_HORN_OF_BLASTING 	5099
#define OBJ_VNUM_DRAGONORB          	47303
...
void do_use (char_data * ch, char *argument)
{
	...
	char_data *victim;
	char *target_name;
	OBJ_DATA  *wield, *hold, *wrist1, *wrist2;
	...
	if (IS_OOC(ch))
	{
		ch->printlnf( "Not while you in OOC." );
		return;
	}
	...
	if (arg1[0] == '\0') 
	{
		ch->println ("Use which what where?");
		return;
	}
	...
	if (wield != NULL && wield->pIndexData->vnum == 5091 && is_name(arg1, wield->name))
        {
    		if (( victim = get_char_world( ch, arg2 )) == NULL ) 
		{
        		ch->printlnf("You can't seem to find '%s' here.", arg2);
        		return;
    		}
		...
		if (!IS_NPC(ch) && 
		   ( (!IS_SET(ch->in_room->room2_flags,ROOM2_WAR)) ||
		 	...
		 	if(!IS_ACTIVE(ch) )
  			{
				ch->println( "You are not active." );
				return;
    			}
		}

        	if (!IS_NPC(victim) && !IS_OOC(victim) && !IS_OOC(ch) )
		{
         		ch->println("You swing `#`SFarslayer`^ about and it flies from your hands!");

		    	connection_data *d;

         		act("$n swings the sword `#`SFarslayer`^ about and lets it fly!",ch,wield,NULL,TO_ROOM);

    			for ( d = connection_list; d; d = d->next )
    			{
				if ( d->connected_state == CON_PLAYING )
				{
					d->character->printlnf( "`#%s `Ycries, `G'For thy heart, for thy heart, who hast wronged me, `^%s`G!'`^", ch->name, victim->name );
					d->character->printlnf( "`#`YA howl fills the air as a sword followed by a `Rr`Ya`Ci`Wn`Cb`Yo`Rw`Y flies through the air!`^");
				}
			}
	 		...
	 		obj_to_room(wield, victim->in_room );
         		...
         		act("The sword `#`SFarslayer`^ slams through $n's chest, piercing $s heart!",victim,wield,NULL,TO_ROOM);
		 	victim->printlnf("`YA sword slams into your chest, piercing your heart!`x");

	 		if (IS_IMMORTAL (victim))
	 			return;

    			act( "`#`SFarslayer `^kills $M in cold blood!",  ch, NULL, victim, TO_CHAR    );
			victim->println( "You have been `#`RKILLED!!!`^");
			act( "$n `#`Rhas been KILLED!!!`^",  ch, NULL, victim, TO_NOTVICT );
    			raw_kill( victim, ch );
	 		return;
        	}
	...
	if (hold != NULL && hold->item_type == ITEM_RUNE && is_name(arg1, hold->name))
        {
                ROOM_INDEX_DATA *start = NULL;
                ROOM_INDEX_DATA *end = NULL;
	...
	end = get_room_index(hold->value[2]);

        if ( ch->in_room->area->world->name != end->area->world->name )
        {
                ch->println("That location is not on this world.");
                return;
        }
		
		act("$n invokes the power of a runestone to travel.",ch, NULL, NULL, TO_ROOM);
		act("A swirl of `#`Cm`Wys`Sti`Wca`Cl `Ce`Wn`Ser`Wg`Cy`^ envelopes $n and they vanish.",ch, NULL, NULL, TO_ROOM);
		act("A swirl of `#`Cm`Wys`Sti`Wca`Cl `Ce`Wn`Ser`Wg`Cy`^ envelopes you from the runestone.",ch, NULL, NULL, TO_CHAR);

		char_from_room(ch);
		char_to_room(ch, end);
		hold->value[1] -= 1;
		act( "A swirl of `#`Cm`Wys`Sti`Wca`Cl `Ce`Wn`Ser`Wg`Cy`^ forms and $n appears suddenly.", ch, NULL, NULL, TO_ROOM );
		do_look(ch, "auto");

    		if ( hold->value[1] == 0 ) /* When v1 reaches 0, the rune dissapears */
    		{
			act( "$p explodes in many fragments.", ch, hold, NULL, TO_ROOM );
			act( "$p explodes in many fragments.", ch, hold, NULL, TO_CHAR );
			extract_obj(hold);
    		}
		return;
	}

	if (hold != NULL && is_name(arg1, hold->name) && hold->ospec_fun == ospec_lookup("ospec_gnomish_lockpick"))
	{
		int door;
		if((door = dir_lookup(arg2)) < 0
		|| ch->in_room->exit[door] == NULL){
			ch->println("You'll need to specify a direction for the critter.");
			return;
		}
		if(!IS_SET(ch->in_room->exit[door]->exit_info, EX_CLOSED)
		|| IS_SET(ch->in_room->exit[door]->exit_info, EX_NOLOCK))
		{
			act("$p jumps out of your hand, scurries towards the exit and scurries back.", ch, hold, NULL, TO_CHAR);
			act("$p jumps out of $n's hand and runs around before coming back.", ch, hold, NULL, TO_ROOM);
			return;
		}
		obj_from_char(hold);
		obj_to_room(hold, ch->in_room);
		act("$p scurries out of your hand and towards the door!", ch, hold, NULL, TO_CHAR);
		act("$p scurries out of $n's hand and towards a door!", ch, hold, NULL, TO_ROOM);
		hold->ospec_value[0] = door + 1;
		ospec_gnomish_lockpick(hold, NULL);
	}
	if (hold != NULL && hold->pIndexData->vnum == OBJ_VNUM_DRAGONORB && is_name(arg1, hold->name))
        {
    		char_data *pet;
		char buf[MSL];
		int dragon, dragon_vnum = 0;

		act("You call upon the power of the orb to summon a dragon.", ch, NULL, ch, TO_CHAR);
		act( "$n calls upon the Dragonorb and summons a dragon!", ch, NULL, NULL, TO_ROOM );

		dragon = number_range(1,6);

		if (dragon == 1) // Red Dragon
			dragon_vnum = 5000;
		if (dragon == 2) // Blue Dragon
			dragon_vnum = 5001;
		if (dragon == 3) // Black Dragon
			dragon_vnum = 5002;
		if (dragon == 4) // Green Dragon
			dragon_vnum = 5003;
		if (dragon == 5) // White Dragon
			dragon_vnum = 5004;
		if (dragon == 6) // Shadow Dragon
			dragon_vnum = 5005;

    		pet = create_mobile( get_mob_index(dragon_vnum), 0 );
		pet->comm = COMM_NOTELL|COMM_NOSHOUT|COMM_NOCHANNELS;
		sprintf( buf, "%sA neck tag says 'I belong to %s'.\r\n",
			pet->description, ch->short_descr );
		free_string( pet->description );
		...
		obj_from_char( hold );
		extract_obj(hold);
		act( "A Dragonorb dissolves in a cloud of smoke in the hands of $n!", ch, hold, NULL, TO_ROOM );
		act( "The Dragonorb dissolves in a cloud of smoke.", ch, hold, NULL, TO_CHAR );
		save_char_obj(ch);
		return;
        }

	if (hold != NULL && hold->pIndexData->vnum == OBJ_VNUM_HOLYWATER && is_name(arg1, hold->name))
	{
		int dam = 0;
		if(IS_VAMPIRE(ch))
		{
			...
			extract_obj(hold);
			save_char_obj(ch);
			act( "Smoke pours from the skin of $n as the holy water spills on them!", ch, hold, NULL, TO_ROOM );
			act( "Smoke pours from your skin as the holy water burns you.", ch, hold, NULL, TO_CHAR );
			dam = dice(ch->level/4, ch->level/2)+10;
		...
		if(IS_VAMPIRE(victim))
		{
			...
			extract_obj(hold);
			save_char_obj(ch);
			ch->printlnf("`#`SSmoke `Ypours from the skin of`^ %s `Yas you throw `Wholy water `Yon them!`^", victim->name);
    			act( "`#$n throws `Wholy water`Y on`^ $N `Yand their skin beings to `Ssmoke`Y!`^",  ch, NULL, victim, TO_NOTVICT );
    			act( "`#$n `Yhas `Rb`ru`Rr`rn`Re`rd`Y you with `Wholy water!`^", ch, NULL, victim, TO_VICT);
			dam = dice(ch->level/10, ch->level/3);
		...
		if(!IS_VAMPIRE(victim))
		{
			...
			extract_obj(hold);
			save_char_obj(ch);
			ch->printlnf("`#%s looks very angry `Yas you throw `Wholy water `Yon them!`^", victim->name);
    			act( "`#$n throws `Wholy water`Y on`^ $N! `SBad idea.`^",  ch, NULL, victim, TO_NOTVICT );
    			act( "`#$n `Yhas `Cc`co`Cv`ce`Cr`ce`Cd `Yyou with `Wholy water!`^", ch, NULL, victim, TO_VICT);
			damage(ch,victim,1,0,DAM_NONE,false);
			return;
		}
	}

	if (hold != NULL && hold->pIndexData->vnum == OBJ_VNUM_WOLVESBANE && is_name(arg1, hold->name))
	{
		int dam = 0;
		if(IS_WEREBEAST(ch))
		{
			...
			extract_obj(hold);
			save_char_obj(ch);
			act( "Smoke pours from the skin of $n as they inhale the wolvesbane!", ch, hold, NULL, TO_ROOM );
			act( "Smoke pours from your skin as the wolvesbane burns you.", ch, hold, NULL, TO_CHAR );
			dam = dice(ch->level/4, ch->level/2)+10;
			damage(ch,ch,dam,0,DAM_HARM,false);
			return;
		}
		...
		if(IS_WEREBEAST(victim))
		{
			...
			extract_obj(hold);
			save_char_obj(ch);
			ch->printlnf("`#`SSmoke `Ypours from the skin of`^ %s `Yas you throw `Wwolvesbane `Yon them!`^", victim->name);
    			act( "`#$n throws `Wwolvesbane`Y on`^ $N `Yand their skin beings to `Ssmoke`Y!`^",  ch, NULL, victim, TO_NOTVICT );
    			act( "`#$n `Yhas `Rb`ru`Rr`rn`Re`rd`Y you with `Wwolvesbane!`^", ch, NULL, victim, TO_VICT);

			if( !IS_NPC(victim) && IS_WEREBEAST(victim) && !IS_WEREFORM(victim))
			{
        			if (form_alternate(victim) == true)
    				{	act("$n collapses on the ground in agony and transforms into a Werewolf!", victim,NULL,NULL,TO_ROOM);
					victim->println("Pain shoots through your body as you transform into a Werewolf.");
    					WAIT_STATE( victim, 10 );
	    			}
			}

			dam = dice(ch->level/10, ch->level/3)+10;
		...
		if(!IS_WEREBEAST(victim))
		{
			...
			extract_obj(hold);
			save_char_obj(ch);
			ch->printlnf("`#%s looks very angry `Yas you throw `Wwolvesbane `Yon them!`^", victim->name);
    			act( "`#$n throws `Wwolvesbane`Y on`^ $N! `SBad idea.`^",  ch, NULL, victim, TO_NOTVICT );
    			act( "`#$n `Yhas `Cc`co`Cv`ce`Cr`ce`Cd `Yyou with `Wwolvesbane!`^", ch, NULL, victim, TO_VICT);
			damage(ch,victim,1,0,DAM_NONE,false);
			return;
		}
	}

	if (hold != NULL && hold->pIndexData->vnum == OBJ_VNUM_HORN_OF_BLASTING && is_name(arg1, hold->name))
	{
		...
		OBJ_DATA *token;
    		if (( token = get_obj_token( ch, "5098")) != NULL)
    		{ 
	   		ch->println("The horn has not recharged yet.");
	   		return;
    		}

	 	if (arg2[0] == '\0')
    		{
	        	ch->println("Blast open which door ?");
        		return;
    		}
    		...
    		if ( (door = find_door(ch,arg2, "lock") ) >= 0)
    		{
    			...
    			{
        			ch->println("It's already unlocked, why not just use the knob?");
        			return;
    			}

			if (IS_SET(pexit->exit_info,EX_NOPASS))
    			{
		    		act("$n blows on a large horn, making a loud sound!",ch,0,dir_name[door],TO_ROOM);
    				act("You blow on the horn and nothing happens.",ch,0,dir_name[door],TO_CHAR);
		    		WAIT_STATE(ch,12);
				token = create_object(get_obj_index(5098)); // Horn Token
    				token->timer = 5;
    				obj_to_char( token, ch ); 
				return;
    			}

	    		act("$n `#`Yblows on a large horn the $T door explodes!`^",ch,0,dir_name[door],TO_ROOM);
    			act("You blow on the horn the $T door explodes!`^",ch,0,dir_name[door],TO_CHAR);
    			WAIT_STATE(ch,12);
	...
	ch->println ("You aren't using or wielding that!" );
	return;
}
/************************************************************************/
void do_castout( char_data *ch, char * )
{
   ...
   if (HAS_CONFIG2(ch, CONFIG2_FISHING))
     {
	ch->printlnf("You are already fishing.");
	return;
     }

   if ( get_eq_char( ch, WEAR_HOLD ) == NULL )
   {
	ch->println("Your not even holding a fishing pole, how can you fish?");
	return;
   }
   ...
   if (obj->item_type != ITEM_POLE)
     {
	ch->println("You need to be holding a fishing pole." );
	return;
     }

   if ( (!IS_SET(ch->in_room->room2_flags, ROOM2_SALTWATER_FISH)) && 
	(!IS_SET(ch->in_room->room2_flags, ROOM2_FRESHWATER_FISH)) )
     {
	ch->printlnf("This is not a good place to fish, you need to find a better spot.");
	return;
     }

   fail = number_range(1,10);
   if (fail <= 3)
      {
	ch->printlnf("You try to cast your line out, but it gets all tangled up.");
	ch->printlnf("Try again.");
	act("$n pulls $s arm back, trying to cast $s fishing line out into the water, but gets tangled up instead.",ch,NULL,NULL,TO_ROOM);
	return;
     }

// Now they are fishing, so flag them as fishing

   SET_CONFIG2(ch, CONFIG2_FISHING);
   ch->printlnf("You cast your line out into the water, hoping for a bite.");
   act("$n casts $s line out into the water, hoping to catch some fish.",ch,NULL,NULL,TO_ROOM);
   return;
}

void do_reelin( char_data *ch, char * )
{
   OBJ_DATA *fish;
   int success, fish_num;

   if ( !HAS_CONFIG2(ch, CONFIG2_FISHING)) {
	ch->printlnf("You aren't even fishing!");
	return;
     }
   ...
   if (!HAS_CONFIG2(ch, CONFIG2_FISH_ON )) {
	act("You reel in your line, but alas... nothing on the end.\n\rBetter luck next time.",ch,NULL,NULL,TO_CHAR);
        act("$n reels $s line in, but with nothing on the end.",ch,NULL,NULL,TO_ROOM);
 	REMOVE_CONFIG2(ch, CONFIG2_FISHING);
	return;
     }

   success = number_range(1,10);
   if (success <= 6) {
	ch->printlnf("You reel in your line, putting up a good fight, but you lose him!");
	ch->printlnf("Try again.");
        act("$n reels $s line in, fighting with whatever is on the end, but loses the catch.",ch,NULL,NULL,TO_ROOM);
	REMOVE_CONFIG2(ch, CONFIG2_FISHING);
	REMOVE_CONFIG2(ch, CONFIG2_FISH_ON);
	return;
     }
   ...
   if ( IS_SET(ch->in_room->room2_flags, ROOM2_SALTWATER_FISH))
     {
	fish_num = number_range(52031,52060);
	fish = create_object(get_obj_index( fish_num));
	act("You reel in $p! Nice catch!", ch, fish, NULL, TO_CHAR);
        act("Wow!! $n reels in a helluva catch! Looks like $p!",ch, fish, NULL, TO_ROOM);
        obj_to_char(fish, ch);
	REMOVE_CONFIG2(ch, CONFIG2_FISHING);
	REMOVE_CONFIG2(ch, CONFIG2_FISH_ON);
	return;
     }else
   if ( IS_SET(ch->in_room->room2_flags, ROOM2_FRESHWATER_FISH))
     {
	fish_num = number_range(52000,52030);
	fish = create_object(get_obj_index( fish_num));
	act("You reel in $p! Nice catch!", ch, fish, NULL, TO_CHAR);
        act("Wow!! $n reels in a helluva catch! Looks like $p!",ch, fish, NULL, TO_ROOM);
        obj_to_char(fish, ch);
   ...
   ch->printlnf("ERROR!!! Please report to Ixliam!!!");
   return;
 }

/************************************************************************/
// Shrink Severed Head Code by Kracus
//         Shrink Command used to make severed heads from the death_cry
//         function in fight.c into shrunken heads.
//         You need to have the severed head in your inventory
//         in order to make a shrunken head from it...
// Syntax: shrink head
...
void do_shrink( char_data *ch, char *argument )
{
    ...
    if (arg[0] == '\0')
    {
	    ch->println( "Shrink what severed head ?");
	    return;
    }
    ...
    {
        ch->println( "You are not carrying a severed head to shrink.");
        return;
    }

    // Kracus- its important to keep this next part in the right order
    //         to make sure the act msgs and str_dup have an obj
    if (obj != NULL && obj->pIndexData->vnum == OBJ_VNUM_SEVERED_HEAD)
    {
        act( "`RYou shrink $p, using the `Sdark arts.`x", ch, obj, 0, TO_CHAR );
        act( "`R$n shrinks $p, using the `Sdark arts.`x", ch, obj, NULL, TO_ROOM );
        
        // Kracus- lets now make the shrunken head from a new object
        shrunken_head = create_object(get_obj_index( 2032));
        // Kracus- str_dup the obj to shrunken
        sprintf( buf, "%s's `Rshrunken trophy`x, %s, lies here.", ch->name, obj->short_descr );
        free_string(shrunken_head->description);
        shrunken_head->description = str_dup(buf);
        sprintf( buf, "%s's `Rshrunken trophy`x, %s", ch->name, obj->short_descr );
        free_string(shrunken_head->short_descr);
        shrunken_head->short_descr = str_dup(buf);
        // Kracus- place the new shrunken_head into the char's room
        obj_to_char(shrunken_head, ch);
        // Kracus- lets now get rid of the severed head cause now we got
        //         a shrunken_head with nice descriptions!
        extract_obj(obj);
	    return;
    }
    else
    {
        ch->println( "You can only shrink severed heads.");
        return;
    }
}

/************************************************************************/
// Impale Command by Kracus
//   Impale corpses with no need for a Pike object
...
void do_impale( char_data *ch, char *argument )
{
    ...
    if (arg[0] == '\0')
    {
	    ch->println( "You cannot impale yourself !" );
	    return;
    }
    ...
    // Kracus- its important to keep this next part in the right order
    //         or the act msgs and str_dup will not be correct.
    //         Setting v1 v2 so we can determine if the corpse has
    //         already been impaled so we dont str_dup again...
    if (obj->in_room != NULL)
    {
        if (obj->item_type == ITEM_CORPSE_PC || obj->item_type == ITEM_CORPSE_NPC)
        {
            if (obj->value[1] != 9 &&
                obj->value[2] != 1)
            {
                act( "`RYou impale $p onto a bloodied pike for all to see.`x", ch, obj, 0, TO_CHAR );
                act( "`R$n impales $p onto a bloodied pike for all to see.`x", ch, obj, NULL, TO_ROOM );
                // Kracus- str_dup needs to come after the acts,
                //         so it sends the right obj descr to act.
                sprintf( buf, "`xDripping `Rblood`x, %s lies here `Rimpaled`x by %s.", obj->short_descr, ch->name );
                free_string(obj->description);
                obj->description = str_dup(buf);
                sprintf( buf, "%s impaled upon a pike by %s", obj->short_descr, ch->name );
                free_string(obj->short_descr);
                obj->short_descr = str_dup(buf);
		REMOVE_BIT(obj->wear_flags, OBJWEAR_TAKE);
                obj->value[1] = 9;
                obj->value[2] = 1;
		obj->timer = 10;
                return;
            ...
            {
                ch->println( "That corpse has already been impaled upon a bloodied pike." );
                return;
            }
        }
        else
        {
            ch->println( "You can only impale corpses." );
            return;
        }
    }
}
...
{
    static char *headers[] = { "the corpse of the ", "corpse of the ",
    ...
    if (arg[0] == '\0')
    {
	    ch->println( "`BSyntax: `xmakebag corpse" );
	    return;
    }
	...
	for ( cont = obj->contains; cont != NULL; cont = cont_next )
	{
		...
		ch->println("The corpse is too full to be made into a bag.");
		return;
	}
    }
    ...
    obj->value[0] = obj->level * 10;                 /* Weight capacity */
    obj->value[1] = 1;                          /* Closeable */
    obj->value[2] = 0;                         /* No key needed */
