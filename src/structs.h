...
#define DECLARE_AFTERAFF_FUN( fun )		void fun ( )
...
#define DECLARE_AFTERAFF_FUN( fun )		void fun ( char_data *,	char *);
...
#define DECLARE_SONG_FUN( fun )         SONG_FUN fun (char_data *, void *);
...
#define DECLARE_AFTERAFF_FUN( fun )		DO_FUN		 fun
#endif
#endif
...
typedef struct	altform_data		altform_data;
...
typedef struct	territory_type				TERRITORY_DATA;
...
typedef void AFTERAFF( char_data *ch, void *vo );
typedef bool SONG_FUN( char_data *ch, void *vo);
/**************************************************************************/
// MUDFTP enums
...
struct  pc_data
{
	...
	time_t		saytimes[RPS_AUDIT_SIZE];
 	...
 	// Bard Singing System - Ixliam
    	sh_int		songp;		// Ixliam
    	sh_int		songn;		// Ixliam

 	// Ignore System - Ixliam
	char *		ignore[MAX_IGNORE]; 	// Ixliam
...
struct  area_data
{
    ...
    sh_int		version;
    int			vnum_offset;
    world_type *world;     // world area is in
...
/**************************************************************************/
struct	territory_type
{
	TERRITORY_DATA	*next;
	TERRITORY_DATA	*prev;
	char		*name;		// Territory Name
	char		*description;	// Territory Description
	sh_int		number;		// Territory Number
	char		*mayor;		// Mayor
	sh_int		taxrate;	// Amount of Tax
	sh_int		taxes;		// Amount of Collected Taxes
	vn_int		jailvnum;	// Jail Vnum
	vn_int		stockade;	// Stockade Vnum
	long		crimeflags;	// Crime Flags
};
/**************************************************************************/
...
/**************************************************************************/
//Weapon style for crafting
struct wstyle_type
{
	char		*name;		// Weapon Style Name
	int			subtype;	// Is it just another type of another style?
	int			amount;		// Required amount of material
	int			difficulty; // Reduction to Success Percent
	int			dam_word;	// Damage word
	int			stability;	// modifier(1-10) to first dice #
	int			power;		// modifier(1-10) to second dice #
};
/**************************************************************************/
class verse_data
{
...
struct song_data
{
		...
		sh_int gsn;
		int  verses;
		verse_data *stanza[20];
};
/**************************************************************************/
struct afteraff_type
{
	char		*affect_name;
	DO_FUN		*afteraffect_fun;
};
