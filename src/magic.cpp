...
bool   immune_dispel args(( char_data *victim, int dispel_level) ); 
...
void say_spell( char_data *ch, int sn, CLASS_CAST_TYPE type )
{
	...
	switch (type){
		case CCT_MAXCAST:
		case CCT_NONE:
		sprintf( buf2, "$n takes in a deep breath.");
		sprintf( buf, "$n takes in a deep breath.");
		break;
		...
		case CCT_BARD:
		sprintf( buf2, "$n's eyes glow with power." );
		sprintf( buf,  "$n's eyes glow with power.");
		break;
	}
    ...
    {
		if ( rch != ch ){
			if(IS_AFFECTED(rch, AFF_DETECT_MAGIC))
				act("Mana swirls around $n as $e shapes it.", ch, NULL, rch, TO_VICT);
			if(!IS_SET(skill_table[sn].flags, SKFLAGS_SILENT))
			{	
...
bool check_dispel( int dis_level, char_data *victim, int sn)
{
    ...
    if (count_affected_by_base_spell(victim, sn)>0)
    {
		...
		if(immune_dispel(victim, dis_level) )
			return false; 
...
void do_newcast( char_data *ch, char *argument, int cast_level, CLASS_CAST_TYPE type )
{
    ...
    if(event_active == 7)
    {
	ch->printlnf("The magic around you shifts and warps.");
	int warp = number_range(1,4);
	if(warp == 1)
		mana -= (mana/4);
	if(warp == 2)
		mana = (mana/2);
	...
	*/
	...
	if(IS_SET(ch->config2, CONFIG2_BEHOLDER_EYE))
	{
		ch->println( "Your anti-magic ray prevents you from using your power." );
		return;
	}
	
    say_spell( ch, sn, type );
	if(victim && IS_AFFECTED3(victim, AFF3_DETECT_TAINT))
    {
            switch(number_range(1, 8))
            {
                    case 1: victim->println("A wave of revulsion rushes over you.");
                                    break;
                    case 2: victim->println("Your stomach flips for a moment.");
                                    break;
                    case 3: victim->println("Nausea and dizziness suddenly hit you full force.");
                                    break;
                    case 4: victim->println("For a moment your vision doubles.");
                                    break;
                    case 5: victim->println("Your skin feels like acid has been poured on it.");
                                    break;
                    case 6: victim->println("A rancid smell floats past your nose.");
                                    break;
                    case 7: victim->println("For just a moment, a painful scream hits your ears full force.");
                                    break;
                    case 8: victim->println("A taste like blood, dirt and rotten eggs sits on your tongue.");
                                    break;
            }
    }
	if(!IS_SET(ch->config2, CONFIG2_HOLYCAST))
	...
	if (!IS_SET(TRUE_CH(ch)->config2, CONFIG2_HOLYCAST) &&
        number_percent( ) > ((get_skill(ch,sn) + ch->modifiers[STAT_SD] - 5) / (is_affected(ch, gsn_deafness) ? 2 : 1)))
    {
		ch->mana -= mana / 2;
		if(is_affected(ch, gsn_deafness))
			ch->println("You think you may have mispronounced something...");
		else
			ch->println( "You lost your concentration." );
    ...
    else
    {
		...
		if(get_skill(ch,gsn_occultism))
		{
			if( (roll = number_range(1,100)) < get_skill(ch,gsn_occultism))
			{
				spell_level+=ch->in_room->death_energy * roll / 100;
				check_improve(ch,gsn_occultism, true, 5);
			}
			else
				check_improve(ch,gsn_occultism, false, 5);
		}
...
void do_breath( char_data *ch, char *argument )
{
	...
	EXIT_DATA *pexit;
	int *pdoor;
   int cast_level = ch->level;
    void *vo;
    int mana;
    int sn=gsn_cupcake_breath;
    int target;
    int roll;
    int spell_level;
	int mana_result;
	...
	{ 
		ch->println("Use POWER <breath weapon> <victim>.");
		return;
	}

	
	if(!IS_IMMORTAL(ch) && (!IS_DRAGON(ch) || IS_DRAGONFORM(ch)) && !IS_NPC(ch))
	{
		do_huh(ch, "");
		return;
	}
	...
	if(!IS_ADMIN( ch ) && IS_OOC( ch )){
		ch->println( "You can not use your breath weapon in an ooc area." );
		return;
	}
    ...
    if ( IS_AFFECTED(ch,AFF_CHARM) && !IS_SET( ch->dyn, DYN_IS_BEING_ORDERED ))
	{
		ch->println( "You must wait for your master to tell you to use your breath weapon" );
		return;
	}
	...
	{	
		case DRAGON_RED:
		case DRAGON_GOLD:
			ch->println( "Breathe fire where?");
			break;
		case DRAGON_WHITE:
		case DRAGON_SILVER:
		case DRAGON_CLOUD:
			ch->println( "Breathe frost where?");
			break;
		case DRAGON_COPPER:
		case DRAGON_EMERALD:
		case DRAGON_BLACK:
			ch->println( "Breathe acid where?");
			break;
		case DRAGON_BRASS:
		case DRAGON_GREEN:
			ch->println( "Breathe poison where?");
			break;
		case DRAGON_BRONZE:
		case DRAGON_CRYSTAL:
		case DRAGON_BLUE:
			ch->println( "Breathe lightning where?");
			break;
		case DRAGON_STEEL:
			ch->println( "Breathe a blast of steel where?");
			break;
		case DRAGON_SHADOW:
			ch->println( "Breathe a shadow blast where?");
			break;
		default:
			ch->println(" Breath a cupcake blast where?");
			break;
	}
	return;
    }
	...
	*/
    ...
    case TAR_CHAR_SELF:
        if ( !IS_NULLSTR(arg1)
			...
			&& !is_name( target_name, ch->name ) )
        {
            ch->println( "You cannot breathe this on another." );
            return;
        }
    ...
    case TAR_OBJ_INV:
        if ( arg1[0] == '\0' )
        {
            ch->println( "What object are you trying to breathe upon?" );
			return;
        }
	...
	case TAR_DIRECTION:
        if (arg1[0] == '\0')
		{
			ch->println( "Breathe in what direction?" );
			return;
		}
		...
		if ( door == -1 )
		{
			ch->printlnf( "'%s' is an invalid direction.", arg1 ); 
			return;
		}
    ...
    case TAR_OBJ_CHAR_OFF:
        if (arg1[0] == '\0')
        {
            if ((victim = ch->fighting) == NULL)
            {
				ch->println( "Breathe on whom or what?" );
				return;
            }
	...
	ch->println( "You take in a deep breath." );
	act("$n takes in a deep breath.", ch, NULL, NULL, TO_ROOM);
    ...
    if ( number_percent( ) > ((get_skill(ch,sn) + ch->modifiers[STAT_SD] - 5)
		/ (is_affected(ch, gsn_deafness) ? 2 : 1)))
    {
		ch->mana -= mana / 2;
		if(is_affected(ch, gsn_deafness))
			ch->println("You think you may have mispronounced something...");
...
void do_commune( char_data *ch, char *argument )
{
	...
	if(!ch->deity)
	{
		ch->println( "You need to worship a diety to grant your request." );
		return;
	}
	 if(IS_WEREFORM(ch))
	{
		ch->println("This form of magic is too complex in your current condition.");
		return;
...
SPRESULT spell_cancellation( int , int level, char_data *ch, void *vo,int  )
{
	...
	if ( IS_SET( ch->in_room->affected_by, ROOMAFF_UTTERDARK )) {
 	...
 	if (check_dispel(level,victim, gsn_regeneration ))
		found = true;
 	if (check_dispel(level,victim, gsn_hold_person ))
    	{
        	found = true;
        	act("$n is now able to move freely.",victim,NULL,NULL,TO_ROOM);
    	}
	...
	if (check_dispel(level, victim, gsn_hallucinate ))
		found = true;
	if (check_dispel(level, victim, gsn_bardic_visions ))
		found = true;
	if (check_dispel(level,victim, gsn_despair))
		found = true;
	...
	if (check_dispel(level,victim, gsn_infravision ))
		found = true;
	if (check_dispel(level,victim, gsn_darkvision ))
		found = true;
...
SPRESULT spell_charm_person( int sn, int level, char_data *ch, void *vo,int  )
{
	...
	if (!IS_NPC(victim))
	{
		ch->println( "You cannot charm other players." );
		return FULL_MANA;
	}
...
SPRESULT spell_enchant_armor( int sn, int level, char_data *ch, void *vo,int )
{
	...
	{
		ch->printf( "It cannot be enchanted.\r\n" );
		return NO_MANA;
	}
	if (obj->wear_loc != -1)
	{
...
SPRESULT spell_enchant_weapon(int sn,int level, char_data *ch, void *vo,int )
{
    ...
    if (dam_found){
        ...
        paf->where      = WHERE_OBJEXTRA;
        paf->type       = sn;
        paf->level 	= UMAX(obj->level,level)/2;
        paf->duration   = level * 20;
...
SPRESULT spell_heal( int , int , char_data *ch, void *vo,int)
{
	...
	{
		victim->bleeding = URANGE( 0, victim->bleeding - 20, 100 );
		if (victim->bleeding < 1 && !IS_NPC(victim))
		{
			victim->bleeding = 0;
			victim->will_die = 0;
			victim->printlnf( "Your wounds close and stop bleeding." );
			act( "$n wounds close.", ch, NULL, victim, TO_NOTVICT );
		}
		else {
			victim->printlnf( "Your wounds close a bit." );
			act( "$n wounds begin to close.", ch, NULL, victim, TO_NOTVICT );
		}
	}
	if ( ch != victim )
		ch->println( "Ok." );
	...
	for ( vch = ch->in_room->people; vch != NULL; vch = vch_next )
	{
			 ...
			 || (IS_EVIL(ch) && IS_GOOD(vch)))
		{
			if (!is_safe_spell(ch,vch,true))
			{
				...
				dam = dice(level,12);
				if(IS_GOOD(ch))
					damage_spell(ch,vch,dam,sn,DAM_HOLY,true);
				if(IS_EVIL(ch))
					damage_spell(ch,vch,dam,sn,DAM_NEGATIVE,true);

			}
		}
		else if (IS_NEUTRAL(vch) &&  !IS_NEUTRAL(vch))
		{
			if (!is_safe_spell(ch,vch,true))
			{
...
SPRESULT spell_identify( int, int, char_data *ch, void *vo, int  )
{
	...
	||	IS_SET(obj->extra2_flags, OBJEXTRA2_NOIDENTIFY)) {
	...
	case ITEM_RUNE:
		ch->printf( "Has %d charges remaining.", obj->value[1] );
		ch->printf( ".\r\n" );
		break;
...
/**************************************************************************/
SPRESULT spell_darkvision( int sn, int level, char_data *ch, void *vo,int )
{
	char_data *victim = (char_data *) vo;
	AFFECT_DATA af;
	...
	if ( IS_AFFECTED(victim, AFF_DARK_VISION) )
	{
		if (victim == ch)
			...
			act("$N already has darkvision.\r\n",ch,NULL,victim,TO_CHAR);
		return HALF_MANA;
	}

	act( "$n's eyes glow with an opaque light.\r\n", ch, NULL, NULL, TO_ROOM );
	...
	affect_to_char( victim, &af );
	victim->printf( "Your eyes glow with an opaque light.\r\n" );
	return FULL_MANA;
}
...
SPRESULT spell_invisibility( int sn, int level, char_data *ch, void *vo,int target )
{
	...
	if ( IS_AFFECTED(victim, AFF_FAERIE_FIRE)) {
		ch->printlnf( "The faerie fire negates your spell." );
		return HALF_MANA;
	}
...
SPRESULT spell_poison( int sn, int level, char_data *ch, void *vo, int target )
{
	...
	||   IS_SET(victim->imm_flags, IMM_POISON) )
	{
...
/**************************************************************************/
SPRESULT spell_light_bonding( int , int level, char_data *ch, void *vo,int )
{
	char_data *victim = (char_data *) vo;
	int heal		  = dice(level/12+1, 2);

	if(!IS_NPC(victim) && victim->pcdata->tired!=-1)
		victim->pcdata->tired = UMAX(victim->pcdata->tired - (level/50+1), 0);

	victim->move= UMIN(victim->move + level / 20, victim->max_move);
	victim->hit = UMIN( victim->hit + heal, victim->max_hit );
	update_pos( victim );
	victim->println( "Life trickles into you!" );
	if ( ch != victim ){
		ch->println( "Ok." );
	}
    return FULL_MANA;
...
/**************************************************************************/
SPRESULT spell_bonding( int , int level, char_data *ch, void *vo,int )
{
	char_data *victim = (char_data *) vo;
    int heal		  = dice(level/8+1, 2);

	if(!IS_NPC(victim) && victim->pcdata->tired!=-1)
		victim->pcdata->tired = UMAX(victim->pcdata->tired - (level/40+2), 0);

	victim->move= UMIN(victim->move + level / 10, victim->max_move);
	victim->hit = UMIN( victim->hit + heal, victim->max_hit );
	update_pos( victim );
	victim->println( "Life flows into you!" );
	if ( ch != victim ){
		ch->println( "Ok." );
	}
    return FULL_MANA;
...
/**************************************************************************/
SPRESULT spell_heavy_bonding( int , int level, char_data *ch, void *vo,int )
{
	char_data *victim = (char_data *) vo;
	int heal		  = dice(level/4+1, 2);

	if(!IS_NPC(victim) && victim->pcdata->tired!=-1)
		victim->pcdata->tired = UMAX(victim->pcdata->tired - (level/30+2), 0);

	victim->move= UMIN(victim->move + level / 5, victim->max_move);
	victim->hit = UMIN( victim->hit + heal, victim->max_hit );
	update_pos( victim );
	victim->println( "Life rushes into you!" );
	if ( ch != victim ){
		ch->println( "Ok." );
	}
    return FULL_MANA;
...
/**************************************************************************/
SPRESULT spell_light_withering( int sn, int level, char_data *ch, void *vo,int  )
{	
	char_data *victim = (char_data *) vo;
	if(!IS_NPC(victim) && victim->pcdata->tired!=-1)
		victim->pcdata->tired = URANGE(0, victim->pcdata->tired + (level/45+1), 90);

	victim->move= UMAX(victim->move - level / 20, 0);

	damage_spell( ch, victim, dice(1, 8) + level / 3, sn, DAMTYPE( sn ),true);
	return FULL_MANA;
}


/**************************************************************************/
SPRESULT spell_withering(int sn,int level,char_data *ch,void *vo,int )
{
	char_data *victim = (char_data *) vo;

	if(!IS_NPC(victim) && victim->pcdata->tired!=-1)
		victim->pcdata->tired = URANGE(0, victim->pcdata->tired + (level/40+2), 90);

	victim->move= UMAX(victim->move - level / 15, 0);
	damage_spell( ch, victim, dice(3, 8) + level - 6, sn, DAMTYPE( sn ), true);
	return FULL_MANA;
}


/**************************************************************************/
SPRESULT spell_heavy_withering(int sn,int level,char_data *ch,void *vo,int )
{
	char_data *victim = (char_data *) vo;

	if(!IS_NPC(victim) && victim->pcdata->tired!=-1)
		victim->pcdata->tired = URANGE(0, victim->pcdata->tired + (level/35+2), 90);

	victim->move= UMAX(victim->move - level / 10, 0);
	damage_spell( ch, victim, dice(2, 8) + level / 2, sn, DAMTYPE( sn ), true);
	return FULL_MANA;
}

/**************************************************************************/
SPRESULT spell_destroy_living(int sn,int level,char_data *ch,void *vo,int )
{
	char_data *victim = (char_data *) vo;
	int dam;
	if(!IS_NPC(victim) && victim->pcdata->tired!=-1)
		victim->pcdata->tired = URANGE(0, victim->pcdata->tired + (level/30+2), 90);
	victim->move= UMAX(victim->move - level / 5, 0);
	dam = UMAX(  20, victim->hit - dice(1,4) );
	if ( saves_spell( level, victim,DAMTYPE(sn)) )
		dam = UMIN( 50, dam / 2 );
...
SPRESULT spell_refresh( int , int level, char_data *ch, void *vo,int  )
{
	...
	if(!IS_NPC(victim) && victim->pcdata->tired!=-1)
	{
		victim->pcdata->tired-=level/20+2;
		if(victim->pcdata->tired<0)
			victim->pcdata->tired=0;
	} 
	if (victim->max_move == victim->move)
		victim->printlnf( "You feel fully refreshed!" );
	else
		victim->printlnf(  "You feel less tired." ); 
	if ( ch != victim )
		ch->println( "Ok." );
	return FULL_MANA;
}
...
/**************************************************************************/
SPRESULT spell_cupcake_breath( int sn, int level, char_data *ch, void *vo,int )
{
	char_data *victim = (char_data *) vo;
	...
	int btype =  number_range (1,4);

	switch(btype)
	{
		case 1:
			act("$n breathes forth a blast of `#`Rhot strawberry `Wfrosting`^.",ch,NULL,victim,TO_NOTVICT);
			act("$n breathes a blast of `#`Rhot strawberry `Wfrosting`^ over you!",ch,NULL,victim,TO_VICT);
			act("You breath forth a blast of `#`Rhot strawberry `Wfrosting`^.",ch,NULL,NULL,TO_CHAR);
			...
			break;
		case 2:			
			act("$n breathes forth a cold blast of `#`Wv`Ca`Wn`Ci`Wl`Cl`Wa `Yfrosting`^.",ch,NULL,victim,TO_NOTVICT);
			act("$n breathes a cold blast of `#`Wv`Ca`Wn`Ci`Wl`Cl`Wa `Yfrosting`^ over you!",ch,NULL,victim,TO_VICT);
			act("You breath forth a cold blast of `#`Wv`Ca`Wn`Ci`Wl`Cl`Wa `Yfrosting`^.",ch,NULL,NULL,TO_CHAR);
			...
			break;
		case 3:	
			act("$n breathes forth a blast of `#`Schocolate `Wfrosting`^.",ch,NULL,victim,TO_NOTVICT);
			act("$n breathes a cone of `#`Schocolate `Wfrosting`^ over you!",ch,NULL,victim,TO_VICT);
			act("You breath forth a cone of `#`Schocolate `Wfrosting`^.",ch,NULL,NULL,TO_CHAR);
			...
			break;
		case 4:
    			act("$n breathes out a rain of deadly `#`?c`?o`?l`?o`?r`?e`?d `?c`?u`?p`?c`?a`?k`?e `?s`?p`?r`?i`?n`?k`?l`?e`?s`^!",ch,NULL,NULL,TO_NOTVICT);
    			act("You breath out a rain of deadly `#`?c`?o`?l`?o`?r`?e`?d `?c`?u`?p`?c`?a`?k`?e `?s`?p`?r`?i`?n`?k`?l`?e`?s`^.",ch,NULL,NULL,TO_CHAR);
			act("$n breathes out a rain of deadly `#`?c`?o`?l`?o`?r`?e`?d `?c`?u`?p`?c`?a`?k`?e `?s`?p`?r`?i`?n`?k`?l`?e`?s`^ over you!",ch,NULL,victim,TO_VICT);
  			hpch = UMAX(16,ch->hit);
...
SPRESULT spell_portal( int sn, int level, char_data *ch, void *,int )
{
    ...
    ||	 (is_clan(victim) && !is_same_clan(ch,victim)))
    {
        ...
        if ( victim->in_room->area->world == NULL)
	{
    ...
    if ( ch->in_room->area->world->name != victim->in_room->area->world->name )
    {
        ch->println( "You failed." );
        return FULL_MANA;
    } 
...
/**************************************************************************/
SPRESULT spell_spiritual_champion( int sn, int level, char_data *ch, void *vo, int)
{
    char_data* victim = (char_data*) vo;
    AFFECT_DATA af;
    ...
    {
		if (victim == ch){
			ch->println("You have already been imbued with your god's power.");
			return NO_MANA;
		}else{
			act("$N is already inhabited by his god's power.",ch,NULL,victim,TO_CHAR);
			return HALF_MANA;
		}
    }
    ...
    af.bitvector = AFF2_POSSESSION;
    affect_to_char( victim, &af );
    act( "$n looks stronger and mightier as the strength of $s god enters $m.",
         victim, NULL, NULL, TO_ROOM );
    victim->println("You feel the power of your god infuse you.");
    return FULL_MANA;
}
/**************************************************************************/
bool immune_dispel( char_data *victim, int dispel_level)
{
	AFFECT_DATA *af;
	char_data *vch, *vch_next;
	int level_reduction = number_range(1, 1 + (dispel_level / 20) );
	int dam = number_range(1, 30);
	bool enchantment_broken = false;
	char room_msg_one[MSL];
	char room_msg_two[MSL];
	char vict_msg[MSL];
	if( IS_SET( victim->affected_by2, AFF2_NO_DISPEL ) )
	{
		if((af = affect_find(victim->affected, gsn_enchantment_roots )) != NULL)
		{
			if( (dispel_level > af->level) || (af->level < 1) )
			{   
			   enchantment_broken = true;
			   affect_remove(victim, af);
			   sprintf( vict_msg, "A spell shatters through your enchantment!" );
			   sprintf( room_msg_one, "$N falls back as magical ties are severed!!" );
			   sprintf( room_msg_two, "$N falls back as if hit by something!" );
			}   
			else
			{
				af->level -= level_reduction;
				sprintf( vict_msg, "You feel the drain of a spell testing your enchantment!" );
				sprintf( room_msg_one, "$N strains as spells pull life away!" );
				sprintf( room_msg_two, "$N seems under some kind of strain!" );
			}
			if(enchantment_broken)
				dam *= 2;

			victim->move -= dam / 2;
			victim->mana -= dam / 2;
			damage_spell( victim, victim, dam, gsn_enchantment_roots, DAMTYPE(gsn_enchantment_roots),false);
		}
		else
		   sprintf( room_msg_one, "Magic slides off of $N harmlessly!" );

		if(!IS_NULLSTR(vict_msg))
			victim->println(vict_msg);
		for ( vch = victim->in_room->people; vch; vch = vch_next )
		{
		   ...
		   if ( vch->in_room == victim->in_room )
		   {
			  if ( vch == victim)
				 continue;
			  if(IS_AFFECTED(vch, AFF_DETECT_MAGIC))
				 act(room_msg_one, vch, NULL, victim, TO_CHAR );
			  else if(!IS_NULLSTR(room_msg_two))
				 act(room_msg_two, vch, NULL, victim, TO_CHAR );
		   }
		}
		return true;
	}
...
void do_eyepower( char_data *ch, char *argument )
{
	char arg1[MIL];
	char target_name[MIL];
   ...
   int cast_level = ch->level;
	...
	{
		ch->println( "Try using eyeb to raise your eyebrow." );
		return;
	}
    ...
    if ( IS_NULLSTR(arg1))
    {
	ch->println("Use which eye at who ?");
	ch->println("`#`BSyntax: `wEye # <target>`^");
	return;
    }
	...
	*/
    ...
    case TAR_CHAR_OFFENSIVE:
        if ( target_name[0] == '\0' )
        {
            if ( ( victim = ch->fighting ) == NULL )
            {
                ch->println( "Use your eyestalk on whom ?" );
                return;
            }
        }
        else
	...
	case TAR_DIRECTION:
        if (target_name[0] == '\0')
		{
			ch->println( "Gaze in what direction?" );
			return;
		}
		
		door = dir_lookup( target_name);
		
		if ( door == -1 )
		{
			ch->printlnf( "'%s' is an invalid direction.", target_name ); 
			return;
		}
	...
	ch->println( "You focus your eyestalk." );
	act("An eyestalk on $n glows with power.", ch, NULL, NULL, TO_ROOM);
	...
	
    {
		...
		if(victim && IS_SET(victim->config2, CONFIG2_BEHOLDER_EYE))
		{
			act("Your anti-magic ray prevents $N's magic.", ch, NULL, victim, TO_CHAR);
			act("$n's anti-magic ray prevents $N magic.", ch, NULL, victim, TO_NOTVICT);
			act("$n anti-magic ray prevents your power.", ch, NULL, victim, TO_VICT);
			return;
		}
...
/**************************************************************************/
void do_antimagiceye( char_data *ch, char *)
{
	if(ch->race != race_lookup("beholder") )
	{
		do_huh(ch, "");
	...
	{
		ch->println("Your main eye is destroyed in your quest for magic.");
		return;
	}

	if(IS_SET(ch->config2, CONFIG2_BEHOLDER_EYE))
	{
		REMOVE_BIT(ch->config2, CONFIG2_BEHOLDER_EYE);
		ch->println("Your anti-magic ray is `#`YOFF`^.");
		act("The central eye on $n stops glowing.", ch,NULL,NULL,TO_ROOM);
		return; 
	}
	else
		SET_BIT(ch->config2, CONFIG2_BEHOLDER_EYE);
		ch->println("Your anti-magic ray is `#`YON`^.");
		act("The central eye on $n `#`Rglows red`^ with power.", ch,NULL,NULL,TO_ROOM);
		return; 
}
/**************************************************************************/
/**************************************************************************/
