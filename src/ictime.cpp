...
void do_setage(char_data *ch, char *argument)
{
    ...
    if (IS_VAMPIRE(ch)){
		ch->println("You must earn your age.");
		return;
	}
...
int get_age( char_data *ch )
{
	...
	{
		year = (int)ch->pcdata->birthdate/ICTIME_IRLSECS_PER_YEAR;
	}
	else 
	{
		year = (int)(ch->id-(17*ICTIME_IRLSECS_PER_YEAR))/ICTIME_IRLSECS_PER_YEAR;
	}
	return year;
}
/**************************************************************************/
