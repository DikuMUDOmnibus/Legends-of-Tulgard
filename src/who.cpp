...
#define WHOHEADER_PKILL_PORT "`r-`R=`r=`R=`r=`R=`r=`R=`r=`R=`r=`R=`r= `R P K I L L   P O R T  `r=`R=`r=`R=`r=`R=`r=`R=`r=`R=`r-`x\r\n"
    ...
    if (nMatch<15){
		if(number_range(1,2)==1){
			...
			add_buf(output,"`xYou can customise the colors of the wholist using the `=Ccolor`x command.\r\n");
		}
	}
...
void do_title(char_data *ch, char *argument)
{
	...
	if(c_str_len(strip_colour(arg))>45){
	...
	if(c_str_len(argument)<0){
		ch->println("You can't put newline color codes in your title sorry.");
		return;
	}
...
void do_immtitle(char_data *ch, char *argument)
{
	...
	if(c_str_len(argument)!=5){
		ch->println("Your immtitle must work out to be exactly 5 visible characters long (you can use color codes).");
		return;
	}
