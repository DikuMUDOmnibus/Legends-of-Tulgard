...
const	struct	mob_cmd_type	mob_cmd_table	[] =
{
	...
	{"echoat",do_mpechoat, "Displays string to the target only",
 	...
 	{"endprison", do_mppardon, "Ends the players prison sentance",
 		"mob endprison <player>",
 		"Example: mob endprison $n",
 		"This will release them from prison and retrieve their belongings." },
...
void do_mppardon( char_data *ch, char *argument )
{
     ...
     if ( arg[0] != '\0' ){
 		ch->mprog_target = get_char_world( ch, arg );
 		if(!ch->mprog_target){ 
 			mpbug( "MpEndprison: target '%s' not found.", arg);
 			return;
 		}
     }else{
 		mpbug( "MpEndprison: argument '%s' is invalid.", arg);
 		return;
 	 }
     do_endprison (ch->mprog_target, "");
     return;
}
/**************************************************************************/
