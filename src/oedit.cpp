...
/**************************************************************************/
bool oedit_clan( char_data *ch, char * argument)
{
    OBJ_INDEX_DATA *pObj;
    ...
    if(!str_cmp(argument, "none")){
		ch->printlnf("Clan cleared - was '%s`x'", pObj->clan?pObj->clan->cname():"none already");
		pObj->clan=NULL;
		return true;
    }
	...
	if(!pObj->clan){
		ch->printlnf("Object clan set to '%s'", newclan->cname());
	}else{
		if(newclan==pObj->clan){
			ch->printlnf("This object is already set to '%s`x' clan.", 
				newclan->cname());
			return false;
		}
		ch->printlnf("Object clan changed from '%s`x' to '%s`x'", 
			pObj->clan->cname(),
			newclan->cname());
	}
    pObj->clan = newclan;
...
void show_obj_values( char_data *ch, OBJ_INDEX_DATA *obj )
{
	...
	case ITEM_RUNE:
		ch->printlnf( "[v0] Rune Marked [0/1]: [%d]", obj->value[0] );
		ch->printlnf( "[v1] Rune Charges:      [%d]", obj->value[1] );
		ch->printlnf( "[v2] Rune Destination:  [%d]", obj->value[2] );
		break;

	case ITEM_AMMO:
		ch->printlnf( "[v0] Rounds per item:      [%d]", obj->value[0] );
		break;

	case ITEM_FIREARM:
		ch->printlnf( "[v0] Firearm Max Shots:      [%d]", obj->value[0] );
		ch->printlnf( "[v1] Firearm Shots:      [%d]", obj->value[1] );
		ch->printlnf( "[v2] Firearm Spell:  %s", obj->value[2] != -1 ? skill_table[obj->value[2]].name : "none" );
		ch->printlnf( "[v3] Ammo Vnum:  [%d]", obj->value[3] );
		ch->printlnf( "[v4] Firearm Flags:   %s", flag_string( firearm_flags, obj->value[4] ));
		break;

	case ITEM_VEHICLE_CONTROLS:
		ch->printlnf( "[v0] Vehicle Vnum:    [%d]", obj->value[0] );
		break;


	case ITEM_VEHICLE_EXIT:
		ch->printlnf( "[v0] Vehicle Vnum: [%d]", obj->value[0] );
		break;

	case ITEM_VEHICLE:
		ch->printlnf( "[v0] Vehicle Room Vnum: [%d]", obj->value[0] );
		ch->printlnf( "[v1] Vehicle Hitpoints: [%d]", obj->value[1] );
		ch->printlnf( "[v2] Vehicle Flags:   %s", flag_string( vehicle_flags, obj->value[2] ));
		break;
	...
	case ITEM_PIECE:
		ch->printlnf( "[v0] Previous Item Vnum: [%d]", obj->value[0] );
		ch->printlnf( "[v1] Next Item Vnum:     [%d]", obj->value[1] );
		ch->printlnf( "[v2] Final Item Vnum:    [%d]", obj->value[2] );
		break;
	...
	case ITEM_STAFF:
	    ...
	    break;
         ...
         case ITEM_TABAC_PIPE:
             	ch->printlnf("[v0] Total Smokes:       [%d]", obj->value[0] );
              	ch->printlnf("[v1] Smokes Remaining:   [%d]", obj->value[1] );
 		break;
 
         case ITEM_SLOTMACHINE:
              	ch->printlnf("[v0] Gold Cost:       [%d]", obj->value[0] );
               	ch->printlnf("[v1] Jackpot Value:   [%d]", obj->value[1] );
               	ch->printlnf("[v2] Number of Bars:  [%d]", obj->value[2] );
 		ch->printlnf("[v3] Partial Jackpot: [%d]", obj->value[3] );
                ch->printlnf("[v4] Freeze Jackpot:  [%d]", obj->value[4] );
              	break;
	...
	case ITEM_MATERIAL:
		ch->printlnf( "[v0] Material Type:   %s",
			flag_string( material_types, obj->value[0] ));
		ch->printlnf( "[v1] Material:	%s",
			flag_string( (flag_type *)ore_table, obj->value[1] ));
		ch->printlnf( "[v2] Purity:   %d", obj->value[2] );
		ch->printlnf( "[v3] Grade:    %d", obj->value[3] );
	    break;


	case ITEM_PROJECTILE:
		ch->printlnf("[v0] Projectile type:  %s", flag_string( projectile_class_types,  obj->value[0] ) );
		ch->printlnf("[v1] Number of Dice:   [%d]", obj->value[1] );
		ch->printlnf("[v2] Type of Dice:     [%d]", obj->value[2] );
		ch->printlnf("[v3] Spell:            %s",
			obj->value[3] != -1 ? skill_table[obj->value[3]].name : "none"  );
		ch->printlnf( "Average damage is %d", 
	...
	case ITEM_DRINK_CON:
		...
		ch->printlnf( "[v3] Food Flags:    [%s]", flag_string(food_flags, obj->value[3]));
		break;
...
bool set_obj_values( char_data *ch, OBJ_INDEX_DATA *pObj, int value_num, char *argument)
{
	...
	case ITEM_LIGHT:
     ...
     case ITEM_TABAC_PIPE:
 	       switch (value_num)
 	         {
 			case 0:
 		 	ch->println( "TOTAL SMOKES SET.");
 		  	pObj->value[0] = atoi( argument );
 		   	break;
 		     	case 1:
 			ch->println( "REMAINING SMOKES SET.");
     ...
     case ITEM_PIECE:
 	       switch (value_num)
 	         {
 			case 0:
 		 	ch->println( "PREVIOUS ITEM VNUM SET.");
 		  	pObj->value[0] = atoi( argument );
 		   	break;
 		     	case 1:
 			ch->println( "NEXT ITEM VNUM SET.");
 			pObj->value[1] = atoi( argument );
 			break;
 		     	case 2:
 			ch->println( "FINAL ITEM VNUM SET.");
 			pObj->value[2] = atoi( argument );
 			break;
		}
		break;
     ...
     case ITEM_AMMO:
 	       switch (value_num)
 	         {
 			case 0:
 		 	ch->println( "ROUNDS PER ITEM SET.");
 		  	pObj->value[0] = atoi( argument );
 		   	break;
		}
		break;
     ...
     case ITEM_FIREARM:
 	       switch (value_num)
 	         {
			default:
			ch->println( "Valid Firearm stats:");
			ch->println( "v0 = Firearm Maximum Shots.");
			ch->println( "v1 = Firearm Shots in Weapon.");
			ch->println( "v2 = Firearm Spell Attack.");
			ch->println( "v3 = Vnum of Ammo Clip.");
			show_olc_flags_types_value(ch, firearm_flags,"v4", pObj->value[4]);
			ch->println( "");
 			case 0:
 		 	ch->println( "FIREARM MAX SHOTS SET.");
 		  	pObj->value[0] = atoi( argument );
 		   	break;
 		     	case 1:
 			ch->println( "FIREARM SHOTS SET.");
 			pObj->value[1] = atoi( argument );
 			break;
			case 2:
			ch->println( "FIREARM SPELL SET.\r\n" );				// Extra lf
			pObj->value[2] = skill_lookup( argument );
			break;
 		     	case 3:
 			ch->println( "AMMO VNUM SET.");
 			pObj->value[3] = atoi( argument );
 			break;
			case 4:
			ch->println( "FIREARM FLAG TOGGLED." );
			pObj->value[4] ^= (flag_value( firearm_flags, argument ) != NO_FLAG
				? flag_value( firearm_flags, argument ) : 0 );
			break;
		}
		break;
     ...
     case ITEM_RUNE:
 	       switch (value_num)
 	         {
 			case 0:
 		 	ch->println( "RUNE MARKED SET.");
 		  	pObj->value[0] = atoi( argument );
 		   	break;
 		     	case 1:
 			ch->println( "RUNE CHARGES SET.");
 			pObj->value[1] = atoi( argument );
 			break;
 		     	case 2:
 			ch->println( "RUNE VNUM SET.");
 			pObj->value[2] = atoi( argument );
 			break;
		}
		break;
     ...
     case ITEM_VEHICLE_CONTROLS:
 	       	...
 	       	{
 			case 0:
 		 	ch->println( "VEHICLE OBJ VNUM SET.");
 		  	pObj->value[0] = atoi( argument );
			break;
		}
		break;
     ...
     case ITEM_VEHICLE:
 	       ...
 	       {
		default:
			ch->println( "Valid Vehicle stats:");
			ch->println( "v0 = Vehicle Room.");
			ch->println( "v1 = Vehicle Hitpoints (1000 norm).");
			ch->println( "v2 = weapon attack type.");
			show_olc_flags_types_value(ch, vehicle_flags,"v3", pObj->value[2]);
			ch->println( "");
			return false;
 		case 0:
 		 	ch->println( "VEHICLE ROOM VNUM SET.");
 		  	pObj->value[0] = atoi( argument );
			break;
 		case 1:
 		 	ch->println( "VEHICLE HITPOINTS SET.");
 		  	pObj->value[1] = atoi( argument );
			break;
		case 2:
			ch->println( "VEHICLE FLAG TOGGLED." );
			pObj->value[2] ^= (flag_value( vehicle_flags, argument ) != NO_FLAG
				? flag_value( vehicle_flags, argument ) : 0 );
			break;
		}
		break;
     ...
     case ITEM_SLOTMACHINE:
        switch (value_num)
          {
          case 0:
            ch->println( "GOLD COST SET.");
            pObj->value[0] = atoi( argument );
            break;
          case 1:
            ch->println( "JACKPOT VALUE.");
            pObj->value[1] = atoi( argument );
            break;
          case 2:
            ch->println( "NUMBER OF BARS SET.");
            pObj->value[2] = atoi( argument );
            break;
          case 3:
            ch->println( "PARTIAL JACKPOT SET.");
            pObj->value[3] = atoi( argument );
           break;
          case 4:
            ch->println( "FREEZE JACKPOT SET.");
	...
	case ITEM_PILL:
     ...
     case ITEM_MATERIAL:
 	       switch (value_num)
 	         {		
			   default:
				ch->println( "Valid v0 material types:");
				show_olc_flags_types_value(ch, material_types,"v0", pObj->value[0]);
				ch->println( "v1 material:");
				show_olc_flags_types_value(ch, (flag_type *)ore_table,"v1", pObj->value[1]);
				ch->printlnf( "v2 = Purity (by percentage): %d", pObj->value[2]);
				ch->printlnf( "v3 = Grade (by percentage): %d", pObj->value[3]);
				ch->println( "");
				ch->println( "SeeAlso: HELP `=_OLC-ITEM-MATERIAL");
				return false;
				case 0:
					{
						if ( ( value = flag_value( material_types, argument ) ) != NO_FLAG ){
							ch->println( "MATERIAL TYPE SET.\r\n" );
							pObj->value[0] = flag_value( material_types, argument );
						}else{
							ch->println( "Valid v0 material types include:");
							show_olc_flags_types_value(ch, material_types,"v0", pObj->value[0]);
							return false;
						}
					}
				break;
 		    case 1:
			if ( ( value = flag_value( (flag_type *)ore_table, argument ) ) != NO_FLAG ){
 				ch->println( "MATERIAL SET.");
 				pObj->value[1] = flag_value( (flag_type *)ore_table, argument);
			}else{
				ch->println( "Valid v1 material include:");
				show_olc_flags_types_value(ch, (flag_type *)ore_table,"v1", pObj->value[1]);
				return false;
			}
 			break;
 		    ...
 		    case 2:
 			ch->println( "PURITY SET.");
 			pObj->value[2] = atoi( argument );
 			break;
 		    case 3:
 			ch->println( "GRADE SET.");
 			pObj->value[3] = atoi( argument );
 			break;
    ...
    case ITEM_PROJECTILE:
		...
		{
		default:
			ch->println( "Valid v0 projectile types:");
			show_olc_flags_types_value(ch, projectile_class_types,"v0", pObj->value[0]);
			ch->println( "v1 = number of dice to roll.");
			ch->println( "v2 = type of dice to roll.");
			ch->println( "v3 = spell affect.");
			ch->println( "");
			return false;
		case 0:
			{
				if ( ( value = flag_value( projectile_class_types, argument ) ) != NO_FLAG ){
					ch->println( "PROJECTILE TYPE SET.\r\n" );
					pObj->value[0] = flag_value( projectile_class_types, argument );
				}else{
					ch->println( "Valid v0 projectile types include:");
					show_olc_flags_types_value(ch, projectile_class_types,"v0", pObj->value[0]);
					return false;
				}
			}
	...
	case ITEM_FOOD:
		...
		{
		default:
			ch->println( "v0 = hours of food value.");
			ch->println( "v1 = hours of full value.");
			ch->println( "v3 = food item flags.");
			show_olc_flags_types_value(ch, food_flags,"v3", pObj->value[3]);
			ch->println( "");
			ch->println( "SeeAlso: HELP `=_OLC-ITEM-FOOD");
			return false;
		case 0:
		...
		case 1:
			...
			break;
		case 3:
			ch->printlnf( "Valid v3 Food flags are: %s", flag_string( food_flags, -1));
			ch->println( "FOOD FLAGS TOGGLED.\r\n" );
	            	pObj->value[3] ^= (flag_value( food_flags, argument ) != NO_FLAG
	            	? flag_value( food_flags, argument ) : 0);
			break;
		}
		break;
...
bool oedit_show( char_data *ch, char *)
{
	...
	ch->printf(   "`=rDurability: `x%-2d ",pObj->durability );
	...
	ch->printlnf("`=rClan: `=x%s`x",	pObj->clan ? pObj->clan->m_pName : "`=$(none)" );
...
bool oedit_type( char_data *ch, char *argument )
{
    ...
    if ( !IS_NULLSTR(argument))
    {
		...
		if ( ( value = flag_value( item_types, argument ) ) != NO_FLAG )
		{
			...
			case ITEM_VEHICLE_CONTROLS:
				pObj->extra_descr=ed_prefix(pObj->extra_descr, "_TRAVEL", "`YREPLACE THIS MESSAGE HERE.`x");
				pObj->value[0] = 0;
				pObj->value[1] = 0;
				pObj->value[2] = 0;
				pObj->value[3] = 0;
...
bool oedit_durability( char_data *ch, char *argument )
{
    ...
    && ( value <= 10 ) )
    {
		...
		ch->printlnf( "Durability changed from %d to %d.", 
			pObj->durability, value);
		pObj->durability = value;
		return true;
    }

    ch->println("Syntax:  durability <number>");
	ch->println("Where number can range from 1 (fragile) to 10 (hardy).");
    return false;
}
/**************************************************************************/
...
/**************************************************************************/
bool oedit_ospec(char_data *ch, char *argument)
{
    OBJ_INDEX_DATA *pObj;

	if (!HAS_SECURITY(ch,7))
	{
		ch->println("The ospec command is not allowed below security 7.");
		return false;
	}
    ...
    if ( argument[0] == '\0' )
    {
        ch->println("Syntax:  ospec [special function]");
        ch->println(" To clear an object special function: ospec none\r\n");
    ...
    if ( ospec_lookup( argument ) )
    {
        ...
        ch->printlnf("Ospec set to %s.", ospec_name( pObj->ospec_fun ));
		return true;
    }

    ch->println("OEdit: No matching object special function.");
