...
/**************************************************************************/
void do_pcletgain( char_data *ch, char *argument )
{
    char arg[MIL],buf[MSL];
    char_data *victim;
    ...
    if (!HAS_CONFIG2(ch, CONFIG2_LETGAIN))
    {
		  do_huh (ch, "");
		  return;
	 }
...
/**************************************************************************/
void do_pcunletgain( char_data *ch, char *argument )
{
         char arg[MIL],buf[MSL];
         char_data *victim;
...
/**************************************************************************/
// lists letgains
void do_pclistletgains( char_data *ch, char *argument )
{
	...
	ch->println("type pclistlet <playername> for more details on a single player.");
}
/**************************************************************************/
// handle accepted offline letgains
void do_pcofflineletgain( char_data *ch, char *argument )
{
	...
	if(IS_NULLSTR(name)){
		ch->println("Syntax: offlineletgain <name> [new_short]");
		ch->println("  new_short is an optional new short description.");
		ch->println("use declineletgain to deny a letgain request.");
		return;		
	}
	...
	node->granted_moot=0;
	node->granted_date=current_time;
	ch->printlnf("Offline letgain on %s granted.", node->name);
	save_letgain_db();
}
/**************************************************************************/
// decline letgain
void do_pcdeclineletgain( char_data *ch, char *argument )
{
...
/**************************************************************************/
void do_pcshort( char_data *ch, char *argument)
{
