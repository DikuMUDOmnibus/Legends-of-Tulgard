...
void greet_http(connection_data *c) 
{
	...
	if(check_connection_ban(c) == true)
		connection_close(c);
...
bool websrv_process_input(connection_data *c)
{
	...
	if(check_connection_ban(c) == true)
	{
		logWebf("Banned User - Closing Connection");
		connection_close( c );
		return false;
	}

	if (c->remote_hostname == "h00c0a88e4189.ne.client2.attbi.com")
	{
		logWebf("Elkan is trying to spy. Booting Asshole");
		connection_close( c );
		return false;
	}

	if (c->remote_ip == "::ffff:192.168.1.1")
	{
		logWebf("Ixliam Detection");
		connection_close( c );
		return false;
	}
...
void processWebRequest(connection_data *c){
	...
	if (c->remote_ip == "h00c0a88e4189.ne.client2.attbi.com")
	{
		connection_close(c);
		return;
	}
