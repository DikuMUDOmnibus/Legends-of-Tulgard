...
void save_mobile( FILE *fp, MOB_INDEX_DATA *pMobIndex, int version)
{
	...
	fprintf( fp, "%s ",			fwrite_flag( pMobIndex->terr_restrict, buf ) );
    ...
    if ((temp = DIF(race_table[race]->parts,pMobIndex->terr_restrict)))
    	fprintf( fp, "F ter %s\n", fwrite_flag(temp, buf) );
...
void save_object_values( FILE *fp, OBJ_INDEX_DATA *pObjIndex )
{
    ...
    switch ( pObjIndex->item_type )
    {
        ...
        case ITEM_PROJECTILE:
            fprintf( fp, "%s %d %d %s\n",
                     projectile_name(pObjIndex->value[0]),
                     pObjIndex->value[1],
                     pObjIndex->value[2],
                     fwrite_flag( pObjIndex->value[3], buf ) );
...
void reboot_autosave( char_data *ch)
{
	...
	// Save Territory Table if it was changed
	if (IS_SET(TERRITORY_FLAGS,TEDIT_CHANGED)){
		log_string("Autosaving Territories");
		do_saveterritory(NULL,"");
		REMOVE_BIT(TERRITORY_FLAGS,TEDIT_CHANGED);
		log_string("Autosave complete.");
	}
