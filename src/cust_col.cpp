...
{
	{	"show",		custom_colour_show,	0, "Shows your current color scheme."},
	{	"showperson",custom_colour_showperson,	0, "Shows another persons colors scheme."},
	{	"set",		custom_colour_set,	0, "Sets a particular color in your color scheme."},
	{	"prefix",	custom_colour_prefix,	0, "Sets a particular character to be used to prefix color codes."},
	{	"clear",	custom_colour_clear,	0, "Clears all color customizations."},	
	{	"list",		custom_colour_list,	0, "Lists all the mud color templates."},
	{	"use",		custom_colour_use,	0, "Use a particular color scheme/template."},	
	{	"on",		custom_colour_on,	0, "Turn on your color."},
	{	"off",		custom_colour_off,	0, "Turn off your color."},
	{	"flashing",	custom_colour_flashing,	0, "Enables/Disables support for flashing ansi colors."},
	{	"mode",		custom_colour_mode,	0, "To change to a color mode other than ansi."},
	{	"codelist",	custom_colour_codelist,	CMD_ADMIN_ONLY, "List sorted custom color codes (find free codes)."},	
	{	"savescheme",custom_colour_savescheme,	CMD_IMMORTAL_ONLY, "Save your current color settings as a scheme."},
	...
	{	"channel", custom_colour_channel, 0, "Enabled/Disable other peoples color codes in channels."},
	{	"socialscolour", custom_colour_socials_colour, 0, "Enabled/Disable the displaying of color in socials."},	
	{	"",		NULL, 0, ""}
};
/**************************************************************************/
...
void do_colour( char_data *ch, char *argument )
{
	...
	if(IS_NULLSTR(arg)){ // no args - show syntax
		ch->printlnf("%s Customing Color System", MUD_NAME);
		ch->println("Syntax:");
		for(i=0;!IS_NULLSTR(custom_colour_cmd_table[i].name); i++){
			...
			if(IS_IMMORTAL(ch)){
				ch->printlnf("  color %-15s - %s", custom_colour_cmd_table[i].name, 
					custom_colour_cmd_table[i].syntax);
			}else{
				ch->printlnf("  color %-10s - %s", custom_colour_cmd_table[i].name, 
					custom_colour_cmd_table[i].syntax);
			}
		}
		return;
	...
	if(IS_NULLSTR(custom_colour_cmd_table[i].name)){
		ch->printlnf("Unrecognised Custom Color command '%s'", arg);
		do_colour(ch,"");
		return;
	}
...
{	
	...
	if(!GAMESETTING(GAMESET_PLAYERS_CANT_ACCESS_OTHERS_COLOURS) || IS_IMMORTAL(ch)){
		add_buf(output,"  Use 'color showplayer <playername>' to see anothers settings.\r\n");
	}
	if(IS_NULLSTR(argument)){
		add_buf(output,"  You can filter this list like the area list - e.g. '`=Ccolor show who`x'\r\n");
	}
...
void custom_colour_set(char_data *ch, char*argument)
{
	...
	if(value<1 || value>max || IS_NULLSTR(argument)){
		ch->println("syntax: color set <number> <colorcode>");
		ch->printlnf("Number must be between 1 and %d", max);
		ch->println("Use a colorcode of . to remove customisation.");
		return;
	}
...
void custom_colour_on(char_data *ch, char *)
{
    ...
    ch->println("You are mudding in `rC`RO`YL`gO`mR`x!!!");
}
/**************************************************************************/
...
void custom_colour_mode(char_data *ch, char *argument)
{
	...
	if(IS_NULLSTR(argument)){
		ch->println( "syntax: `=Ccolor mode <new color mode>`x");
		ch->wraplnf( "Valid color modes include: %s", flagtable_names(colourmode_types));
	...
	if(value == NO_FLAG ){
		ch->printlnf( "Invalid color mode '%s'.", argument);
		ch->wraplnf( "Valid color modes include: %s", flagtable_names(colourmode_types));
		return;
	}
	...
	ch->printlnf("Color mode set to %s.", flag_string(colourmode_types, value));
}
/**************************************************************************/
void custom_colour_clear(char_data *ch, char *)
{
	memset(ch->desc->colour_memory.custom_colour,'.', MAX_CUSTOM_COLOUR);
	ch->println("Color customisations cleared."); 
}
...
void custom_colour_use(char_data *ch, char *argument)
{
	...
	if(IS_NULLSTR(argument)){
		ch->println("Syntax: color use <scheme/template name>   (as per color list)");
		ch->println("Syntax: color use <scheme/template number> (as per color list)");
		return;
	}
	...
	ch->printlnf("Base color scheme changed to '%s'", new_scheme->template_name);
}
/**************************************************************************/
