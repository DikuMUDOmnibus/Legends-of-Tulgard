...
void bust_a_prompt( connection_data *d )
{
	...
	if(!IS_NPC(ch) && IS_WEREFORM(ch) )
        {
		ch->printlnf("`S** %s Form **", capitalize(were_types[ch->altform.subtype].name)) ;
	}

	if(IS_VAMPAFF(ch,VAM_FANGS) && !IS_VAMPAFF(ch,VAM_CLAWS))
        {
		ch->println("`S** Fangs Out **"); 
	}

	if(!IS_VAMPAFF(ch,VAM_FANGS) && IS_VAMPAFF(ch,VAM_CLAWS))
        {
		ch->println("`S** Claws Out **"); 
	}

	if(IS_VAMPAFF(ch,VAM_FANGS) && IS_VAMPAFF(ch,VAM_CLAWS))
        {
		ch->println("`S** Fangs & Claws Out **"); 
	}

	if(!IS_NPC(ch) && IS_DRAGONFORM(ch))
	{
		ch->println("`S** Humanoid Form **"); 
	}

	if(!IS_NPC(ch) && IS_SET(ch->config2, CONFIG2_BEHOLDER_EYE))
	{
		ch->println("`S** Anti-Magic Active **"); 
	}

	if(ch->desc->editor){ // working in olc
		if(ch->desc->editor == ED_CRAFT){
			ch->print("`=p");
			prompt="`#`C[`cCrafting`C -`c %t%d`C]`^";
		}
		else{
			ch->print("`=p");
			prompt = ch->olcprompt;
