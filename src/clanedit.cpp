...
bool clanedit_create( char_data *ch, char *newName )
{
	...
	*node		= zero_node;
	...
	replace_string(clan_list->m_pFounder, newName );
...
void do_clanedit( char_data *ch, char *argument )
{
	...
	if ( !str_cmp( arg, "create" ))
	{
		...
		if(!str_infix("`", arg)){
			ch->println("A new clan name must not contain any color characters.");
			return;
		}
...
bool clanedit_show(char_data *ch, char *)
{
	...
	ch->printlnf(  "`=rFounder:  `x%s`=r\r\n`=rClanColor:`s", pC->m_pFounder);
	ch->printlnf("`=rWho name:  %-20s   `=r",	pC->m_pWhoName );
	ch->printlnf("`=rWho short: [`x%-3s`=r]   `=r", pC->m_pWhoCat );
	ch->printlnf("`=rHidden:     `x%-3s`=r   `=r", pC->m_Hidden ? "`GYes`=r" : "`RNo`=r");
...
/**************************************************************************/
bool clanedit_founder(char_data *ch, char *argument)
{
    CClanType		*pClan;
    ...
    if ( argument[0] == '\0' )
    {
		ch->println("Syntax:   founder [name]");
		return false;
    }
	
    free_string( pClan->m_pFounder );
    pClan->m_pFounder = str_dup( argument );
	
    ch->println("Clan founder set.");
...
bool clanedit_colour(char_data *ch, char *argument)
{
    ...
    if ( IS_NULLSTR(argument) || c_str_len(argument)>0)
    {
		...
		{
            ch->println("The color code should no contain any visible characters!\r\n");
		}
        ch->println("Syntax:   color <code>");
        ch->wrapln( "This is the color code prefixed to the clan name and "
			"whoname in some situations, it should be non visible color codes" );        
		return false;
    }

    ch->printlnfbw( "Clan color code changed from '%s' to '%s'",
		pClan->m_pColorStr, argument);
    replace_string( pClan->m_pColorStr, argument );
    return true;
}
/**************************************************************************/
bool clanedit_hidden( char_data *ch, char *argument )
{
	CClanType *pC;
    ...
    {
	pC->m_Hidden = false;
	ch->printlnf ( "The clan is no longer hidden on the who list.");
    } else
    {
	pC->m_Hidden = true;
	ch->printlnf ( "The clan is now hidden on the who list.");
    }
    return true;
}
