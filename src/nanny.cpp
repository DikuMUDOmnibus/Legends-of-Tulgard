...
{	time_t rawtime;
	...
	if(ptm->tm_hour==4 && ptm->tm_min>29)
	{
		return 0;
	} else
	if(ptm->tm_hour==4 && ptm->tm_min==29)
	...
	if(ptm->tm_hour==4 && ptm->tm_min==28)
	{
		return 2;
	} else
	if(ptm->tm_hour==4 && ptm->tm_min==27)
	{
		return 3;
	} else
	if(ptm->tm_hour==4 && ptm->tm_min==26)
	{
		return 4;
	} else
	if(ptm->tm_hour==4 && ptm->tm_min==25)
	{
		return 5;
	} 
	return 10;
...
void nanny_char_version_updates(char_data *ch)
{
    ...
    if(ch->short_descr[0]=='\0')
    {
		...
		ch->orig_short_descr = str_dup(tempbuf);
    }
	...
	if(ch->version < 10 && IS_OLD_WEREWOLF(ch))
		ch->werecircle = 10-(ch->remort+1);
...
void nanny_enter_unlock_id(connection_data *d, const char *argument)
{
	...
	if(!str_cmp("change",argument)){
		...
		"`1  Your e-mail must be from your ISP. `YHotmail, Yahoo, Gmail or any "
		"other free e-mail accounts ARE NOT ALLOWED.`x If you do not know your "
		"e-mail address, contact your ISP. `1"
...
/*		// not supported at this stage
	...
	if ( class_table[ch->clss].class_cast_type == CCT_BARD 
	  && ch->pcdata->learned[gsn_bardic_visions]==0)
		 ch->pcdata->learned[gsn_bardic_visions]=1;

	if ( class_table[ch->clss].class_cast_type == CCT_BARD 
          && ch->pcdata->learned[gsn_bardic_peace]==0)
		 ch->pcdata->learned[gsn_bardic_peace]=1;

	if ( class_table[ch->clss].class_cast_type == CCT_BARD 
          && ch->pcdata->learned[gsn_enhanced_healing]==0)
		 ch->pcdata->learned[gsn_enhanced_healing]=1;

	if ( class_table[ch->clss].class_cast_type == CCT_BARD 
          && ch->pcdata->learned[gsn_summon_light]==0)
		 ch->pcdata->learned[gsn_summon_light]=1;

	if ( class_table[ch->clss].class_cast_type == CCT_BARD 
          && ch->pcdata->learned[gsn_kill_magic]==0)
		 ch->pcdata->learned[gsn_kill_magic]=1;

	if ( class_table[ch->clss].class_cast_type == CCT_BARD 
          && ch->pcdata->learned[gsn_sober_drunks]==0)
		 ch->pcdata->learned[gsn_sober_drunks]=1;

	if ( class_table[ch->clss].class_cast_type == CCT_BARD 
          && ch->pcdata->learned[gsn_fog_of_truth]==0)
		 ch->pcdata->learned[gsn_fog_of_truth]=1;

	if ( class_table[ch->clss].class_cast_type == CCT_BARD 
          && ch->pcdata->learned[gsn_battle_rage]==0)
		 ch->pcdata->learned[gsn_battle_rage]=1;

	if ( class_table[ch->clss].class_cast_type == CCT_BARD 
          && ch->pcdata->learned[gsn_create_sustinance]==0)
		 ch->pcdata->learned[gsn_create_sustinance]=1;

	if ( class_table[ch->clss].class_cast_type == CCT_BARD 
          && ch->pcdata->learned[gsn_sleepytime]==0)
		 ch->pcdata->learned[gsn_sleepytime]=1;

	if ( class_table[ch->clss].class_cast_type == CCT_BARD 
          && ch->pcdata->learned[gsn_natures_howl]==0)
		 ch->pcdata->learned[gsn_natures_howl]=1;

	if ( class_table[ch->clss].class_cast_type == CCT_BARD 
          && ch->pcdata->learned[gsn_dreamscape]==0)
		 ch->pcdata->learned[gsn_dreamscape]=1;
	...
	******************************************************/
	...
	if(ch->race == race_lookup("werewolf-c")
	...
	if(ch->race == race_lookup("dragon-form"))
	{
		ch->altform.type = ALTTYPE_DRAGONFORM;
		revert_race1(ch);
	}else 
	if(ch->version < 10 && ch->race == race_lookup("dragon") && ch->level!=1)
	{
		ch->altform.type = ALTTYPE_DRAGONFORM;
		revert_race2(ch);
	}else 
	...
	if(ch->remort > 0)
	{
		affect_parentspellfunc_strip( ch, gsn_remort);
...
void nanny_get_name(connection_data *d, char *argument)
{
	...
	{ 
		if(maint_time()!=10 )
		{
			write_to_buffer( d, "\r\nThe game currently undergoing system maintenance.\r\nPlease try back after 5:00 EST.\r\n", 0 );
			if(!IS_IMMORTAL(ch) && ch->level<100 &&!ch->host_validated && !d->ident_confirmed)
			{
				connection_close( d );
				return;
			}			
...
void nannysup_process_correct_connect_password(connection_data *d)
{
	...
	nanny_char_version_updates(ch);
 	...
 	{
 		if(ch->pcdata->learned[gsn_werewolf] == 100)
...
void nanny_get_new_race(connection_data *d, char *argument)
{
	...
	if (ch->race == race_lookup("dragon"))
	{
		CH(d)->printf("`=j%s`x", creation_titlebar("DRAGON BREED SELECTION"));
    		write_to_buffer( d, "`REVIL: `Y1. `wred, `Y2. `wblack, 3. `wblue, `Y4. `wwhite, `Y5. `wgreen.\r\n`WGOOD: `Y6. `wgold, `Y7. `wsilver, `Y8. `wbrass, `Y9. `wcopper, `Y10. `wbronze\r\n", 0 );
    		write_to_buffer( d, "`CNEUTRAL: `Y11. `wcrystal, `Y12. `wemerald, `Y13. `wcloud, `Y14. `wsteel, `Y15. `wshadow\r\n`x", 0 );
    		write_to_buffer( d, "\r\nWhat is your breed of dragon (`#`Rnumeric`^)? ", 0 );

		if(IS_IRCCON(d)){
			write_to_buffer( d, "\r\n", 0 );
		}

		d->connected_state = CON_GET_DRAGON_BREED;
		return;
	}
...
/**************************************************************************/
void nanny_get_dragon_breed(connection_data *d, const char *argument)
{
	char_data *ch=d->character;

	ch->altform.type = ALTTYPE_DRAGONFORM;
	ch->pcdata->learned[gsn_draconic] = 100;
	ch->pcdata->learned[gsn_common] = 100;

	int dform = atoi(argument);
	...
	switch( dform )
	{
		...
		default:
	    		write_to_buffer( d, "That's not a proper dragon breed.\r\n", 0 );
    			write_to_buffer( d, "`REVIL: `Y1. `wred, `Y2. `wblack, 3. `wblue, `Y4. `wwhite, `Y5. `wgreen.\r\n`WGOOD: `Y6. `wgold, `Y7. `wsilver, `Y8. `wbrass, `Y9. `wcopper, `Y10. `wbronze\r\n", 0 );
    			write_to_buffer( d, "`CNEUTRAL: `Y11. `wcrystal, `Y12. `wemerald, `Y13. `wcloud, `Y14. `wsteel, `Y15. `wshadow\r\n`x", 0 );
    			write_to_buffer( d, "\r\nWhat is your breed of dragon (`#`Rnumeric`^)? ", 0 );
			if(IS_IRCCON(d))
...
void nanny_get_alliance(connection_data *d, const char *argument)
{
	...
	if(is_number(argument))
	{
		if (ch->clss == class_lookup("bard")) 
		{
			if(atoi(argument)<-1 || atoi(argument)>3)
			{
				write_to_buffer(d,"Value between -1 and 3 please:",0);
				return;
			}
		...
		if (ch->race == race_lookup("dragon"))
		{
			    ...
			    ch->altform.subtype == DRAGON_COPPER) 
			{
				if(atoi(argument)<2)
				{
					write_to_buffer(d,"Value between 2 and 3 please:",0);
					return;
				}
			...
			if (ch->altform.subtype == DRAGON_CRYSTAL || 
			    ...
			    ch->altform.subtype == DRAGON_CLOUD) 
			{
				if(atoi(argument)<-1 || atoi(argument)>1)
				{
					write_to_buffer(d,"Value between -1 and 3 please:",0);
					return;
				}
		...
		else if(GAMESETTING(GAMESET_MAX_ALIGN_RANGE22)){
		...
		if (ch->clss == class_lookup("anti-paladin") ||
		...
		{  	
			ch->alliance = -3;
			write_to_buffer(d,"Alliance set to -3 due to class/race.",0);
	   	}

		if (ch->clss == class_lookup ("paladin") ||
		    ch->race == race_lookup("angel"))
		{  	
			ch->alliance = 3;
			write_to_buffer(d,"Alliance set to 3 due to class/race.",0);
	   	}

		if (ch->race == race_lookup("dryad"))
		{  	
			ch->alliance = 0;
			write_to_buffer(d,"Alliance set to 0 due to dryad race.",0);
	   	}
	...
	if (ch->clss == class_lookup("bard")) {
				write_to_buffer(d,
	"this value can be from -3 to 0(including 0 being neutral) 0 being neutral,\r\n"
	...
	"lawful and -3 being extremely chaotic.  \r\nWhat is your choice? ",0); }
		...
		if (ch->race == race_lookup("cambion"))
		{  	
			ch->alliance = -3;
			write_to_buffer(d,"Tendancy set to -3 due to your race.",0);
	   	}
...
void nanny_get_tendency(connection_data *d, const char *argument)
{
	...
	if(is_number(argument))
	{
		...
		}else if (ch->clss == class_lookup("bard")) {

			if(atoi(argument)<-3 || atoi(argument)>1)
			{
				write_to_buffer(d,"Value between -3 and 1 please:",0);
				return;
			}
	...
	case 'y': case 'Y': 
		{
			...
			ch->gen_data = new_gen_data();
			ch->gen_data->points_chosen = ch->pcdata->points;
			SET_BIT(ch->territory1, SUPERBONUS);
			do_help(ch,"group-header");
			list_group_costs(ch);
			write_to_buffer(d,"You already have the following skills:\r\n",0);
			do_skills(ch,"");
...
//		argument, *argument, *(argument+1) ); // debug testing code
    ...
    switch( d->connected_state )
    {
		...
		case CON_GET_DRAGON_BREED:		nanny_get_dragon_breed(d, argument); break;
...
/**************************************************************************/
void do_relogin( char_data *ch, char *argument)
{
    	connection_data *d;
    	char_data *oldch;
    	bool fOld;
	...
	if(IS_SET(ch->dyn,DYN_USING_ATLEVEL)){
		ch->println( "You can't relogin while using at level!" );
		return;
	}

 	if(IS_SET(ch->in_room->room2_flags,ROOM2_ARENA))
	{
	  	ch->println("You can't relogin while in the arena!");
	   	return;
	}
 	...
 	if(!IS_SET(ch->act, PLR_CAN_ADVANCE ))
	{
		...
		if (IS_IMMORTAL(ch)){
			...
			ch->println( "You are not in your mortal body, type `=Creturn`x to get back to it then relogin.!" );
		}
        	return;
    	}
    	...
    	if(IS_NPC(ch))
	{
        	ch->println("Only players can use this command.");
        	return;
    	}

	if(ch->pload && ch->pload->dont_save){
		if(ch->pload->loaded_by){
			ch->pload->loaded_by->printlnf( "do_relogin(): character %s can't 'relogin' cause it was ploaded.", ch->name);
		}			
		logf( "do_relogin(): character %s can't 'relogin' cause it was ploaded.", ch->name);
		return;
	}
	...
	{
            ch->println( "You can not relogin while a moot is called against you." );
            return;
        }
   ...
   if(IS_NULLSTR(argument))
	{
          	ch->println("Syntax: RELOGIN <playername> <password>");
          	return;
	} 
	...
	if (IS_IMMORTAL(ch))
   	{
	    	sprintf( log_buf, "Immortal relogin of %s DENIED to %s@%s.", ch->name, capitalize(name), d->remote_hostname );
	    	log_string( log_buf );
		wiznet(log_buf,NULL,NULL,WIZ_SECURE,0,get_trust(ch));
       		write_to_buffer( d, "You cannot relogin an Immortal.\n\r", 0 );
      		d->character = oldch;
       		return;
   	}
   	...
   	if ( IS_SET(ch->act, PLR_DENY) )
   	{
       		logf("Denying access to %s@%s.", capitalize(name), d->remote_hostname );
       		write_to_buffer( d, "You are denied access.\n\r", 0 );
      		d->character = oldch;
       		return;
   	}

       	if (GAMESETTING(GAMESET_PLAYER_LOCKED)&& !IS_IMMORTAL(ch))
       	{
	...
	if ( fOld )
   	{
		if(!is_valid_password(argument, ch->pcdata->pwd, d))
    		...
    		act( "$n fades from the realm.", oldch, NULL, NULL, TO_ROOM );
		info_broadcast(oldch, "%s has quit.", oldch->name); 
   	...
   	{
      		write_to_buffer( d,"That character does not exist.\n\r",0);
   		d->character = oldch;
   		return;
   	}
}
/**************************************************************************/
void do_createnewpc( char_data *oc, char *argument)
{
    	char_data *ch=NULL;
    	char name[MIL];
		int stat;

	if ( IS_SET( oc->dyn, DYN_IS_BEING_ORDERED ))
	{
		if ( oc->master )
			oc->master->println( "Not going to happen." );
		return;
	}

    	argument= one_argument(argument, name);

	if(IS_SET(oc->dyn,DYN_USING_ATLEVEL)){
		oc->println( "You can't createnewpc while using at level!" );
		return;
	}

 	if(IS_SET(oc->in_room->room2_flags,ROOM2_ARENA))
	{
	  	oc->println("You can't createnewpc while in the arena!");
	   	return;
	}
    	...
    	{
		if (IS_IMMORTAL(oc)){
			oc->println( "You are currently controlling a mob... type `=Creturn`x first!" );
		}else{
			oc->println( "You are not in your mortal body, type `=Creturn`x to get back to it then relogin.!" );
		}
        	return;
    	}
    	...
    	if(IS_NPC(oc))
	{
        	oc->println("Only players can use this command.");
        	return;
    	}

	if(oc->pload && oc->pload->dont_save){
		if(oc->pload->loaded_by){
			oc->pload->loaded_by->printlnf( "do_createnewpc(): character %s can't 'relogin' cause it was ploaded.", oc->name);
		}			
		logf( "do_relogin(): character %s can't 'createnewpc' cause it was ploaded.", oc->name);
		return;
	}

        if ( oc->position == POS_FIGHTING )
        {
            oc->println( "No way! You are fighting." );
            return;
        }

 	if(!IS_SET(oc->act, PLR_CAN_ADVANCE ))
	{
	  	oc->println("You are not letgained.");
	   	return;
	}
	...
	if (moot->moot_type<0)
	{
        ...
        if(oc->pcdata->diplomacy && moot->moot_type)
	{
            	oc->println( "You can not quit while a moot is in progress." );
           	return;
        }
    
        if(moot->moot_victim==oc)
	{
            oc->println( "You can not relogin while a moot is called against you." );
            return;
        }
       	...
       	if(oc->pknoquit>0)
	{
		if (!IS_IMMORTAL(oc))
		{
			oc->println( "You may not leave this world so soon after PK interactions." );
			oc->println( "(you have a pknoquit timer)" );
		}
       		return;
       	}
		
       	if ( oc->position  < POS_STUNNED  )
       	{
       		oc->println( "You're not DEAD yet." );
       		return;
    	}
	...
	replace_string(ch->name	, capitalize(name) );
	...
	act( "$n fades from the realm.", oc, NULL, NULL, TO_ROOM );
	info_broadcast(oc, "%s has quit.", oc->name); 
	ch->desc		= oc->desc;
	oc->desc = NULL;
...
//	ch->printf( "`1Type in the name of the race you wish to play now:`1");
	...
	ch->desc->connected_state=CON_NAME_SELECT;
	ch->printf("`=j%s`x", creation_titlebar("NAME REQUIREMENTS"));
	ch->lines += 15;
	ch->wrapln(		
		"The first stage in creating a character is to come up with a name "
