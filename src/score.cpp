...
void show_score( char_data *ch, char_data *v)
{
	...
	ch->printlnf("`=S|\\`=s___________________________________________________________________________`=S/|");
	if(IS_VAMPAFF(v, VAM_BRUJAH))
	ch->printlnf("`=S|`=s|`x Clan: Brujah     Bloodpool: %3d/%3d       Vampire Generation: %2d          `=s|`=S|",
		  v->bloodpool, bp, v->vampgen);
	if(IS_VAMPAFF(v, VAM_ASSAMITE))
	ch->printlnf("`=S|`=s|`x Clan: Assamite   Bloodpool: %3d/%3d       Vampire Generation: %2d          `=s|`=S|",
		  v->bloodpool, bp, v->vampgen);
	if(IS_VAMPAFF(v, VAM_CAPPADOCIAN))
	ch->printlnf("`=S|`=s|`x Clan: Cappadocian   Bloodpool: %3d/%3d       Vampire Generation: %2d       `=s|`=S|",
		  v->bloodpool, bp, v->vampgen);
	if(IS_VAMPAFF(v, VAM_GANGREL))
	ch->printlnf("`=S|`=s|`x Clan: Gangrel       Bloodpool: %3d/%3d       Vampire Generation: %2d       `=s|`=S|",
		  v->bloodpool, bp, v->vampgen);
	if(IS_VAMPAFF(v, VAM_MALKAVIAN))
	ch->printlnf("`=S|`=s|`x Clan: Malkavian     Bloodpool: %3d/%3d       Vampire Generation: %2d       `=s|`=S|",
		  v->bloodpool, bp, v->vampgen);
	if(IS_VAMPAFF(v, VAM_NOSFERATU))
	ch->printlnf("`=S|`=s|`x Clan: Nosferatu     Bloodpool: %3d/%3d       Vampire Generation: %2d       `=s|`=S|",
		  v->bloodpool, bp, v->vampgen);
	if(IS_VAMPAFF(v, VAM_TOREADOR))
	ch->printlnf("`=S|`=s|`x Clan: Toreador      Bloodpool: %3d/%3d       Vampire Generation: %2d       `=s|`=S|",
		  v->bloodpool, bp, v->vampgen);
	if(IS_VAMPAFF(v, VAM_TREMERE))
	ch->printlnf("`=S|`=s|`x Clan: Tremere       Bloodpool: %3d/%3d       Vampire Generation: %2d       `=s|`=S|",
		  v->bloodpool, bp, v->vampgen);
	if(IS_VAMPAFF(v, VAM_VENTRUE))
	ch->printlnf("`=S|`=s|`x Clan: Ventrue       Bloodpool: %3d/%3d       Vampire Generation: %2d       `=s|`=S|",
		  v->bloodpool, bp, v->vampgen);
	ch->printlnf("`=S|`=s|___________________________________________________________________________|`=S|");
	ch->printlnf("`=S|/___________________________________________________________________________\\|`x");
    }
	...
	if(v->werecircle == 1)
	{
		gp = 200;
			...
			ch->printlnf("`=S|\\`=s___________________________________________________________________________`=S/|");
				ch->printlnf("`=S|`=s|`x Breed: %-13s      Gnosis: %3d/%3d       Lycanthrope Circle: %2d    `=s|`=S|",
				capitalize(were_types[v->altform.subtype].name), v->gnosis, gp, v->werecircle);
			ch->printlnf("`=S|`=s|___________________________________________________________________________|`=S|");
			ch->printlnf("`=S|/___________________________________________________________________________\\|`x");
	}
    if(v->fealty)
    {	
	ch->printlnf("`=S|\\`=s___________________________________________________________________________`=S/|");
	ch->printlnf("`=S|`=s|`x            Title: %-15s             Leige: %-16s     `=s|`=S|",
		  capitalize( flag_string(title_flags, v->title)), v->fealty);
	ch->printlnf("`=S|`=s|___________________________________________________________________________|`=S|");
	ch->printlnf("`=S|/___________________________________________________________________________\\|`x");
    }
    ...
    {	
	ch->printlnf("`=S|\\`=s___________________________________________________________________________`=S/|");
	ch->printlnf("`=S|`=s|`x            Deity: %-15s     Quazer Points: %-16d     `=s|`=S|",
		  v->deity->name, v->quazer);
	ch->printlnf("`=S|`=s|___________________________________________________________________________|`=S|");
	ch->printlnf("`=S|/___________________________________________________________________________\\|`x");
    }else
    {	
	ch->printlnf("`=S|\\`=s___________________________________________________________________________`=S/|");
	ch->printlnf("`=S|`=s|`x            Deity: None                Quazer Points: %-16d     `=s|`=S|", v->quazer);
	ch->printlnf("`=S|`=s|___________________________________________________________________________|`=S|");
	ch->printlnf("`=S|/___________________________________________________________________________\\|`x");
    }
	if(IS_IMMORTAL(v))
	...
	if (IS_AFFECTED2(v, AFF2_BOUNTYHUNTER))
		ch->println("`#`YYou are a licensed bounty hunter.");
	
	if (HAS_CONFIG2( v, CONFIG2_CLOAKED))
		ch->println("`#`CYour cloak is hiding your face.");
	
	if (v->pcdata->jail >= current_time) {
		ch->printlnf("Your prison release date is %s (%.24s)", 
			get_shorticdate_from_time(v->pcdata->jail, "", 0),
			ctime(&v->pcdata->jail));
		
		ch->printlnf("The current date is %s (%.24s)", 
			get_shorticdate_from_time(current_time, "", 0),
			ctime(&current_time));
	}

	if (IS_AFFECTED2(v, AFF2_PREGNANT)){
		ch->printlnf("You will give birth on %s (%.24s)", 
			get_shorticdate_from_time(v->pcdata->pregnant, "", 0),
			ctime(&v->pcdata->pregnant));
