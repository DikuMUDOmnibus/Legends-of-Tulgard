...
void build_staff( char_data *ch, int chance )
{
	...
	vlevel	   = UMAX( ch->level, 10 );
	// first try to get a random damage based on level
	staff->wear_flags = OBJWEAR_TAKE;
	staff->wear_flags += OBJWEAR_WIELD;
	staff->level	= vlevel;
	vlevel += number_range(  (-7 * (100 - get_skill(ch, gsn_build)) / 100), 
							 (7 * get_skill(ch, gsn_build) / 100));
	staff->value[1] = weapon_stat_lookup( vlevel+3, 0, 0 );
	staff->value[2] = weapon_stat_lookup( vlevel+3, 0, 1 );
	staff->value[3] = 25;
