...
SPRESULT spell_rocky_skin( int sn, int level, char_data *ch, void *vo,int )
{
	...
	victim->println( "Your skin becomes rock hard!" );
		if(check_immune(ch, DAM_ACID) != IS_IMMUNE)
	{
	af.where	 = WHERE_VULN;
...
SPRESULT spell_cyclone( int sn, int level, char_data *ch, void *vo, int )
{
    ...
    af.type      = sn;
    af.level     = level;
	//So that this spell lasts decently - if cast in mid air it lasts 3x as long
    af.duration  = (ch->in_room->sector_type == SECT_AIR ? ch->level/3 : ch->level/10);
